...
void backend_aot_write_c_dispatch_loop(FILE *file, TZrUInt32 functionFlatIndex, TZrUInt32 instructionCount) {
    ...
    fprintf(file,
            "        default: ZR_AOT_C_RETURN(ZrLibrary_AotRuntime_ReportUnsupportedInstruction(state, %u, zr_aot_next_instruction, 0));\n",
            (unsigned)functionFlatIndex);
    fprintf(file, "    }\n");
}
...
void backend_aot_write_c_direct_jump_if_greater_signed(FILE *file,
    ...
    fprintf(file,
            ...
            "        ZR_AOT_C_GUARD(ZrLibrary_AotRuntime_ShouldJumpIfGreaterSigned(state, &frame, %u, %u, &zr_aot_should_jump));\n"
...
void backend_aot_write_c_tail_return(FILE *file, TZrUInt32 sourceSlot, TZrBool publishExports) {
    ...
    if (publishExports) {
        fprintf(file,
                "    ZR_AOT_C_RETURN(ZrLibrary_AotRuntime_Return(state, &frame, %u, ZR_TRUE));\n",
                (unsigned)sourceSlot);
        return;
    }
