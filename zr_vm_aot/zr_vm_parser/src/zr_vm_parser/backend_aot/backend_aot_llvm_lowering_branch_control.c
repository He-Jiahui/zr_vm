...
static TZrBool backend_aot_llvm_lower_jump_if_null_instruction(const SZrAotLlvmLoweringContext *context,
                                                               const SZrAotLlvmInstructionContext *instruction) {
    TZrInt64 targetIndex =
            (TZrInt64)instruction->instructionIndex + (TZrInt64)(TZrInt16)instruction->operandB1 + 1;
    TZrChar checkedLabel[96];
    TZrChar targetLabel[96];
    TZrChar argsBuffer[256];
    ...
    backend_aot_llvm_make_instruction_label(checkedLabel,
                                            sizeof(checkedLabel),
                                            context->entry->flatIndex,
                                            instruction->instructionIndex,
                                            "null_checked");
