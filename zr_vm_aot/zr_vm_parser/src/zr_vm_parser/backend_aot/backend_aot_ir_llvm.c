...
TZrBool backend_aot_ir_llvm_emit(
    ...
    return backend_aot_ir_llvm_emit_ex(module, options, ZR_FALSE,
                                       outFacts, diagnostic);
}

TZrBool backend_aot_ir_llvm_emit_ex(
        const SZrAotIrModule *module,
        const SZrAotIrEmitOptions *options,
    ...
    if (!backend_aot_ir_adapter_emit_target(module, ZR_AOT_IR_EMITTER_LLVM,
