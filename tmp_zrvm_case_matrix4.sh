set -e
cd /mnt/e/Git/zr_vm
mkdir -p /tmp/zrvm-cases4
cat > /tmp/zrvm-cases4/release_both.zr <<'EOF'
class Loop {
...
EOF
cat > /tmp/zrvm-cases4/release_both_after.zr <<'EOF'
class Loop {
    pub @call(n: int, acc: int): int {
        if (n == 0) { return acc; }
        return this(n - 1, acc + 1);
...
return directValue + metaValue + guardedValue;
EOF
cat > /tmp/zrvm-cases4/release_owner_after.zr <<'EOF'
class Loop {
    pub @call(n: int, acc: int): int {
        if (n == 0) { return acc; }
        return this(n - 1, acc + 1);
...
return directValue + metaValue + guardedValue;
EOF
cat > /tmp/zrvm-cases4/release_alias_after.zr <<'EOF'
class Loop {
    pub @call(n: int, acc: int): int {
        if (n == 0) { return acc; }
        return this(n - 1, acc + 1);
...
return directValue + metaValue + guardedValue;
EOF
for f in /tmp/zrvm-cases4/*.zr; do
  echo "==== $(basename "$f")"
