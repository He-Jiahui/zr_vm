...
TZrBool ZrParser_ExecIr_LayoutPackedFrame(const SZrExecIrPackedFrameRequest *request,
    ...
    if (request->valueCount > 0u) {
        ...
        for (i = 0u; i < request->valueCount; ++i) {
            ...
            if (v->valueId == ZR_EXEC_IR_VALUE_ID_INVALID ||
                ...
                ((v->flags & ZR_EXEC_IR_PACKED_SLOT_PARAMETER) != 0u &&
                 i >= request->parameterPrefixCount) ||
    ...
    for (i = 0u; i < request->valueCount; ++i) {
        ...
        hash = hash_mix(hash, request->values[i].flags);
        hash = hash_mix(hash, request->values[i].typeToken);
        hash = hash_mix(hash,
                        ((TZrUInt64)request->values[i].byteSize << 32u) |
                        request->values[i].byteAlign);
    }
