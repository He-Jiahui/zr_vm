...
TZrBool note_inline_struct_result_slot(SZrCompilerState *cs,
    ...
    if (cs == ZR_NULL || stackSlot == ZR_PARSER_SLOT_NONE || typeName == ZR_NULL) {
        return ZR_TRUE;
    }
    ...
    ZrParser_InferredType_InitFull(cs->state, &resultType, ZR_VALUE_TYPE_OBJECT, ZR_FALSE, typeName);
    ok = compiler_register_stack_slot_type_hint(cs, stackSlot, &resultType);
    ZrParser_InferredType_Free(cs->state, &resultType);
    return ok;
}

static TZrBool note_inline_struct_member_result_slot(SZrCompilerState *cs,
                                                     TZrUInt32 stackSlot,
                                                     SZrString *fieldTypeName) {
    return note_inline_struct_result_slot(cs, stackSlot, fieldTypeName);
}
...
static const SZrTypeMemberInfo *find_compiler_type_constructor_member(SZrCompilerState *cs, SZrString *typeName) {
    SZrTypePrototypeInfo *info = find_compiler_type_prototype(cs, typeName);
    if (info == ZR_NULL) {
        return ZR_NULL;
    }
...
static TZrBool type_has_constructor(SZrCompilerState *cs, SZrString *typeName) {
    return find_compiler_type_constructor_member(cs, typeName) != ZR_NULL;
}

static TZrBool type_is_struct_constructor_member(SZrCompilerState *cs,
                                                 SZrString *typeName,
    ...
    if (node->type == ZR_AST_IDENTIFIER_LITERAL) {
        ...
        if (candidateType != ZR_NULL && cs->currentTypeName != ZR_NULL) {
            ...
            if (candidateText != ZR_NULL && strcmp(candidateText, "this") == 0) {
                *outTypeName = cs->currentTypeName;
                *outIsTypeReference = ZR_FALSE;
                return ZR_TRUE;
            }
        }
...
static TZrBool emit_hidden_constructor_call(SZrCompilerState *cs,
    ...
    if (constructorArgs != ZR_NULL) {
        for (TZrSize i = 0; i < constructorArgs->count; i++) {
            ...
            TZrUInt32 argSlot = receiverSlot + 1 + (TZrUInt32)i;
            const SZrInferredType *expectedType =
                    (constructorMemberInfo != ZR_NULL && i < constructorMemberInfo->parameterTypes.length)
                            ? (const SZrInferredType *)ZrCore_Array_Get(
                                      (SZrArray *)&constructorMemberInfo->parameterTypes,
                                      i)
                            : ZR_NULL;
            if (argNode != ZR_NULL &&
                compile_expression_into_slot(cs, argNode, argSlot) == ZR_PARSER_SLOT_NONE) {
                return ZR_FALSE;
            }
            if (expectedType != ZR_NULL && !compiler_register_stack_slot_type_hint(cs, argSlot, expectedType)) {
                return ZR_FALSE;
            }
        }
    ...
    if (syncStructReceiver) {
        /* Struct value construction keeps the object seed separate from the inline result slot. */
        if (splitStructValueResult) {
            resultSlot = allocate_stack_slot(cs);
            if (!note_inline_struct_result_slot(cs, resultSlot, typeName)) {
                return ZR_FALSE;
            }
        }
        emit_instruction(cs,
...
TZrUInt32 emit_shorthand_constructor_instance(SZrCompilerState *cs, const TZrChar *op, SZrString *typeName,
    ...
    if (splitStructValueResult && resultSlot == destSlot) {
        resultSlot = allocate_stack_slot(cs);
        if (!note_inline_struct_result_slot(cs, resultSlot, typeName)) {
            return ZR_PARSER_SLOT_NONE;
        }
...
static TZrUInt32 call_argument_direct_local_slot(
        SZrCompilerState *cs,
        const SZrAstNode *argument) {
    const SZrAstNode *identifier = argument;

    if (cs == ZR_NULL || argument == ZR_NULL) {
        return ZR_PARSER_SLOT_NONE;
    }
    if (argument->type == ZR_AST_PRIMARY_EXPRESSION) {
        const SZrPrimaryExpression *primary =
                &argument->data.primaryExpression;
        if (primary->members != ZR_NULL && primary->members->count > 0U) {
            return ZR_PARSER_SLOT_NONE;
        }
        identifier = primary->property;
    }
    ...
    return find_local_var(cs, identifier->data.identifier.name);
}

static TZrBool call_argument_is_owner_in_reborrow(
        SZrCompilerState *cs,
        const SZrAstNode *argument,
    ...
    if (outSourceSlot != ZR_NULL) {
        *outSourceSlot = ZR_PARSER_SLOT_NONE;
    }
    if (cs == ZR_NULL || argument == ZR_NULL || expectedType == ZR_NULL ||
        outSourceSlot == ZR_NULL ||
        passingMode != ZR_PARAMETER_PASSING_MODE_IN ||
        expectedType->ownershipQualifier != ZR_OWNERSHIP_QUALIFIER_NONE) {
        return ZR_FALSE;
    }
    ...
    ZrParser_InferredType_Init(cs->state, &actualType, ZR_VALUE_TYPE_OBJECT);
    if (ZrParser_ExpressionType_Infer(cs, (SZrAstNode *)argument, &actualType) &&
        (actualType.ownershipQualifier == ZR_OWNERSHIP_QUALIFIER_UNIQUE ||
         actualType.ownershipQualifier == ZR_OWNERSHIP_QUALIFIER_SHARED)) {
        borrowedType = actualType;
        borrowedType.ownershipQualifier = ZR_OWNERSHIP_QUALIFIER_NONE;
        result = ZrParser_InferredType_IsCompatible(
                &borrowedType, expectedType);
    }
    ZrParser_InferredType_Free(cs->state, &actualType);
    if (result) {
...
static TZrUInt32 compile_call_argument_into_slot(
    ...
    if (compiler_semantic_ir_begin_receiver_call(
                ...
                argument->location) == ZR_SEMANTIC_LOAN_ID_INVALID) {
        ZrParser_Compiler_Error(
                cs,
                "Failed to record in-parameter owner reborrow in pre-SemIR",
                argument->location);
        return ZR_PARSER_SLOT_NONE;
    }
    emit_instruction(cs,
...
static TZrBool compile_arguments_against_parameter_types(SZrCompilerState *cs,
    ...
    for (TZrSize index = 0; index < argsToCompile->count; index++) {
        ...
        expectedType = parameterTypes != ZR_NULL && index < parameterTypes->length
                               ? (SZrInferredType *)ZrCore_Array_Get(
                                         (SZrArray *)parameterTypes, index)
                               : ZR_NULL;
        argSlot = compile_call_argument_into_slot(
                cs,
                argNode,
                firstArgSlot + (TZrUInt32)index,
                passingMode != ZR_NULL
                        ? *passingMode
    ...
    for (TZrSize i = memberStartIndex; i < members->count; i++) {
        ...
        if (member->type == ZR_AST_MEMBER_EXPRESSION) {
            ...
            if (rootTypeName != ZR_NULL && memberName != ZR_NULL) {
                ...
                pendingDirectMemberCallIsStructConstructor =
                        (TZrBool)(nextIsFunctionCall &&
                                  bindReceiverForCall &&
                        ...
                        typeMember->isMetaMethod &&
                            ...
                            if (canLowerContiguousViewCall) {
                                ...
                                if (preferredDirectMemberCallResultSlot != ZR_PARSER_SLOT_NONE &&
                                    ...
                                    pendingReceiverWritebackSlot == ZR_PARSER_SLOT_NONE &&
                                    pendingReceiverSourceSlot == currentSlot) {
                                    if (!stage_pending_receiver_binding(
                                                cs,
                                                pendingReceiverSourceSlot,
                                    ...
                                    pendingDirectMemberCallResultSlot =
                                            preferredDirectMemberCallResultSlot;
                                } else if (preferredDirectMemberCallResultSlot != ZR_PARSER_SLOT_NONE &&
                                    pendingReceiverWritebackSlot == ZR_PARSER_SLOT_NONE &&
                                    pendingReceiverSourceSlot == currentSlot &&
                ...
                const SZrInferredType *memberReturnType =
                        hasResolvedMemberSignature
                                ...
                                : (hasContractReturnType
                                           ? &contractReturnType
                                           : ZR_NULL);
                TZrBool returnsReference =
                        memberReturnType != ZR_NULL &&
                        memberReturnType->referenceAccess !=
                                ZR_REFERENCE_ACCESS_NONE;
                TZrBool receiverLoanRecorded =
                        returnsReference &&
                        activeReceiverLoanId != ZR_SEMANTIC_LOAN_ID_INVALID
                                ? compiler_semantic_ir_project_receiver_call_result(
                                          cs,
                                          activeReceiverLoanId,
                                          callResultSlot,
                                          member->location)
                                : compiler_semantic_ir_end_receiver_call(
                                          cs,
                                          activeReceiverLoanId,
                                          member->location);

                if (!receiverLoanRecorded) {
                    ZrParser_Compiler_Error(
                            cs,
                            returnsReference
                                    ? "Failed to project receiver borrow into reference call result"
                                    : "Failed to end receiver borrow in pre-SemIR",
                            member->location);
                    if (argsToCompile != call->args &&
                        argsToCompile != ZR_NULL) {
                        ZrParser_AstNodeArray_Free(
                                cs->state, argsToCompile);
                    }
            ...
            if (!note_call_result_stack_slot_type(
                        cs,
                        currentSlot,
                        hasResolvedMemberSignature
                                ? &resolvedMemberSignature.returnType
                                : (hasResolvedFunctionSignature
                                           ...
                                           : (hasContractReturnType
                                                      ? &contractReturnType
                                                      : (resolvedFunctionType != ZR_NULL ? &resolvedFunctionType->returnType
                                                                                         : ZR_NULL))))) {
                if (argsToCompile != call->args && argsToCompile != ZR_NULL) {
                    ZrParser_AstNodeArray_Free(cs->state, argsToCompile);
                }
