...
TZrBool compiler_semantic_cfg_begin_if(SZrCompilerState *cs,
    ...
    TZrBool allowPendingBreak = ZR_FALSE;
    ...
    if (cs->loopLabelStack.length != 0U) {
        SZrLoopLabel *loopLabel = (SZrLoopLabel *)ZrCore_Array_Get(
                &cs->loopLabelStack,
                cs->loopLabelStack.length - 1U);

        allowPendingBreak = (TZrBool)(
                loopLabel != ZR_NULL &&
                compiler_semantic_cfg_break_through_finally_is_active(
                        cs, loopLabel->semanticBreakBlockId));
    }
    thenFlow = compiler_semantic_cfg_if_arm_flow(
            node->data.ifExpression.thenExpr, allowPendingBreak);
    elseFlow = compiler_semantic_cfg_if_arm_flow(
            node->data.ifExpression.elseExpr, allowPendingBreak);
    if (thenFlow == ZR_COMPILER_SEMANTIC_CFG_ARM_UNSUPPORTED ||
        elseFlow == ZR_COMPILER_SEMANTIC_CFG_ARM_UNSUPPORTED) {
...
TZrBool compiler_semantic_cfg_branch_value(
    ...
    if (cs == ZR_NULL || !cs->preSemanticIrCfgActive ||
        condition == ZR_VALUE_ID_INVALID ||
        trueBlock == ZR_PARSER_CFG_INVALID_BLOCK_ID ||
        falseBlock == ZR_PARSER_CFG_INVALID_BLOCK_ID) {
        return ZR_FALSE;
    }
    cfg = &cs->preSemanticIr.cfg;
    if (!compiler_semantic_cfg_emit_branch(
                cs, trueBlock, condition, range) ||
        !compiler_semantic_cfg_bind_current(
                cs, ZR_PARSER_CFG_TERMINATOR_BRANCH) ||
        !ZrParser_Cfg_Connect(
                cfg, cs->preSemanticIrCfgBlock, trueBlock,
                ZR_PARSER_CFG_EDGE_TRUE_BRANCH, sourceNode) ||
        !ZrParser_Cfg_Connect(
                cfg, cs->preSemanticIrCfgBlock, falseBlock,
                ZR_PARSER_CFG_EDGE_FALSE_BRANCH, sourceNode)) {
        return ZR_FALSE;
    }
    return ZR_TRUE;
}
...
static TZrBool compiler_semantic_cfg_terminate_value_id(
        SZrCompilerState *cs,
        TZrValueId valueId,
        SZrFileRange range,
...
static TZrBool compiler_semantic_cfg_terminate_value(
    ...
    return compiler_semantic_cfg_terminate_value_id(
            cs,
            compiler_semantic_ir_slot_value(cs, valueSlot),
            range,
            opcode,
            terminatorKind);
}
