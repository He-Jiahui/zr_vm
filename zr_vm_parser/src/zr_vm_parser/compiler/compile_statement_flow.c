...
void compile_break_continue_statement(SZrCompilerState *cs, SZrAstNode *node) {
    ...
    if (semanticTargetBlockId != ZR_PARSER_CFG_INVALID_BLOCK_ID &&
        ...
        } else if (hasFinallyContext || hasOwnershipCleanupContext) {
...
static TZrBool compile_destructuring_infer_source_type(SZrCompilerState *cs,
                                                       SZrAstNode *value,
                                                       SZrInferredType *outSourceType) {
    if (cs == ZR_NULL || value == ZR_NULL || outSourceType == ZR_NULL) {
        return ZR_FALSE;
    }

    ZrParser_InferredType_Init(cs->state, outSourceType, ZR_VALUE_TYPE_OBJECT);
...
void compile_destructuring_object(SZrCompilerState *cs, SZrAstNode *pattern, SZrAstNode *value) {
    ...
    TZrBool hasTypedSource = ZR_FALSE;
    ...
    if (destruct->keys != ZR_NULL) {
        for (TZrSize i = 0; i < destruct->keys->count; i++) {
            ...
            if (canEmitMemberSlot) {
                ...
                TZrInstruction getTableInst = create_instruction_2(ZR_INSTRUCTION_ENUM(GET_MEMBER),
                                                                   (TZrUInt16)valueSlot,
