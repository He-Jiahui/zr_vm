...
static void optimizer_classify_instruction(const SZrFunction *function,
    ...
    switch (opcode) {
        ...
        case ZR_INSTRUCTION_ENUM(META_CALL):
            /*
             * operand1[1] holds the argument count (and related call metadata), not a stack slot index.
             * If operand1Index1IsSlot stays true, optimizer_rewrite_instruction_read_slot can remap it during
             * slot renaming and corrupt the call (for example turning a unary call into argCount==0).
             */
            info->operand1Index1IsSlot = ZR_FALSE;
        ...
        case ZR_INSTRUCTION_ENUM(KNOWN_VM_MEMBER_CALL_LOAD1_U8):
            optimizer_info_add_read(info, instruction->instruction.operand.operand0[1]);
            optimizer_info_add_read(info, instruction->instruction.operand.operand0[2]);
            optimizer_info_add_write(info, instruction->instruction.operandExtra);
            info->allowSlotReuse = ZR_FALSE;
            return;
        ...
        case ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_MEMBER_CALL):
            ...
            info->hasRangeRead = ZR_TRUE;
            info->rangeReadStart = instruction->instruction.operandExtra;
            info->rangeReadCount = (TZrUInt16)(instruction->instruction.operand.operand1[1] + 1u);
            optimizer_info_add_write(info, instruction->instruction.operandExtra);
            info->allowSlotReuse = ZR_FALSE;
            return;
...
static TZrBool optimizer_opcode_supports_adjacent_get_stack_forwarding(EZrInstructionCode opcode) {
    switch (opcode) {
        /*
         * SET_STACK is intentionally excluded. Forwarding a GET_STACK temp into
         * SET_STACK changes materialization semantics: SET_STACK may transfer
         * ownership from its source slot, so bypassing the temp can consume the
...
static void optimizer_remap_instruction_slots(TZrInstruction *instruction,
    ...
    switch (opcode) {
        ...
        case ZR_INSTRUCTION_ENUM(KNOWN_VM_MEMBER_CALL_LOAD1_U8):
            ...
            if ((TZrSize)instruction->instruction.operand.operand0[2] < slotCount &&
                slotMap[instruction->instruction.operand.operand0[2]] <= 0xFFu) {
