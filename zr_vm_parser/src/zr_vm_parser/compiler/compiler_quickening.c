...
static TZrBool compiler_quickening_temp_slot_is_dead_after_instruction_cfg(const SZrFunction *function,
                                                                           ...
                                                                           TZrUInt32 instructionIndex,
                                                                           TZrUInt32 slot);
static const SZrFunctionFrameSlotLayout *compiler_quickening_find_frame_slot_layout(const SZrFunction *function,
                                                                                   TZrUInt32 stackSlot);
static TZrBool compiler_quickening_slot_has_inline_struct_layout(const SZrFunction *function, TZrUInt32 stackSlot);
static TZrBool compiler_quickening_slot_requires_materialized_stack_store(const SZrFunction *function,
                                                                          TZrUInt32 stackSlot,
                                                                          TZrUInt32 instructionIndex);
...
static TZrBool compiler_quickening_fuse_jump_if_less_equal_signed(SZrFunction *function);
...
static TZrBool compiler_quickening_fuse_known_native_member_call_recv_u8(SZrFunction *function);
...
static EZrCompilerQuickeningSlotKind compiler_quickening_known_call_result_slot_kind(SZrState *state,
    ...
    if (opcode == ZR_INSTRUCTION_ENUM(KNOWN_VM_MEMBER_CALL_LOAD1_U8) ||
        opcode == ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_MEMBER_CALL_RECV_U8)) {
    ...
    switch (opcode) {
        case ZR_INSTRUCTION_ENUM(SUPER_ARRAY_GET_INT):
            *outVariant = ZR_INSTRUCTION_ENUM(SUPER_ARRAY_GET_INT_PLAIN_DEST);
            return ZR_TRUE;
        case ZR_INSTRUCTION_ENUM(SUPER_ARRAY_GET_INT_ITEMS):
            *outVariant = ZR_INSTRUCTION_ENUM(SUPER_ARRAY_GET_INT_ITEMS_PLAIN_DEST);
            return ZR_TRUE;
        case ZR_INSTRUCTION_ENUM(ADD_INT):
...
static TZrBool compiler_quickening_promote_plain_destination_opcodes(SZrFunction *function,
    ...
    for (index = 0; index < function->instructionsLength; index++) {
        ...
        if (opcode == ZR_INSTRUCTION_ENUM(RESET_STACK_NULL2)) {
            TZrUInt32 secondSlot = (TZrUInt32)instruction->instruction.operand.operand1[0];
            if (secondSlot < aliasCount) {
                plainState[secondSlot] = tempOnlySlots[secondSlot];
            }
        }
...
static TZrBool compiler_quickening_promote_plain_destination_opcodes_with_fresh_blocks(SZrFunction *function) {
    TZrBool *blockStarts;
    TZrBool success;
    ...
    blockStarts = (TZrBool *)calloc(function->instructionsLength, sizeof(*blockStarts));
    ...
    success = compiler_quickening_promote_plain_destination_opcodes(function, blockStarts);
    free(blockStarts);
    return success;
}
...
static TZrBool compiler_quickening_forward_super_array_store_to_load_reads(SZrFunction *function) {
    ...
    for (index = 0; index + 1 < function->instructionsLength; index++) {
        ...
        if (storeOpcode != ZR_INSTRUCTION_ENUM(SUPER_ARRAY_SET_INT) &&
            storeOpcode != ZR_INSTRUCTION_ENUM(SUPER_ARRAY_SET_INT_ITEMS)) {
            continue;
        }
...
static TZrBool compiler_quickening_instruction_can_stay_between_member_load_and_known_call(
        const TZrInstruction *instruction,
        TZrUInt32 functionSlot) {
    EZrInstructionCode opcode;

    if (instruction == ZR_NULL ||
        compiler_quickening_instruction_writes_slot(instruction, functionSlot) ||
        compiler_quickening_instruction_may_read_slot(instruction, functionSlot)) {
        return ZR_FALSE;
    }
...
static TZrBool compiler_quickening_fuse_known_native_member_calls(SZrFunction *function) {
    ...
    for (index = 0; index + 1 < function->instructionsLength; index++) {
        ...
        for (callIndex = index + 1; callIndex < function->instructionsLength; callIndex++) {
            ...
            if (compiler_quickening_instruction_can_stay_between_member_load_and_known_call(candidate, functionSlot)) {
                continue;
            }
...
static TZrBool compiler_quickening_fuse_known_native_member_call_recv_u8(SZrFunction *function) {
    ...
    for (index = 0; index + 2u < function->instructionsLength; index++) {
        ...
        TZrInstruction *receiverStage = &function->instructionsList[index + 1u];
        TZrUInt32 resultSlot;
        TZrUInt32 receiverDestinationSlot;
        TZrUInt32 receiverSourceSlot;
        TZrUInt32 callIndex;

        if (blockStarts[index + 1u] ||
            receiverLoad->instruction.operationCode != ZR_INSTRUCTION_ENUM(GET_STACK) ||
            receiverStage->instruction.operationCode != ZR_INSTRUCTION_ENUM(SET_STACK)) {
            continue;
        }

        resultSlot = receiverLoad->instruction.operandExtra;
        receiverDestinationSlot = receiverStage->instruction.operandExtra;
        receiverSourceSlot = (TZrUInt32)receiverLoad->instruction.operand.operand2[0];
        if (receiverDestinationSlot != resultSlot + 1u ||
            (TZrUInt32)receiverStage->instruction.operand.operand2[0] != resultSlot ||
            resultSlot > UINT16_MAX ||
            receiverSourceSlot > 0xFFu) {
            continue;
        }

        for (callIndex = index + 2u; callIndex < function->instructionsLength; callIndex++) {
            TZrInstruction *candidate = &function->instructionsList[callIndex];
            EZrInstructionCode candidateOpcode = (EZrInstructionCode)candidate->instruction.operationCode;
            ...
            if (candidateOpcode == ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_MEMBER_CALL)) {
                TZrUInt32 cacheIndex = (TZrUInt32)candidate->instruction.operand.operand1[0];
                TZrUInt32 parameterCount = (TZrUInt32)candidate->instruction.operand.operand1[1];

                if (candidate->instruction.operandExtra == resultSlot &&
                    parameterCount > 0u &&
                    cacheIndex <= 0xFFu &&
                    parameterCount <= 0xFFu) {
                    *candidate = create_instruction_4(
                            ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_MEMBER_CALL_RECV_U8),
                            (TZrUInt16)resultSlot,
                            (TZrUInt8)cacheIndex,
                            (TZrUInt8)receiverSourceSlot,
                            (TZrUInt8)parameterCount,
                            0u);
                    compiler_quickening_write_nop(receiverLoad);
                    compiler_quickening_write_nop(receiverStage);
                    index = callIndex;
                }
                break;
            }

            if (compiler_quickening_instruction_can_stay_between_member_load_and_known_call(candidate, resultSlot) &&
                !compiler_quickening_instruction_writes_slot(candidate, receiverDestinationSlot) &&
                !compiler_quickening_instruction_may_read_slot(candidate, receiverDestinationSlot)) {
                continue;
            }
...
static TZrBool compiler_quickening_fuse_known_vm_member_call_load1_u8(SZrFunction *function) {
    ...
    for (index = 0; index + 2u < function->instructionsLength; index++) {
        ...
        if (index + 3u < function->instructionsLength &&
            !blockStarts[index + 1u] &&
            !blockStarts[index + 2u] &&
            !blockStarts[index + 3u] &&
            receiverLoad->instruction.operationCode == ZR_INSTRUCTION_ENUM(GET_STACK) &&
            argumentLoad->instruction.operationCode == ZR_INSTRUCTION_ENUM(GET_STACK) &&
            function->instructionsList[index + 2u].instruction.operationCode == ZR_INSTRUCTION_ENUM(SET_STACK) &&
            function->instructionsList[index + 3u].instruction.operationCode ==
                    ZR_INSTRUCTION_ENUM(KNOWN_VM_MEMBER_CALL)) {
            TZrInstruction *argumentStage = &function->instructionsList[index + 2u];
            TZrInstruction *stagedCallInstruction = &function->instructionsList[index + 3u];
            TZrUInt32 argumentTempSlot = argumentLoad->instruction.operandExtra;

            resultSlot = stagedCallInstruction->instruction.operandExtra;
            cacheIndex = (TZrUInt32)stagedCallInstruction->instruction.operand.operand1[0];
            receiverDestinationSlot = receiverLoad->instruction.operandExtra;
            argumentDestinationSlot = argumentStage->instruction.operandExtra;
            receiverSourceSlot = (TZrUInt32)receiverLoad->instruction.operand.operand2[0];
            ...
            if (cacheIndex < function->callSiteCacheLength &&
                ...
                (TZrUInt32)argumentStage->instruction.operand.operand2[0] == argumentTempSlot &&
                resultSlot <= UINT16_MAX &&
                cacheIndex <= 0xFFu &&
                receiverSourceSlot <= 0xFFu &&
                argumentSourceSlot <= 0xFFu) {
                *stagedCallInstruction = create_instruction_4(
                ...
                compiler_quickening_write_nop(argumentStage);
                index += 3u;
            }
            continue;
        }
...
static TZrBool compiler_quickening_fuse_reset_stack_null_pairs(SZrFunction *function) {
    TZrBool *blockStarts = ZR_NULL;
    TZrUInt32 index;
    TZrBool success = ZR_FALSE;
    ...
    for (index = 0; index + 1u < function->instructionsLength; index++) {
        TZrInstruction *firstInstruction = &function->instructionsList[index];
        TZrInstruction *secondInstruction = &function->instructionsList[index + 1u];
        TZrUInt32 firstSlot;
        TZrUInt32 secondSlot;

        if (blockStarts[index + 1u] ||
            (EZrInstructionCode)firstInstruction->instruction.operationCode != ZR_INSTRUCTION_ENUM(RESET_STACK_NULL) ||
            (EZrInstructionCode)secondInstruction->instruction.operationCode != ZR_INSTRUCTION_ENUM(RESET_STACK_NULL)) {
            continue;
        }

        firstSlot = firstInstruction->instruction.operandExtra;
        secondSlot = secondInstruction->instruction.operandExtra;
        if (firstSlot == secondSlot || firstSlot > UINT16_MAX || secondSlot > UINT16_MAX) {
            continue;
        }

        *firstInstruction = create_instruction_2(ZR_INSTRUCTION_ENUM(RESET_STACK_NULL2),
                                                 (TZrUInt16)firstSlot,
                                                 (TZrUInt16)secondSlot,
                                                 0u);
        compiler_quickening_write_nop(secondInstruction);
        index++;
    }
...
static TZrBool compiler_quickening_fuse_set_member_slot_null_pairs(SZrFunction *function) {
    ...
    for (index = 0; index + 1u < function->instructionsLength; index++) {
        TZrInstruction *nullInstruction = &function->instructionsList[index];
        TZrInstruction *setInstruction = &function->instructionsList[index + 1u];
        TZrUInt32 nullSlot;
        TZrUInt32 receiverSlot;
        TZrUInt32 cacheIndex;

        if (blockStarts[index + 1u] ||
            (EZrInstructionCode)nullInstruction->instruction.operationCode !=
                    ZR_INSTRUCTION_ENUM(RESET_STACK_NULL) ||
            (EZrInstructionCode)setInstruction->instruction.operationCode !=
                    ZR_INSTRUCTION_ENUM(SET_MEMBER_SLOT)) {
            continue;
        }

        nullSlot = nullInstruction->instruction.operandExtra;
        receiverSlot = setInstruction->instruction.operand.operand1[0];
        cacheIndex = setInstruction->instruction.operand.operand1[1];
        if (setInstruction->instruction.operandExtra != nullSlot || receiverSlot == nullSlot ||
        ...
        *nullInstruction = create_instruction_2(ZR_INSTRUCTION_ENUM(SET_MEMBER_SLOT_NULL),
                                                0u,
                                                (TZrUInt16)receiverSlot,
                                                (TZrUInt16)cacheIndex);
        compiler_quickening_write_nop(setInstruction);
        index++;
    }
...
static TZrBool compiler_quickening_fuse_reset_null_not_equal_jump_pairs(SZrFunction *function) {
    ...
    for (index = 0; index + 2u < function->instructionsLength; index++) {
        TZrInstruction *nullInstruction = &function->instructionsList[index];
        TZrInstruction *compareInstruction = &function->instructionsList[index + 1u];
        TZrInstruction *jumpInstruction = &function->instructionsList[index + 2u];
        TZrUInt32 nullSlot;
        ...
        if (compareInstruction->instruction.operand.operand1[0] == nullSlot) {
            valueSlot = compareInstruction->instruction.operand.operand1[1];
        } else if (compareInstruction->instruction.operand.operand1[1] == nullSlot) {
            valueSlot = compareInstruction->instruction.operand.operand1[0];
        } else {
            continue;
        }

        if (valueSlot == nullSlot) {
            continue;
        }

        if (compareSlot != nullSlot &&
            !compiler_quickening_temp_slot_is_dead_after_instruction_cfg(function,
                                                                        blockStarts,
                                                                        index + 1u,
                                                                        nullSlot)) {
        ...
        fusedOffset = (TZrInt64)jumpInstruction->instruction.operand.operand2[0] + 2;
        if (fusedOffset < INT16_MIN || fusedOffset > INT16_MAX) {
            continue;
        }

        *nullInstruction = create_instruction_2(ZR_INSTRUCTION_ENUM(JUMP_IF_NULL),
                                                (TZrUInt16)valueSlot,
                                                0u,
                                                (TZrUInt16)((TZrInt16)fusedOffset));
        compiler_quickening_write_nop(compareInstruction);
        compiler_quickening_write_nop(jumpInstruction);
        index += 2u;
    }
...
static TZrBool compiler_quickening_try_get_add_mod_left_right_slots(const TZrInstruction *instruction,
...
static TZrBool compiler_quickening_fuse_add_signed_mod_const_pairs(SZrFunction *function) {
    TZrBool *blockStarts = ZR_NULL;
    TZrUInt32 index;
    TZrBool success = ZR_FALSE;
    ...
    for (index = 0; index + 1u < function->instructionsLength; index++) {
        TZrInstruction *addInstruction = &function->instructionsList[index];
        TZrInstruction *modInstruction = &function->instructionsList[index + 1u];
        TZrUInt32 addDestination;
        TZrUInt32 modDestination;
        TZrUInt32 leftSlot;
        TZrUInt32 rightSlot;
        TZrUInt32 constantIndex;

        if (blockStarts[index + 1u] ||
            !compiler_quickening_try_get_add_mod_left_right_slots(addInstruction, &leftSlot, &rightSlot) ||
            (EZrInstructionCode)modInstruction->instruction.operationCode != ZR_INSTRUCTION_ENUM(MOD_SIGNED_CONST)) {
            continue;
        }

        addDestination = addInstruction->instruction.operandExtra;
        modDestination = modInstruction->instruction.operandExtra;
        constantIndex = modInstruction->instruction.operand.operand1[1];
        if (modInstruction->instruction.operand.operand1[0] != addDestination ||
            leftSlot > UINT8_MAX || rightSlot > UINT8_MAX || constantIndex > UINT16_MAX ||
            compiler_quickening_find_active_local_variable(function, addDestination, index) != ZR_NULL) {
            continue;
        }
        ...
        addInstruction->instruction.operationCode = (TZrUInt16)ZR_INSTRUCTION_ENUM(ADD_SIGNED_MOD_CONST);
        addInstruction->instruction.operandExtra = (TZrUInt16)modDestination;
        addInstruction->instruction.operand.operand0[0] = (TZrUInt8)leftSlot;
        addInstruction->instruction.operand.operand0[1] = (TZrUInt8)rightSlot;
        addInstruction->instruction.operand.operand1[1] = (TZrUInt16)constantIndex;
        compiler_quickening_write_nop(modInstruction);
        index++;
    }
...
static TZrBool compiler_quickening_try_fold_dead_super_array_add_setup(SZrFunction *function,
    ...
    TZrUInt32 receiverReloadIndex;
    TZrUInt32 receiverStageIndex;
    TZrUInt32 receiverLoadIndex;
    TZrUInt32 receiverSourceSlot;
    TZrUInt32 destinationSlot;
    TZrUInt32 receiverSlot;
    TZrUInt32 valueSlot;
    TZrUInt32 blockStartIndex;
    TZrUInt32 scan;
    TZrBool destinationOverwrittenImmediately;

    if (function == ZR_NULL || function->instructionsList == ZR_NULL || blockStarts == ZR_NULL ||
        instructionIndex < 3 || instructionIndex + 1 >= function->instructionsLength) {
        return ZR_FALSE;
    }
    ...
    if (valueSlot == destinationSlot || valueSlot == receiverSlot) {
        return ZR_FALSE;
    }
    ...
    receiverReloadIndex = UINT32_MAX;
    for (scan = instructionIndex; scan > blockStartIndex; scan--) {
        TZrUInt32 candidateIndex = scan - 1u;
        TZrInstruction *candidate = &function->instructionsList[candidateIndex];

        if ((EZrInstructionCode)candidate->instruction.operationCode == ZR_INSTRUCTION_ENUM(GET_STACK) &&
            candidate->instruction.operandExtra == destinationSlot &&
            (TZrUInt32)candidate->instruction.operand.operand2[0] == receiverSlot) {
            receiverReloadIndex = candidateIndex;
            break;
        }

        if (compiler_quickening_instruction_may_read_slot(candidate, destinationSlot) ||
            compiler_quickening_instruction_writes_slot(candidate, destinationSlot) ||
            compiler_quickening_instruction_may_read_slot(candidate, receiverSlot) ||
            compiler_quickening_instruction_writes_slot(candidate, receiverSlot)) {
            return ZR_FALSE;
        }
    }
    if (receiverReloadIndex == UINT32_MAX || receiverReloadIndex < 2) {
        return ZR_FALSE;
    }

    receiverStageIndex = receiverReloadIndex - 1u;
    receiverLoadIndex = receiverReloadIndex - 2u;
    destinationOverwrittenImmediately =
            (TZrBool)(instructionIndex + 1 < function->instructionsLength &&
                      !compiler_quickening_instruction_may_read_slot(
                              &function->instructionsList[instructionIndex + 1], destinationSlot) &&
                      compiler_quickening_instruction_writes_slot(
                              &function->instructionsList[instructionIndex + 1], destinationSlot));

    if (blockStarts[instructionIndex] || blockStarts[receiverReloadIndex] ||
        blockStarts[receiverStageIndex] ||
        (!destinationOverwrittenImmediately && blockStarts[instructionIndex + 1])) {
        return ZR_FALSE;
    }

    receiverReloadInstruction = &function->instructionsList[receiverReloadIndex];
    receiverStageInstruction = &function->instructionsList[receiverStageIndex];
    receiverLoadInstruction = &function->instructionsList[receiverLoadIndex];
    ...
    receiverSourceSlot = (TZrUInt32)receiverLoadInstruction->instruction.operand.operand2[0];
    if (receiverSourceSlot > UINT16_MAX) {
        return ZR_FALSE;
    }
    for (scan = receiverLoadIndex + 1u; scan < instructionIndex; scan++) {
        if (compiler_quickening_instruction_may_read_slot(&function->instructionsList[scan], receiverSourceSlot) ||
            compiler_quickening_instruction_writes_slot(&function->instructionsList[scan], receiverSourceSlot)) {
            return ZR_FALSE;
        }
    }

    if (!destinationOverwrittenImmediately &&
        !compiler_quickening_slot_is_overwritten_before_read(function, blockStarts, instructionIndex + 1, destinationSlot) &&
    ...
    addInstruction->instruction.operand.operand1[0] = (TZrUInt16)receiverSourceSlot;
    addInstruction->instruction.operandExtra = ZR_INSTRUCTION_USE_RET_FLAG;
...
static TZrBool compiler_quickening_try_fold_super_array_add_int4_burst(SZrFunction *function,
    ...
    if ((EZrInstructionCode)firstInstruction->instruction.operationCode == ZR_INSTRUCTION_ENUM(SET_STACK) &&
        instructionIndex + 7 < function->instructionsLength) {
        TZrUInt32 stagedValueSlot = firstInstruction->instruction.operandExtra;
        TZrUInt32 valueSourceSlot = (TZrUInt32)firstInstruction->instruction.operand.operand2[0];
        TZrInstruction *firstAddInstruction = &function->instructionsList[instructionIndex + 1];

        if (valueSourceSlot > UINT16_MAX ||
            (EZrInstructionCode)firstAddInstruction->instruction.operationCode !=
                    ZR_INSTRUCTION_ENUM(SUPER_ARRAY_ADD_INT) ||
            firstAddInstruction->instruction.operandExtra != ZR_INSTRUCTION_USE_RET_FLAG ||
            firstAddInstruction->instruction.operand.operand1[1] != stagedValueSlot) {
            return ZR_FALSE;
        }

        receiverBaseSlot = firstAddInstruction->instruction.operand.operand1[0];
        for (burstIndex = 0; burstIndex < 4; burstIndex++) {
            TZrUInt32 stageIndex = instructionIndex + burstIndex * 2u;
            TZrInstruction *stageInstruction = &function->instructionsList[stageIndex];
            TZrInstruction *addInstruction = &function->instructionsList[stageIndex + 1u];

            if ((burstIndex > 0 && blockStarts[stageIndex]) ||
                blockStarts[stageIndex + 1u] ||
                (EZrInstructionCode)stageInstruction->instruction.operationCode != ZR_INSTRUCTION_ENUM(SET_STACK) ||
                (EZrInstructionCode)addInstruction->instruction.operationCode != ZR_INSTRUCTION_ENUM(SUPER_ARRAY_ADD_INT) ||
                stageInstruction->instruction.operandExtra != stagedValueSlot ||
                (TZrUInt32)stageInstruction->instruction.operand.operand2[0] != valueSourceSlot ||
                addInstruction->instruction.operandExtra != ZR_INSTRUCTION_USE_RET_FLAG ||
                addInstruction->instruction.operand.operand1[0] != receiverBaseSlot + burstIndex ||
                addInstruction->instruction.operand.operand1[1] != stagedValueSlot) {
                return ZR_FALSE;
            }
        }
        ...
        firstInstruction->instruction.operand.operand1[1] = (TZrUInt16)valueSourceSlot;
        compiler_quickening_write_nop(firstAddInstruction);
        for (burstIndex = 1; burstIndex < 4; burstIndex++) {
            compiler_quickening_write_nop(&function->instructionsList[instructionIndex + burstIndex * 2u]);
            compiler_quickening_write_nop(&function->instructionsList[instructionIndex + burstIndex * 2u + 1u]);
        }
        return ZR_TRUE;
    }
...
static TZrBool compiler_quickening_direct_result_store_preserves_producer_reads(
        const TZrInstruction *producerInstruction,
        TZrUInt32 destinationSlot) {
    EZrInstructionCode opcode;

    if (producerInstruction == ZR_NULL) {
        return ZR_FALSE;
    }
...
static TZrBool compiler_quickening_try_fold_direct_result_store(SZrFunction *function,
    ...
    if (compiler_quickening_slot_requires_materialized_stack_store(function,
                                                                   destinationSlot,
                                                                   instructionIndex + 1)) {
        return ZR_FALSE;
    }

    if (!compiler_quickening_direct_result_store_preserves_producer_reads(producerInstruction, destinationSlot)) {
        return ZR_FALSE;
    }
...
static TZrBool compiler_quickening_slot_has_inline_struct_layout(const SZrFunction *function, TZrUInt32 stackSlot) {
    const SZrFunctionFrameSlotLayout *layout = compiler_quickening_find_frame_slot_layout(function, stackSlot);
    return (TZrBool)(layout != ZR_NULL &&
                     layout->slotKind == (TZrUInt8)ZR_FUNCTION_FRAME_SLOT_KIND_INLINE_STRUCT);
}

static TZrBool compiler_quickening_binding_is_named_object_value(const SZrFunctionTypedLocalBinding *binding) {
...
static TZrBool compiler_quickening_slot_requires_materialized_stack_store(const SZrFunction *function,
    ...
    if (compiler_quickening_slot_has_inline_struct_layout(function, stackSlot)) {
        return ZR_TRUE;
    }

    binding = compiler_quickening_find_active_typed_local_binding(function, stackSlot, instructionIndex);
    return compiler_quickening_binding_is_named_object_value(binding);
}

static TZrBool compiler_quickening_slots_share_inline_struct_layout(const SZrFunction *function,
                                                                    TZrUInt32 leftSlot,
                                                                    TZrUInt32 rightSlot) {
    const SZrFunctionFrameSlotLayout *leftLayout =
            compiler_quickening_find_frame_slot_layout(function, leftSlot);
    const SZrFunctionFrameSlotLayout *rightLayout =
            compiler_quickening_find_frame_slot_layout(function, rightSlot);

    return (TZrBool)(leftLayout != ZR_NULL &&
                     rightLayout != ZR_NULL &&
                     leftLayout->slotKind == (TZrUInt8)ZR_FUNCTION_FRAME_SLOT_KIND_INLINE_STRUCT &&
                     rightLayout->slotKind == (TZrUInt8)ZR_FUNCTION_FRAME_SLOT_KIND_INLINE_STRUCT &&
                     leftLayout->typeLayoutId != ZR_FUNCTION_FRAME_TYPE_LAYOUT_ID_NONE &&
                     leftLayout->typeLayoutId == rightLayout->typeLayoutId);
}

static TZrBool compiler_quickening_try_replace_inline_struct_set_stack_source(
        const SZrFunction *function,
        TZrInstruction *instruction,
        TZrUInt32 oldSlot,
        TZrUInt32 newSlot,
    ...
    if (instruction == ZR_NULL ||
        (EZrInstructionCode)instruction->instruction.operationCode != ZR_INSTRUCTION_ENUM(SET_STACK) ||
        (TZrUInt32)instruction->instruction.operand.operand2[0] != oldSlot ||
        !compiler_quickening_slots_share_inline_struct_layout(function,
                                                              newSlot,
                                                              instruction->instruction.operandExtra)) {
        return ZR_TRUE;
    }
...
static TZrBool compiler_quickening_try_forward_get_stack_copy_reads(SZrFunction *function,
    ...
    if (sourceSlot == tempSlot ||
        ((EZrInstructionCode)copyInstruction->instruction.operationCode == ZR_INSTRUCTION_ENUM(SET_STACK) &&
         compiler_quickening_slot_has_any_binding(function, tempSlot)) ||
    ...
    for (scan = instructionIndex + 1; scan <= blockEndIndex; scan++) {
        ...
        if (!compiler_quickening_try_replace_inline_struct_set_stack_source(function,
                                                                            instruction,
                                                                            tempSlot,
                                                                            sourceSlot,
                                                                            ZR_FALSE,
                                                                            &replacedThis) ||
            (!replacedThis &&
             !compiler_quickening_instruction_replace_read_slot_if_supported(
                     instruction,
                     tempSlot,
                     sourceSlot,
                     ZR_FALSE,
    ...
    for (scan = instructionIndex + 1; scan <= stopIndex; scan++) {
        ...
        if (!compiler_quickening_try_replace_inline_struct_set_stack_source(function,
                                                                            &function->instructionsList[scan],
                                                                            tempSlot,
                                                                            sourceSlot,
                                                                            ZR_TRUE,
                                                                            &replacedThis) ||
            (!replacedThis &&
             !compiler_quickening_instruction_replace_read_slot_if_supported(&function->instructionsList[scan],
                                                                             tempSlot,
                                                                             sourceSlot,
                                                                             ZR_TRUE,
                                                                             &replacedThis))) {
...
static TZrBool compiler_quickening_try_fuse_jump_if_less_equal_signed(SZrFunction *function,
                                                                      const TZrBool *blockStarts,
                                                                      TZrUInt32 instructionIndex) {
    TZrInstruction *compareInstruction;
    TZrInstruction *jumpIfInstruction;
    ...
    leftSlot = compareInstruction->instruction.operand.operand1[0];
    rightSlot = compareInstruction->instruction.operand.operand1[1];
    compareInstruction->instruction.operationCode = (TZrUInt16)ZR_INSTRUCTION_ENUM(JUMP_IF_LESS_EQUAL_SIGNED);
    compareInstruction->instruction.operandExtra = (TZrUInt16)leftSlot;
    compareInstruction->instruction.operand.operand1[0] = (TZrUInt16)rightSlot;
    compareInstruction->instruction.operand.operand1[1] = (TZrUInt16)((TZrInt16)fusedOffset);
    compiler_quickening_write_nop(jumpIfInstruction);
    return ZR_TRUE;
}
...
static TZrBool compiler_quickening_fuse_jump_if_less_equal_signed(SZrFunction *function) {
    ...
    for (index = 0; index + 1 < function->instructionsLength; index++) {
        compiler_quickening_try_fuse_jump_if_less_equal_signed(function, blockStarts, index);
    }
...
static TZrBool compiler_quickening_try_fold_right_converted_uint_const_arithmetic(
    ...
    if (instruction->instruction.operandExtra == convertedSlot ||
        instruction->instruction.operandExtra == constantSlot) {
...
static EZrInstructionCode compiler_quickening_signed_load_stack_arithmetic_opcode(EZrInstructionCode opcode) {
    switch (opcode) {
        ...
        case ZR_INSTRUCTION_ENUM(MUL_SIGNED):
        case ZR_INSTRUCTION_ENUM(MUL_SIGNED_PLAIN_DEST):
            return ZR_INSTRUCTION_ENUM(MUL_SIGNED_LOAD_STACK);
        default:
            return ZR_INSTRUCTION_ENUM(ENUM_MAX);
    }
}
...
static TZrBool compiler_quickening_try_fold_super_array_fill_int4_const_loop_compact_fused_self_update(
    ...
    TZrInstruction *fillConstantInstruction;
    TZrInstruction *fillBurstInstruction;
    ...
    if (instructionIndex + 5 < function->instructionsLength) {
        fillConstantInstruction = &function->instructionsList[instructionIndex + 2];
        fillBurstInstruction = &function->instructionsList[instructionIndex + 3];
        incrementInstruction = &function->instructionsList[instructionIndex + 4];
        jumpBackInstruction = &function->instructionsList[instructionIndex + 5];
        ...
        if ((EZrInstructionCode)initConstantInstruction->instruction.operationCode == ZR_INSTRUCTION_ENUM(GET_CONSTANT) &&
            ...
            (EZrInstructionCode)fillConstantInstruction->instruction.operationCode ==
                    ZR_INSTRUCTION_ENUM(GET_CONSTANT) &&
            (EZrInstructionCode)fillBurstInstruction->instruction.operationCode ==
                    ZR_INSTRUCTION_ENUM(SUPER_ARRAY_ADD_INT4) &&
            ...
            for (scan = instructionIndex + 1; scan <= instructionIndex + 5; scan++) {
                if (blockStarts[scan] && scan != instructionIndex + 3) {
                    return ZR_FALSE;
                }
            }

            if (!compiler_quickening_function_constant_read_int64(
                        function,
            ...
            receiverBaseSlot = fillBurstInstruction->instruction.operand.operand1[0];
            fillConstantIndex = (TZrUInt32)fillConstantInstruction->instruction.operand.operand2[0];
            ...
            if (!compiler_quickening_function_constant_is_int(function, fillConstantIndex) ||
                !compiler_quickening_function_constant_read_int64(function,
                                                                  minusOneConstantIndex,
                                                                  &constantValue) ||
                constantValue != 1 ||
                !compiler_quickening_function_constant_read_int64(function,
                                                                  incrementOneConstantIndex,
                                                                  &constantValue) ||
                constantValue != 1) {
                return ZR_FALSE;
            }
            ...
            if (jumpIfGreaterInstruction->instruction.operandExtra != indexSlot ||
                ...
                jumpIfGreaterInstruction->instruction.operand.operand1[1] != 4 ||
                fillBurstInstruction->instruction.operandExtra != ZR_INSTRUCTION_USE_RET_FLAG ||
                fillBurstInstruction->instruction.operand.operand1[1] != fillConstantInstruction->instruction.operandExtra ||
                incrementInstruction->instruction.operandExtra != indexSlot ||
                incrementInstruction->instruction.operand.operand1[0] != indexSlot ||
                jumpBackInstruction->instruction.operand.operand2[0] != -6) {
                return ZR_FALSE;
            }

            if (!compiler_quickening_slot_is_overwritten_before_any_read_linear(function,
                                                                                instructionIndex + 6,
                                                                                indexSlot)) {
                return ZR_FALSE;
            }

            subtractInstruction->instruction.operationCode =
                    (TZrUInt16)ZR_INSTRUCTION_ENUM(SUPER_ARRAY_FILL_INT4_CONST);
            ...
            compiler_quickening_write_nop(fillConstantInstruction);
            compiler_quickening_write_nop(fillBurstInstruction);
    ...
    for (index = 0; index < function->instructionsLength; index++) {
        ...
        switch (opcode) {
            case ZR_INSTRUCTION_ENUM(META_CALL):
                ...
                break;
            case ZR_INSTRUCTION_ENUM(DYN_CALL):
                if (instruction->instruction.operandExtra < instruction->instruction.operand.operand1[0]) {
                    continue;
                }
    ...
    for (index = 0; index < function->instructionsLength; index++) {
        ...
        switch ((EZrInstructionCode)callInst->instruction.operationCode) {
            ...
            case ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_TAIL_CALL):
                ...
                break;
            case ZR_INSTRUCTION_ENUM(DYN_CALL):
                if (callInst->instruction.operandExtra < callInst->instruction.operand.operand1[0]) {
                    break;
                }
                callInst->instruction.operationCode =
...
static TZrBool compiler_quicken_child_functions(SZrState *state,
    ...
    ZR_QUICKENING_RUN_PASS("fuse_jump_if_less_equal_signed",
                           compiler_quickening_fuse_jump_if_less_equal_signed(function));
    ...
    ZR_QUICKENING_RUN_PASS("known_native_member_call_recv_u8",
                           compiler_quickening_fuse_known_native_member_call_recv_u8(function));
    ...
    ZR_QUICKENING_RUN_PASS("fuse_reset_stack_null_pairs",
                           compiler_quickening_fuse_reset_stack_null_pairs(function));
    ZR_QUICKENING_RUN_PASS("fuse_set_member_slot_null_pairs",
                           compiler_quickening_fuse_set_member_slot_null_pairs(function));
    ZR_QUICKENING_RUN_PASS("fuse_reset_null_not_equal_jump_pairs",
                           compiler_quickening_fuse_reset_null_not_equal_jump_pairs(function));
    ZR_QUICKENING_RUN_PASS("fuse_add_signed_mod_const_pairs",
                           compiler_quickening_fuse_add_signed_mod_const_pairs(function));
    ZR_QUICKENING_RUN_PASS("compact_nops_12", compiler_quickening_compact_nops(state, function));
    ZR_QUICKENING_RUN_PASS("known_native_member_call_recv_u8_after_compact",
                           compiler_quickening_fuse_known_native_member_call_recv_u8(function));
    ZR_QUICKENING_RUN_PASS("compact_nops_12b", compiler_quickening_compact_nops(state, function));
    ...
    ZR_QUICKENING_RUN_PASS("promote_plain_destination_after_items_cache",
                           compiler_quickening_promote_plain_destination_opcodes_with_fresh_blocks(function));
    ZR_QUICKENING_RUN_PASS("forward_get_stack_copy_reads_after_items_cache",
                           compiler_quickening_forward_get_stack_copy_reads(function));
    ZR_QUICKENING_RUN_PASS("compact_nops_after_items_cache_copy_forward",
                           compiler_quickening_compact_nops(state, function));
    ZR_QUICKENING_RUN_PASS("forward_super_array_store_to_load_reads_after_items_cache",
                           compiler_quickening_forward_super_array_store_to_load_reads(function));
    ZR_QUICKENING_RUN_PASS("compact_nops_after_items_cache_store_forward",
                           compiler_quickening_compact_nops(state, function));
    ZR_QUICKENING_RUN_PASS("forward_get_stack_copy_reads_after_items_cache_store_forward",
                           compiler_quickening_forward_get_stack_copy_reads(function));
    ZR_QUICKENING_RUN_PASS("compact_nops_after_items_cache_store_copy_forward",
                           compiler_quickening_compact_nops(state, function));
    ZR_QUICKENING_RUN_PASS("fold_direct_result_stores_after_items_cache",
                           compiler_quickening_fold_direct_result_stores(function));
    ZR_QUICKENING_RUN_PASS("compact_nops_after_items_cache_direct_stores",
                           compiler_quickening_compact_nops(state, function));
