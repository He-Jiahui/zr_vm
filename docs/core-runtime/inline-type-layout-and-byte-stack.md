...
- Known native, known native member, generic, and dynamic tail-call native fallback paths stage native call windows outside caller frame storage so native temporary frames cannot overwrite inline struct payload bytes.
