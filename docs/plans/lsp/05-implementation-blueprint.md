...
L8目前完成了六个独立合同：resolved type-reference fact以generic owner base identifier的精确range进入`CanonicalTypeAt`，semantic tokens删除`Unique`/`Shared`/`Weak`名称表；member token只消费semantic query或structured metadata，不能从`.`或`(`推断namespace/method/property；source declaration hover/completion type display只消费resolved canonical type fact，删除AST type/ownership字符串重建并在identity unavailable时显示`cannot infer exact type`；source inlay type display只消费精确SymbolId声明事实和canonical TypeId，identity unavailable时不生成提示；source symbol markdown documentation复用同一identity helper，不能从`symbol->typeInfo`独立生成类型；source property signature按Property SymbolId重新查询PropertyDef并格式化匹配的canonical TypeId，不能从copied contract或`symbol->typeInfo`重建。同名local变量保持variable，unresolved member保持unclassified；其余本地fallback删除、provider/project覆盖与完整protocol矩阵仍未完成，不改变L8整体状态。
