...
这些记录证明query机制可用，并完成Q4中source resolved callable identity/display consumer、Q5中source module public-contract hash的首个canonical query，以及Q5/Q6中的binary export location、descriptor-plugin type-member、native module-function、instance-method与unconstrained generic instance-method call-site callable parity切片；L8 source declaration、inlay、symbol markdown、property signature、project receiver member和project imported symbol type display均已删除对应本地AST/inferred type文本重建，只按精确SymbolId声明/PropertyDef事实与canonical TypeId格式化。native source construct receiver进一步只按receiver-prefix AST node的exact expression fact格式化TypeId；不完整 member completion 也在 semantic/import/scoped fallback 之前消费同一事实，缺失、unknown或invalid则返回空结果并 fail closed。`init TypeRef(...)` 的 `STRUCT_INIT_EXPRESSION` signature help 也只消费同节点 exact fact，不能回退到局部AST type inference或project specialized constructor lookup。source declaration-backed的free、receiver、闭合generic receiver和lambda callable value调用在canonical `CallAt/FormatCall`不可用时也直接不可用，禁止局部overload、member、callee-name、initializer AST或AST specialization重建。lambda value以其精确lambda declaration AST identity发布resolved SymbolId、canonical TypeId与declaration range；hover、definition和signature help均消费同一fact。binary/provider callable value现已发布canonical TypeId与signature display，同时明确保持unresolved SymbolId和空declaration range；其fact缺失时hover/signature不得按name、metadata或initializer AST重建。该external callable leaf已完成GCC/Clang/MSVC focused、interface/project与stdio/CLI复验，不表示全部Canonical/Place/Module facts或完整provider parity已覆盖。
