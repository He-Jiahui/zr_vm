...
- [x] handler 返回 status + result；“没有语义结果”可返回合法 null/empty，“解析失败”只能是 InvalidParams。Sub22–Sub26 已覆盖四十二个普通 handler、initialize 与 response envelope；内部 serializer/runtime 分配、调用方 publication fence 和父级门禁仍按独立子项推进；[Sub26 记录](2026-09-07-plan01-task02-sub26-response-envelope-ownership.md)。
