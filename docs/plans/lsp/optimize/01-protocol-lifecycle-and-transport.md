...
- [x] Sub28：position/range/location、完整诊断树和 document/workspace full/unchanged report 检查所有 JSON 构造及挂接失败，错误逐层传递并清理部分树。587 个直接 serializer/report 分配点和 130 个非空通知分配点的单次/持续注入通过；三工具链 13 项 CTest 全部通过，两项 Valgrind 均为 0 字节/0 错误；[记录](2026-09-07-plan01-task02-sub28-diagnostic-json-ownership.md)。
...
- [x] Sub05：关闭释放文档时同步清理所有匹配的项目 source record，避免诊断枚举残留无 fileVersion 的 URI；三工具链关闭回归 4/4、相关 CTest 3/3、关闭相关协议 3/3，Valgrind 0 字节/0 错误。完整 smoke 均推进至 binary member definition 断言，父门禁未完成；[记录](2026-09-07-plan01-task06-sub05-closed-project-diagnostic-target.md)。
