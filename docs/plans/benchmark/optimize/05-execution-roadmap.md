...
- Interpreter Task 2 packed signed-address follow-up：strict packed VALUE frame 在 outer dispatch 每帧缓存一次 fixed-stride byte-mirror base；signed quickened 算术、比较、转换、fused load、branch 与完整 VALUE destination 复用该地址，call-info/frame identity 变化立即 fail closed，其他类型与布局走原 getter。逐访问 summary 候选 `+7.076%` 被拒绝；最终 numeric `111,746,343 -> 94,984,122 Ir`（`-15.000241%`），mixed/object 为 `+0.437322%/-6.308004%`，checksum 不变。四配置 13 项矩阵通过，frame `37/37`，sanitizer 日志 clean。完整 unboxed payload/materialization 与 Task 2 总门禁仍开放。
