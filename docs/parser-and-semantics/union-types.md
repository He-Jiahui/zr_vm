...
- `switch` and `using` payload bindings inherit the matched variant field's declared type. `Unique<T>` and `Shared<T>` owner payloads become non-consuming `ref readonly T` views by default and lower through `OWN_VIEW_SHARED`; declared `ref` / `ref readonly` payloads preserve their reference mode.
- Block-style `using` payload bindings can opt into owner transfer with a local binding `move` marker:
  ...
  Explicit `move` keeps the payload field's declared owner type, skips `OWN_VIEW_SHARED`, and clears the matched inline union payload after binding so active-variant drop does not release the moved owner again.
...
- Guard-scoped module ownership is compiler-internal: a hidden owner keeps the payload alive and is released at scope exit. `Module.share()` is ordinary module-member dispatch and never ownership promotion; only reserved `share(owner)` performs ownership control, and it rejects the plain guard payload. Complete cross-region/global/async plugin type escape semantics remain open. No-type-annotation block `import(...)` destructuring sugar, typed/untyped import payload escape scanner reuse, and inner-region assignment propagation for outer plugin aliases are implemented. No-block `import(...)` guard lifetime has not been extended.
