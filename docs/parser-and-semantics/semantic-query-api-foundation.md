...
All position and node-scope range checks use exact optional source identity. Two
missing sources compare equal, two non-null sources compare by string value,
...
For a terminal member of an imported module-link chain, the semantic query first consumes the same complete external identity from `SymbolAt`. The metadata identity adapter validates the owner as the canonical module prototype, selects exactly one matching member row by owner, target kind, metadata token, signature token, and signature hash, and only then hydrates the structured member payload. A request-time AST is not required for this terminal path. Missing, ambiguous, stale, or tampered identity is invalid and blocks the legacy chain consumer. Intermediate module hops and completion remain separate until their producers publish an equivalent canonical fact; consumers must not infer them from member or module spelling.

Module-link intermediate hops now use the same identity contract when the parser
publishes `EXTERNAL_TARGET_MODULE`. The provider must resolve the canonical
module owner and a unique module-typed row before the semantic query returns a
module target, so `system.console` remains available when the request-time AST
is detached. Missing or changed tokens, hashes, provider generations, or
ambiguous rows fail closed. Completion remains outside this slice because its
cursor position does not yet carry an equivalent canonical member range; it
must not reconstruct a hop from neighboring text.
...
`ZrLanguageServer_SemanticAnalyzer_AppendSemanticQueryDiagnostics` runs after normal LSP semantic type checking. It runs the CFG-backed definite-assignment resolver when the analyzer AST is available and otherwise falls back to the straight-line resolver, explicitly materializes the module-scope diagnostic view, then reads it through `ZrParser_SemanticQuery_Diagnostics`. It converts each item with `ZrLanguageServer_Diagnostic_FromStructured`. A pre-existing analyzer diagnostic with the same source identity, exact range, and stable code is a stale duplicate: the complete canonical projection replaces the earliest matching item, including severity, message, cause, suggestion, descriptor/help URI, related information, typed fixes, and no-fix reason. Any later stale items with that same identity are freed and removed from the array from back to front, so one canonical query row yields one protocol diagnostic without changing the retained slot's order. Source identity matches when both source pointers are identical, both are explicitly absent, or two non-null source strings are equal. One missing source never matches a non-null source, so equal offsets and code cannot replace a diagnostic across snapshots/documents. A diagnostic with a different source, range, or code remains independent. The regular `ZrLanguageServer_Lsp_GetDiagnostics` path then publishes those diagnostics through the existing LSP conversion flow.
...
- `tests/language_server/test_lsp_semantic_query_diagnostics.c` covers LSP publishing of semantic query diagnostics by opening a constant ternary branch fixture and requiring `GetDiagnostics` to include `unreachable_code`; it also covers `numeric_overflow`, array bounds errors/warnings, known non-integer array index errors, min-only array negative-interval warnings, and a branch-join `possibly_uninitialized_read` with one declaration relatedInformation entry through the same LSP diagnostic path. Its stale-duplicate case preloads a warning with the canonical source/range/code and requires query projection to replace the complete object with the parser-owned error severity, text fields, descriptor/help URI, related range/message, and explicit no-fix reason. A companion case gives the stale item the same offsets/code but no source identity and requires both source-scoped diagnostics to survive. A third case preloads two stale rows with the same complete identity and requires a single canonical row in the first slot after projection.
