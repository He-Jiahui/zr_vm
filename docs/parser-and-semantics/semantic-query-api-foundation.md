...
After compiler semantic references have resolved, `ZrParser_Semantic_BuildSourceScopeFacts` publishes source module, function, block, and loop scope facts. It projects function declarations as hoisted module candidates and exact resolved parameter/local declaration facts into their lexical scopes. Loop scopes end at their body rather than using an over-wide parser statement range, so a `for` initializer cannot leak into the following statement. The builder uses AST nodes only to identify scope boundaries; candidate identity, ranges, and display contract come from the exact resolved declaration fact, with no range, name, or member fallback. A missing resolved declaration fact is omitted, so the query fails closed rather than inventing source visibility. Source generic/type scopes, imports/aliases, receiver members, and binary/native scope producers remain separate pending work.
