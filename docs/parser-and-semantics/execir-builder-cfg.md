...
One operand-free `break` in a no-catch `try/finally` directly nested in a
supported `while` is also represented. The compiler records the existing loop
join as the pending target. Terminal cleanup branches through a completion
block to that target; a conditional break uses the same ordered cleanup
dispatch with a selector-only private Place, because this completion carries no
...
or ownership cleanup reject the entire shape and keep the legacy path. A
`continue`, multiple break sites, and combinations of break with another
completion kind remain unsupported.
