...
- `Borrow<T>` / `Loan<T>` 的直连本地 owner scope-end 已接入；`Borrow<T>` scope 退出结束借用，`Loan<T>` scope 退出通过 `OWN_RETURN_LOAN` 把 loaned value 归还 source owner
...
- language server semantic metadata
  ...
  - `using (Loan<T>(owner))` 会在正常 scope 退出和 break 前通过 `OWN_RETURN_LOAN` 归还 source owner
