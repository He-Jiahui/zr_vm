...
This module provides the shared capture and revalidation boundary for workspace edits whose ranges were computed from multiple documents. Ordinary `textDocument/rename` and source-file `workspace/willRenameFiles` both capture every edited URI before JSON serialization, validate the complete set immediately before publishing, and discard the whole response if any document no longer matches.
...
Code action, safe-fix, and other workspace-edit producers have not yet adopted this helper. Cancellation, concurrent stale-response stress, latency percentiles, and peak-memory budgets also remain open L6 gates.
