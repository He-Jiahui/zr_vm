...
- stdio diagnostics 的可选 `previousResultId` / `identifier` 字段若出现必须是字符串，workspace `previousResultIds` 必须是含字符串 `uri` / `value` 的数组；畸形可选字段返回 `-32602 InvalidParams`，省略字段和合法空数组保持现有 report 语义。
...
2026-09-05 Plan 00 Task 4 Sub04 撤销未具备 compiler typed facts 的颜色能力。
native initialize 不再发布 `colorProvider`，registry 和 dispatch 不再登记
`textDocument/documentColor` / `textDocument/colorPresentation`，显式请求返回
完整 JSON-RPC `-32601` MethodNotFound。旧 `stdio_document_color.c`、私有函数和
颜色专用常量均已删除。WASM worker 同样不发布或注册颜色能力。

`"#336699"` 保持普通 string literal 的 canonical 类型、常量 hover 和准确范围；
变量仍使用准确的 symbol definition，注释不产生符号引用。该协议层不创建类型身份、
不拥有语义快照或返回值，也不生成编辑计划。未来恢复颜色能力需先由 compiler 发布
明确的颜色类型/表达式事实，再提供绑定快照和版本的 presentation edits；普通字符串
拼写不能充当类型依据。专门的 stdio fixture 覆盖空客户端与显式 color client 的
完整错误 envelope、真实字符串/标识符语义输出和正常 teardown。
...
- `textDocument/inlineCompletion` 仍是 stdio 层 keyword-prefix 轻量能力，不执行 parser completion 或 symbol inference；它只在 prefix 字节位于 code span 时返回 keyword item，comment/string/template 中返回空数组；缺失或畸形 params 返回 `-32602 InvalidParams`，不声明 parser token-stream replacement。
- `textDocument/documentColor` / `colorPresentation` 在 compiler color facts 缺失时不提供；原 hex string scanner 已删除。
...
2026-06-05 documentColor comment color 过滤历史验证（已由 2026-09-05 能力撤销取代）：
...
2026-06-05 colorPresentation comment range 过滤历史验证（已由 2026-09-05 能力撤销取代）：
...
RED：新增 `if ()` / `while ()` 的 LSP diagnostic 断言后，旧 parser/LSP 路径没有 `missing_condition` code、具体问题文本和建议。GREEN：parser 在 `if` / `while` 的 `(` 之后立即识别空条件，并通过 `ZrParser_DiagnosticBuilder_BuildMissingCondition` 产生结构化诊断；LSP interface 和 stdio JSON 都保留 `missing_condition`、`Missing condition inside ...` 与 `Add a boolean expression between '(' and ')'` 建议。后续 focused parser-diagnostics suite 又覆盖 `if (ready { ... }` / `while (ready { ... }` 的 `missing_condition_close`，并加入 `switch` 条件家族：`switch (choice { ... }` 在条件表达式后报告 `Missing ')' after 'switch' condition`，`switch ()` 在 `(` 后立即报告 `Missing condition inside 'switch'`，都不再走 generic expected-token fallback。2026-07-21 safe-fix follow-up让`missing_condition_close`由parser直接发布block opener前的零宽`)` machine fix，code action和stdio JSON只投影structured fix，修正后的document version不再发布该code；后续`missing_index_close`也保留opening `[` primary range，并在当前token起点发布零宽`]` machine fix，`value[0;`因此只在`;`前插入`]`，修复后的version清除该code。WSL gcc、WSL clang 和 Windows MSVC十八目标矩阵与stdio smoke均通过；这些follow-up仍不代表其他delimiter或L3整体完成。
...
同日 array-close safe-fix follow-up让`return [1, 2`保留opening `[`的line 0 character 7..8 primary range，并在EOF character 12发布零宽`]` machine fix。Support-first RED显示旧producer用lexer current cursor构造出character 9..9 primary；`parse_array_literal`改用exact current-token range后，code action与stdio JSON只消费structured fact/fix，version 2应用`]`并补齐statement semicolon后清除`missing_array_close`。GCC、Clang、MSVC十八目标矩阵和两套stdio smoke均通过；object及其他delimiter、replacement与L3整体仍未完成。
...
同日 object property-separator safe-fix follow-up让identifier key `return {a: 1 b: 2};`与string key `return {a: 1 "b": 2};`保留next-key token primary，并在其起点发布零宽`,` machine fix。最初computed-key fixture的`[`被表达式语法吸收为前值postfix index，测试据此改为可达string-key边界，不修改语法。Code action与stdio JSON只投影structured fix，version 2应用逗号后清除`missing_object_property_separator`；GCC、Clang、MSVC十八目标矩阵和两套stdio smoke均通过，其他delimiter/replacement与L3整体仍未完成。
