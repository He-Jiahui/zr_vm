...
`ZrParser_DiagnosticBuilder_BuildMissingArrayElementSeparator` keeps the next element token as its primary range and publishes a zero-width machine-applicable comma at that range's start. `return [1 2];` therefore keeps characters 10..11 and inserts at character 10. The parser still accepts comma or semicolon separators; the quickfix chooses the canonical comma. `array_element_assignment` remains diagnostic-only because punctuation cannot make an assignment a valid value element.

`ZrParser_DiagnosticBuilder_BuildMissingObjectClose` applies the corresponding contract to object-shaped producers. Object literals and `using (var { ...` destructuring patterns capture the exact opening `{`; switch struct patterns already used that token identity. The shared reporter publishes a zero-width machine-applicable `}` at the current token start. `return {a: 1` therefore keeps characters 7..8 and inserts at EOF character 12, while `using (var {value,` keeps characters 11..12 and inserts at character 18.

`ZrParser_DiagnosticBuilder_BuildMissingObjectComputedKeyClose` preserves the exact opening `[` for both the first and subsequent computed object-property branches. Its separate current-token range becomes a zero-width machine-applicable `]` edit. `return {[1: 2};` therefore keeps characters 8..9 and inserts before the colon at character 10; `return {a: 0, [1: 2};` keeps characters 14..15 and inserts at character 16.

`ZrParser_DiagnosticBuilder_BuildMissingObjectPropertyColon` keeps the unexpected value token as the primary range and publishes a zero-width machine-applicable `:` at that range's start. Both property branches already pass the current value token to the shared reporter. `return {a 1};` therefore keeps characters 10..11 and inserts at character 10; `return {a: 0, b 1};` keeps characters 16..17 and inserts at character 16.

`ZrParser_DiagnosticBuilder_BuildMissingObjectPropertySeparator` keeps the next reachable property-key token as the primary range and publishes a zero-width machine-applicable comma at its start. `return {a: 1 b: 2};` keeps identifier characters 13..14/fix 13; `return {a: 1 "b": 2};` keeps string-token characters 13..16/fix 13. A following `[` is parsed as postfix indexing on the previous value before the object separator check, so this contract does not reinterpret that grammar path as a computed property.

`ZrParser_DiagnosticBuilder_BuildMissingConditionalColon` keeps the conditional `?` token as the primary range and publishes a zero-width machine-applicable `:` at the current token start only when that token can start the alternate expression. `return true ? 1 2;` keeps characters 12..13 and inserts at character 16. `return true ? 1;` keeps its diagnostic but publishes no fix because `;` is not an alternate expression; missing consequent and missing alternate diagnostics also remain non-machine-applicable.
...
- Diagnostic-fix smoke checks JSON `Diagnostic.data.fixes` for placeholder, semicolon, condition-close, index-close, parameter-list-close, call-close, group-close, array-close, array-element-separator, object-close, object-computed-key-close, object-property-colon, object-property-separator and conditional-colon machine fixes, including each version 2 diagnostic-clear boundary plus conditional no-alternate and array-assignment no-fix boundaries.

Detailed three-toolchain acceptance evidence is recorded in `docs/plans/lsp/02-diagnostics/2026-07-21-semicolon-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-condition-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-index-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-parameter-list-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-call-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-group-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-array-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-object-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-object-computed-key-close-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-object-property-colon-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-object-property-separator-safe-fix-convergence.md`, `docs/plans/lsp/02-diagnostics/2026-07-21-conditional-colon-safe-fix-convergence.md` and `docs/plans/lsp/02-diagnostics/2026-07-21-array-element-separator-safe-fix-convergence.md`.

## Open Boundaries

This contract covers local semicolon insertion plus missing control-condition `)`, index `]`, declaration parameter-list `)`, call `)`, grouped-expression `)`, array-literal `]`, array-element comma, object-shaped `}`, computed object-key `]`, object property `:`, reachable object-property comma and conditional-colon insertions. Other delimiter families, delimiter replacement, migration edits, multi-document fixes, registry-wide applicability audits, cancellation/race stress and performance/peak-memory budgets remain open.
