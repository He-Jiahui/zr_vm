...
// switch 表达式（具有懒推断性质，实际上是闭包函数的语法糖）
...
// 此时j变成一个需要里面使用this作为lambda表达式传递默认参数，调用时需要`j([1,2,3,4,5])`
var j = for (var i in this) {
    if (i % 2 == 0) {
        continue i;
    }
    if (i >= 4) {
...
**迭代器说明**:
- 迭代器允许在 `continue` 时暂时退出闭包函数并返回结果，直到 `break`
- `continue` 可以返回值，`break` 也可以返回值
- 默认 `continue null`
...
- **语句分隔**: 语句可以以分号结尾（必须）
- **块表达式**: 块表达式不可返回值，但是生成器、迭代器等可以延迟返回
- **控制流表达式**: 控制流结构可以作为表达式使用，此时控制流结构变成延迟运算结构，类似 lambda 表达式的语法糖
