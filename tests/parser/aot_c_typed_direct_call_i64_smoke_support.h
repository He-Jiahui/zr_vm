#ifndef ZR_VM_TESTS_AOT_C_TYPED_DIRECT_CALL_I64_SMOKE_SUPPORT_H
#define ZR_VM_TESTS_AOT_C_TYPED_DIRECT_CALL_I64_SMOKE_SUPPORT_H
...
static void aot_c_i64_smoke_run_case(const SZrAotTypedDirectCallI64SmokeCase *testCase) {
    ...
    (void)testCase;
    TEST_IGNORE_MESSAGE("AOT C typed direct-call i64 smoke currently validates the Unix shared-library path");
#else
    SZrState *state;
    ...
    aot_c_i64_smoke_format_child_path(sourceDirectory,
                                      sizeof(sourceDirectory),
                                      testCase->projectDirectory,
                                      "src");
    aot_c_i64_smoke_format_child_path(binaryDirectory,
                                      sizeof(binaryDirectory),
                                      testCase->projectDirectory,
                                      "bin");
    aot_c_i64_smoke_format_child_path(generatedSourceDirectory,
                                      sizeof(generatedSourceDirectory),
                                      testCase->projectDirectory,
                                      "bin/aot_c/src");
    aot_c_i64_smoke_format_child_path(sharedLibraryDirectory,
                                      sizeof(sharedLibraryDirectory),
                                      testCase->projectDirectory,
                                      "bin/aot_c/lib");

    TEST_ASSERT_TRUE(ZrTests_Path_GetGeneratedArtifact("aot_c_typed_direct_call_i64",
                                                       testCase->projectDirectory,
