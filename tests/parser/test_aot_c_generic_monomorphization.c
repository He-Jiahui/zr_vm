...
static void test_aot_c_value_generic_instance_emits_monomorphized_layout_and_function(void) {
    const char *source =
            ...
            "    }\n"
            "}\n"
            "pub fn makePair(seed: int): Pair<int, int> {\n"
            "    var local: Pair<int, int> = init Pair<int, int>(seed, seed + 1);\n"
            "    return local;\n"
            "}\n"
            "var returned: Pair<int, int> = makePair(40);\n"
            "return returned.left + returned.right;";
    const char *projectJson =
            "{"
            "\"name\":\"aot-c-generic-monomorphization\","
            "\"source\":\"src\","
            "\"binary\":\"bin\","
            "\"entry\":\"main\""
            "}";
    ...
    TEST_ASSERT_NOT_NULL(state);
    function = compile_source(state, source, "aot_c_generic_monomorphization.zr");
    TEST_ASSERT_NOT_NULL(function);

    TEST_ASSERT_TRUE(ZrTests_Path_GetGeneratedArtifact("aot_c_generic_monomorphization",
                                                       "project",
    ...
    TEST_ASSERT_NOT_NULL(strstr(generatedCText, "/* zr_aot_generic_monomorphization_table */"));
    TEST_ASSERT_NOT_NULL(strstr(generatedCText, "Pair<int, int>"));
    TEST_ASSERT_NOT_NULL(strstr(generatedCText, "share=monomorphized"));
    TEST_ASSERT_NOT_NULL(strstr(generatedCText, "typedef ZR_AOT_C_LAYOUT_STRUCT(ZrLayout_"));
    TEST_ASSERT_NOT_NULL(strstr(generatedCText, "static TZrInt64 zr_fn_pair__"));
    TEST_ASSERT_NULL(strstr(generatedCText, "const SZrAotGenericDictionary *"));
    free(generatedCText);
    ...
    TEST_ASSERT_EQUAL_INT64(81, result.value.nativeObject.nativeInt64);
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_aot_c_value_generic_instance_emits_monomorphized_layout_and_function);
    return UNITY_END();
}
