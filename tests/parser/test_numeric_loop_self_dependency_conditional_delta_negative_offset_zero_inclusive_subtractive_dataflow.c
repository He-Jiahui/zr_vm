...
static void run_zero_inclusive_subtractive_case(
    ...
    written = snprintf(
            ...
            sizeof(source),
            "while (flag) {\n"
            "    other = (+narrowed) - (zero - (zero - maybeUnit));\n"
            "    mirror = other;\n"
            "    narrowed = narrowed %s;\n"
            "}\n"
            "narrowed + 0;\n"
...
static void test_while_self_dependent_target_reading_subtract_zero_minus_zero_inclusive_subtractive_negative_offset_conditional_positive_delta_keeps_upward_range(void) {
    run_zero_inclusive_subtractive_case(
            "numeric_while_self_dependent_target_reading_subtract_zero_minus_zero_inclusive_subtractive_negative_offset_conditional_positive_delta_dataflow_test.zr",
...
static void test_while_self_dependent_target_reading_subtract_zero_minus_zero_inclusive_subtractive_negative_offset_conditional_negative_delta_stays_guarded(void) {
    run_zero_inclusive_subtractive_case(
            "numeric_while_self_dependent_target_reading_subtract_zero_minus_zero_inclusive_subtractive_negative_offset_conditional_negative_delta_guarded_dataflow_test.zr",
            "- (choose ? step : step + bias)",
            5,
            5,
            0,
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_while_self_dependent_target_reading_subtract_zero_minus_zero_inclusive_subtractive_negative_offset_conditional_positive_delta_keeps_upward_range);
    RUN_TEST(test_while_self_dependent_target_reading_subtract_zero_minus_zero_inclusive_subtractive_negative_offset_conditional_negative_delta_stays_guarded);
    return UNITY_END();
}
