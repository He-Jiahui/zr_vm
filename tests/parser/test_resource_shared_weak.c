...
static TZrInt64 observe_drop_time_upgrade(SZrState *state) {
    ...
    if (g_drop_time_weak != ZR_NULL &&
        ZrCore_Ownership_UpgradeValue(state, &upgraded, g_drop_time_weak)) {
...
static void test_shared_clone_and_repeated_upgrade_account_strong_refs(void) {
    ...
    TEST_ASSERT_TRUE(ZrCore_Ownership_WeakValue(g_state, &weak, &shared));
    TEST_ASSERT_EQUAL_UINT32(2U, control->weakRefCount);
    TEST_ASSERT_TRUE(ZrCore_Ownership_UpgradeValue(g_state, &firstUpgrade, &weak));
    TEST_ASSERT_TRUE(ZrCore_Ownership_UpgradeValue(g_state, &secondUpgrade, &weak));
...
static void test_many_weak_handles_survive_final_strong_and_upgrade_to_none(void) {
    ...
    for (index = 0U; index < WEAK_COUNT; index++) {
        ...
        TEST_ASSERT_TRUE(ZrCore_Ownership_WeakValue(g_state, &weakValues[index], &shared));
    }
    ...
    for (index = 0U; index < WEAK_COUNT; index++) {
        ...
        TEST_ASSERT_TRUE(ZrCore_Ownership_UpgradeValue(
...
static void test_resource_shared_surface_runs_clone_upgrade_and_last_strong_drop(void) {
    SZrFunction *function = compile_source(
            ...
            "  var shared: Shared<Tracker> = unique.share();\n"
            "  var clone: Shared<Tracker> = shared;\n"
            "  var watcher: Weak<Tracker> = shared.weak();\n"
            "  var upgraded = watcher.upgrade();\n"
            "  drop(shared);\n"
            "  drop(clone);\n"
            "  drop(upgraded);\n"
            "  var expired = watcher.upgrade();\n"
...
static void test_shared_value_parameter_releases_its_copy(void) {
    SZrFunction *function = compile_source(
            ...
            "}\n"
            "observe(value: Shared<Leaf>) {}\n"
            "run(): int {\n"
            "  var unique: Unique<Leaf> = own Leaf();\n"
            "  var shared: Shared<Leaf> = unique.share();\n"
            "  observe(shared);\n"
            "  drop(shared);\n"
