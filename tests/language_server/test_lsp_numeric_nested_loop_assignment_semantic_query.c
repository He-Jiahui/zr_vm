...
static TZrBool test_local_expression_query_joins_nested_loop_assignment_range(
    ...
    uri = ZrCore_String_Create(
            state,
            "file:///local_nested_loop_assignment_dataflow_numeric_range_fact.zr",
            strlen("file:///local_nested_loop_assignment_dataflow_numeric_range_fact.zr"));
    if (context == ZR_NULL ||
        uri == ZR_NULL ||
        ...
        printf("FAIL: unable to prepare nested loop assignment local query fixture\n");
        return ZR_FALSE;
    }
    ...
    if (!ZrLanguageServer_LspLocalSemanticQuery_ExpressionAt(state, context, uri, position, &query)) {
        ZrLanguageServer_LspContext_Free(state, context);
        printf("FAIL: ExpressionAt returned false for nested loop assignment range\n");
        return ZR_FALSE;
    }
    ...
    passed = query.status == ZR_LSP_LOCAL_SEMANTIC_QUERY_FACT &&
             ...
             query.numericFact->minValue == 6 &&
             query.numericFact->maxValue == 11 &&
             query.numericFact->hasUnsignedRange &&
             query.numericFact->minUnsignedValue == 6 &&
             query.numericFact->maxUnsignedValue == 11 &&
             !query.numericFact->mayOverflow;

    if (!passed) {
        printf("FAIL: expected nested loop assignment range fact; status=%d expr=%p exprKind=%d exprType=%d "
...
int main(void) {
    ...
    printf("ZR VM LSP Numeric Nested Loop Assignment Semantic Query Tests\n");
    printf("===========================================================\n");
    passed = test_local_expression_query_joins_nested_loop_assignment_range(state);
    printf("%s: LSP Local Expression Query Joins Nested Loop Assignment Range\n",
           passed ? "PASS" : "FAIL");
