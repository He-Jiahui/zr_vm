...
const documentUri = 'file:///zr-duplicate-type-query-smoke.zr';
const documentText = [
    'class Pair {',
    '}',
    'class Pair {',
...
assert(publication, 'Expected duplicate type diagnostic publication');

const duplicates = publication.params.diagnostics.filter((diagnostic) =>
    diagnostic.code === 'duplicate_type');
assert(duplicates.length === 1,
    `Expected one canonical duplicate diagnostic: ${JSON.stringify(publication.params.diagnostics)}`);
assert(!publication.params.diagnostics.some((diagnostic) =>
    diagnostic.code === 'compiler_error'),
    'Duplicate type projection must not retain a parallel compiler_error');

const diagnostic = duplicates[0];
assert(diagnostic.severity === 1 &&
    ...
    `Expected exact duplicate declaration range: ${JSON.stringify(diagnostic)}`);
assert(diagnostic.message ===
    "Type name 'Pair' is already declared in this context\n" +
    'Cause: Another type binding with the same name is already visible in this context.\n' +
...
assert(Array.isArray(diagnostic.relatedInformation) &&
    ...
    diagnostic.relatedInformation[0].location.range.start.character === 6 &&
    diagnostic.relatedInformation[0].location.range.end.line === 0 &&
    diagnostic.relatedInformation[0].location.range.end.character === 10 &&
    diagnostic.relatedInformation[0].message === 'Type was first declared here',
    'Expected exact first-declaration related information');
assert(diagnostic.data && diagnostic.data.descriptorId === 2010 &&
    diagnostic.data.noFixReason === 'requires_user_decision',
    `Expected descriptor 2010 and canonical no-fix reason: ${JSON.stringify(diagnostic)}`);
assert(!diagnostic.data.fixes || diagnostic.data.fixes.length === 0,
    'Duplicate type diagnostics must not publish a machine fix');
