...
static void assert_extern_struct_decorator_golden_parity(
    ...
    TEST_INFO("Compiler query to LSP projection",
              "Extern struct and field decorator diagnostics must preserve the parser-owned descriptor, exact decorator range, and no-fix disposition");
    ...
    if (analyzer == ZR_NULL || ast == ZR_NULL ||
        ...
        TEST_FAIL(timer, summary, "Failed to analyze extern struct decorator parity fixture");
        return;
    }
    ...
    if (!ZrParser_SemanticQuery_MaterializeDiagnostics(
        ...
        ZrParser_Ast_Free(state, ast);
        ZrLanguageServer_SemanticAnalyzer_Free(state, analyzer);
        TEST_FAIL(timer, summary, "Failed to query extern struct decorator diagnostics");
        return;
    }
    ...
    for (TZrSize index = 0U; index < queryDiagnostics.count; index++) {
        ...
        if (!diagnostic_projection_matches_query(projected, structured) ||
            ...
            TEST_FAIL(timer,
                      summary,
                      "The projected extern struct decorator diagnostic diverged from the compiler query fact");
            return;
        }
    }
...
static void test_semantic_analyzer_preserves_invalid_extern_struct_decorator_golden_parity(
    ...
    assert_extern_struct_decorator_golden_parity(
            state,
            "Semantic Analyzer Preserves Invalid Extern Struct Decorator Golden Parity",
            "invalid_extern_struct_decorator_golden_parity.zr",
            testCode,
            2,
            5,
            25);
...
static void test_semantic_analyzer_preserves_invalid_extern_field_decorator_golden_parity(
    ...
    assert_extern_struct_decorator_golden_parity(
            state,
            "Semantic Analyzer Preserves Invalid Extern Field Decorator Golden Parity",
            "invalid_extern_field_decorator_golden_parity.zr",
            testCode,
            3,
            9,
            31);
