...
const documentUri = 'file:///zr-extern-struct-decorator-query-smoke.zr';
const documentText = [
...
assert(publication, 'Expected extern struct decorator diagnostic publication');
...
assert(!publication.params.diagnostics.some((diagnostic) =>
    ...
    'Extern struct decorator projection must not retain a parallel compiler_error');
...
assert(diagnostic.severity === 1 &&
    ...
    diagnostic.range.end.character === 30,
    `Expected exact field decorator range: ${JSON.stringify(diagnostic)}`);
assert(diagnostic.message.includes(
    'zr.ffi.offset has invalid arguments for this extern struct field'),
    `Expected compiler-owned decorator message: ${JSON.stringify(diagnostic)}`);
...
assert(!diagnostic.data.fixes || diagnostic.data.fixes.length === 0,
    'Invalid extern struct decorator diagnostics must not publish a machine fix');
