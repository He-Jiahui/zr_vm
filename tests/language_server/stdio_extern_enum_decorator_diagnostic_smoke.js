...
const documentUri = 'file:///zr-extern-enum-decorator-query-smoke.zr';
const documentText = [
...
assert(publication, 'Expected extern enum decorator diagnostic publication');
...
assert(!publication.params.diagnostics.some((diagnostic) =>
    ...
    'Extern enum decorator projection must not retain a parallel compiler_error');
...
assert(diagnostic.severity === 1 &&
    ...
    diagnostic.range.end.character === 29,
    `Expected exact enum member decorator range: ${JSON.stringify(diagnostic)}`);
assert(diagnostic.message.includes(
    'zr.ffi.value has invalid arguments for this extern enum member'),
...
assert(!diagnostic.data.fixes || diagnostic.data.fixes.length === 0,
    'Invalid extern enum decorator diagnostics must not publish a machine fix');
