...
static SZrFunction *compile_source(SZrState *state, const TZrChar *source, TZrBool testBuild) {
    SZrString *sourceName = ZrCore_String_CreateFromNative(state, "test_role_binding.zr");
...
static void test_test_build_emits_manifest_for_ordinary_function(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("parsesEmptyInput", manifest.entries[0].qualifiedName);
...
static void test_async_test_uses_explicit_task_void_contract(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("loadsFixture", manifest.entries[0].qualifiedName);
