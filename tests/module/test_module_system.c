    ...
    if (func != ZR_NULL) {
        ...
        for (TUInt32 i = 0; i < func->localVariableLength; i++) {
            ...
            if (varName != ZR_NULL) {
                ...
                if (ZrStringEqual(varName, xName)) foundX = ZR_TRUE;
                if (ZrStringEqual(varName, yName)) foundY = ZR_TRUE;
                if (ZrStringEqual(varName, zName)) foundZ = ZR_TRUE;
            }
        }
...
void test_visibility_modifiers_parsing(void) {
    ...
    TEST_INFO("Visibility modifiers parsing",
              "Testing parsing of pub/pri/pro visibility modifiers for variables");
...
void test_module_cache_operations(void) {
    ...
    TEST_INFO("Module cache operations",
              "Testing module cache lookup and add operations with path as key");
    ...
    TEST_ASSERT_EQUAL_INT(100, retrievedPub->value.nativeObject.nativeInt);
...
void test_zr_import_function_call(void) {
    ...
    TEST_INFO("zr.import function call",
              "Testing zr.import native function to load and compile modules");
...
void test_module_cache_hit_miss(void) {
    ...
    TEST_INFO("Module cache hit/miss scenarios",
...
void test_module_visibility_access_control(void) {
    ...
    TEST_INFO("Module visibility access control",
              "Testing that pub exports are accessible across modules, pro exports are accessible within same module library");
    ...
    TEST_ASSERT_EQUAL_INT(200, retrievedPro->value.nativeObject.nativeInt);
    ...
    TEST_ASSERT_EQUAL_INT(100, pubInPro->value.nativeObject.nativeInt);
...
void test_path_hash_calculation(void) {
    ...
    TEST_INFO("Path hash calculation",
              "Testing path hash calculation using xxhash");
