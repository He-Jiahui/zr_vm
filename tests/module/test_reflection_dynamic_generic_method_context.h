...
#include "test_reflection_dynamic_generic_method_make.h"
...
typedef struct SMethodSpecExecutionCapture {
    ...
    TZrBool observedDebugGenericMethodContext;
    TZrBool observedDebugGenericTypeUnavailable;
...
static TZrDebugSignal test_capture_method_spec_execution_context(
    ...
    if (ZrCore_Debug_GetEvaluationContext(state, 0u, &capture->evaluationContext) ==
        ...
        if (ZrCore_Debug_EvaluationContext_GetGenericArgument(
            ...
            capture->observedDebugGenericMethodContext = ZR_TRUE;
        }
        capture->observedDebugGenericTypeUnavailable =
...
static void test_method_spec_executes_resolved_vm_function_with_context(void) {
    ...
    TEST_ASSERT_TRUE(capture.observedDebugGenericMethodContext);
    TEST_ASSERT_TRUE(capture.observedDebugGenericTypeUnavailable);
