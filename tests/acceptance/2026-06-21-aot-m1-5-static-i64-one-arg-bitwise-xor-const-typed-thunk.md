# AOT M1.5 07-S5 Static I64 One-Arg Bitwise-Xor-Constant Typed Thunk

Date: 2026-06-22 10:00:14 +08:00
...
- The typed i64 thunk module recognizes typed one-parameter int64 functions that return a bitwise-xor with a signed constant.
...
- The recognizer handles `GET_CONSTANT`, `BITWISE_XOR`, and `FUNCTION_RETURN`, with the constant operand accepted on either side.
...
- The dedicated bitwise shared-library smoke executes `flip(value: int): int { return value ^ 6; }`, verifies runtime result `42`, and rejects `CallStaticDirect` / `CallStackValue` in this path.

Out of scope:

- General expression returns beyond this one-arg bitwise-xor-constant form.
...
- `zr_vm_aot_c_call_contracts_test`: missing `backend_aot_c_try_get_i64_arg0_bitwise_xor_constant_return(`.
...
This slice reuses the one-argument direct-call gate and the shared bitwise-constant recognizer. Wider typed return ABI lowering and dynamic/deopt bridge work remain open.
