# AOT M1.5 07-S5 Static I64 One-Arg Bitwise-Or-Constant Typed Thunk

Date: 2026-06-22 09:42:53 +08:00
...
- The typed i64 thunk module recognizes typed one-parameter int64 functions that return a bitwise-or with a signed constant.
- The shared bitwise-constant recognizer accepts a proven `GET_STACK` / `SET_STACK` parameter-copy prefix from parameter slot 0 before the constant and bitwise operation.
- The recognizer handles `GET_CONSTANT`, `BITWISE_OR`, and `FUNCTION_RETURN`, with the constant operand accepted on either side.
...
- The dedicated bitwise shared-library smoke executes `flags(value: int): int { return value | 10; }`, verifies runtime result `42`, and rejects `CallStaticDirect` / `CallStackValue` in this path.

Out of scope:

- General expression returns beyond this one-arg bitwise-or-constant form.
...
- `zr_vm_aot_c_call_contracts_test`: missing `backend_aot_c_try_get_i64_arg0_bitwise_or_constant_return(`.
...
This slice reuses the one-argument direct-call gate and the shared bitwise-constant recognizer. Wider typed return ABI lowering and dynamic/deopt bridge work remain open. The bitwise smoke is now close enough to the large-file threshold that the next broad bitwise expansion should split shared smoke helpers first.
