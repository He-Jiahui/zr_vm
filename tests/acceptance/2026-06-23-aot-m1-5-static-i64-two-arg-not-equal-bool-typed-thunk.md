# AOT M1.5 07-S5 static i64 two-arg not-equal bool typed thunk

Status: sub-slice complete; 07-S5 partial; M1.5/07 partial; stages 08-12 not started.

## Scope

This slice broadens the mixed `bool(i64, i64)` typed-to-typed direct-call route from signed less-than/equality to signed inequality.

## Baseline

- The mixed bool-return route already handled `LOGICAL_LESS_SIGNED` and `LOGICAL_EQUAL_SIGNED`.
- The caller-side route already proved a bool destination local and two written i64 argument locals.
- A source function `different(left: int, right: int): bool { return left != right; }` still generated a frame-backed callee body and fell back to `CallStaticDirect` plus `SyncBoolLocal`.

## RED

- The bool shared-library smoke failed as expected with `Expected Non-NULL`.
- The generated C for `different(21, 22)` had no `TZrInt64`-parameter bool thunk declaration/definition.
...
- The new inequality wrapper accepts `LOGICAL_NOT_EQUAL_SIGNED` followed by `FUNCTION_RETURN`.
- The mixed `bool(i64, i64)` can-emit gate now accepts signed less-than, equality, and inequality.
...
- Focused GREEN result:
  ...
  - bool typed direct-call smoke 10/0
...
- `backend_aot_c_typed_bool_thunks.c`: 546 physical / 475 non-empty lines.
- `test_aot_c_typed_call_contracts.c`: 719 physical / 681 non-empty lines.
- `test_aot_c_typed_direct_call_bool_shared_library_smoke.c`: 282 physical / 261 non-empty lines.

## Acceptance Decision

Accepted for the narrow mixed i64-parameter bool-return inequality typed direct-call slice. This does not close 07-S5: remaining signed comparisons, inline structs, `in`/`out` writeback, deopt/dynamic bridges, broader ABI coverage, dynamic value access helpers, runtime-failure-capable division/modulo policy, and stages 08-12 remain open.
