# AOT M1.5 07-S2 I64 Bitwise Result Local-Only

Date: 2026-06-21 05:52:10 +08:00
Status: partial 07-S2 slice complete

## Scope

This slice removes frame destination/result materialization from the focused signed i64 bitwise result when every reachable later consumer can use the maintained signed scalar local.

For the focused typed scalar source, the first signed i64 bitwise result:

- `dstSlot=16`
- `leftSlot=12`
- `rightSlot=0`
- generated expression `zr_aot_s16 = zr_aot_s12 & zr_aot_s0;`

now emits only the scalar-local assignment. The generated block no longer declares `SZrTypeValue *zr_aot_destination`, creates `zr_aot_s_result`, or writes the result payload through `frame.slotBase[16].value`.

The bitwise emitter reuses the signed i64 result-skip proof introduced for i64 binary results. It only skips frame materialization when both signed sources are proven written C locals and reachable later consumers can read the signed scalar local.

Affected layers:

- AOT C scalar bitwise emission
- focused generated-product parser/AOT fixture
- AOT semantic contract documentation and plan records

## Baseline

Before this slice, the focused generated C emitted a frame-backed destination block for the first signed i64 bitwise result. The RED test failed because the generated product still contained:
...
- Focused subsystem case: `tests/parser/test_aot_c_typed_scalar.c` verifies generated C for the signed i64 bitwise result.
...
- Negative case: forbidden generated C tokens for the old i64 bitwise destination/result materialization.
...
- RED before implementation: failed 1/1 because the generated C still contained the old i64 bitwise destination block.
...
- GREEN: target rebuilt and the test binary reported 19 tests, 0 failures.
...
- The RED generated-product assertion first proved the old i64 bitwise destination/result materialization was still present.
- `backend_aot_write_c_scalar_i64_bitwise()` now skips frame result materialization only after source locals are proven written and `backend_aot_c_scalar_locals_i64_result_can_skip_value_slot()` proves reachable consumers can use the signed scalar local.
- The focused generated C now contains only `zr_aot_s16 = zr_aot_s12 & zr_aot_s0;` for the first i64 bitwise block.
- The focused typed scalar executable, source-contract target, and registered CTest pass.

## Acceptance Decision

Accepted for the focused 07-S2 i64 bitwise result local-only slice.
