# AOT M1.5 07-S5 THROW Boundary Helper

## Scope

- `backend_aot_write_c_throw()` now emits only the existing `zr_aot_throw_direct` marker, initializes `zr_aot_next_instruction`, calls `ZrLibrary_AotRuntime_Throw(state, &frame, sourceSlot, &zr_aot_next_instruction)`, and dispatches when the helper returns a resume instruction.
- The runtime helper now owns payload slot validation, pending-control clear, exception normalization, current-frame unwind/refresh, resume-index calculation, and unhandled-exception propagation through `ZrCore_Exception_Throw`.
...
- RED: `zr_vm_aot_c_control_contracts_test` failed 1/1 after requiring `ZrLibrary_AotRuntime_Throw(state, &frame, %u, &zr_aot_next_instruction)` and forbidding the old generated THROW inline normalize/unwind template.
...
- Runtime source contract inside the focused control contract: `ZrLibrary_AotRuntime_Throw()` must keep unhandled-exception propagation through `ZrCore_Exception_Throw`.
...
- Generated-product boundary check: control smoke C must contain the THROW helper and resume dispatch, and must not contain the old generated exception normalization template.
...
- RED observed: `Missing source contract text: ZrLibrary_AotRuntime_Throw(state, &frame, %u, &zr_aot_next_instruction)`.
...
- Generated check passed: `aot_c_control_smoke.c` contains `ZrLibrary_AotRuntime_Throw(state, ...)` and the resume-dispatch guard, and does not contain generated `ZrCore_Exception_NormalizeThrownValue(state, ...)`.
...
- Accepted for the THROW boundary-helper slice.
- 07-S5 remains partial. END_FINALLY, pending control, typed-to-typed native signature routing, in/out writeback, deopt/dynamic bridges, and remaining boundary templates are still later 07 work.
