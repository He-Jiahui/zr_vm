# AOT M1.5 07-S5 Static I64 Two-Arg Multiply Typed Thunk

Date: 2026-06-22 07:28:09 +08:00

## Scope

This slice extends the two-argument i64 typed direct-call route from addition and subtraction to multiplication for functions that return the first i64 parameter multiplied by the second.

Covered:

- The AOT C emitter recognizes typed two-parameter int64 functions that return `arg0 * arg1`.
- The recognizer handles `MUL_SIGNED`, `MUL_SIGNED_PLAIN_DEST`, and `MUL_SIGNED_LOAD_STACK` followed by `FUNCTION_RETURN`.
...
- The dedicated arithmetic shared-library smoke executes `product(left: int, right: int): int { return left * right; }`, verifies runtime result `42`, and rejects `CallStaticDirect` / `CallStackValue` in this path.

Out of scope:

- General expression returns beyond this two-arg multiply form.
...
- `zr_vm_aot_c_call_contracts_test`: missing `backend_aot_c_try_get_i64_arg0_arg1_multiply_return(`.
...
- `zr_vm_aot_c_typed_direct_call_arithmetic_shared_library_smoke_test`: 1/0.
...
This slice reuses the two-argument direct-call gate added by the add-return slice. It only broadens the callee body recognizer and emitted thunk expression; wider typed return ABI lowering and dynamic/deopt bridge work remain open. The runtime smoke lives in a new focused arithmetic test file to avoid further growing the existing typed direct-call smoke file.
