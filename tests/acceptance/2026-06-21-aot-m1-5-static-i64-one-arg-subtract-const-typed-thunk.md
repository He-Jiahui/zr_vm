# AOT M1.5 07-S5 Static I64 One-Arg Subtract-Const Typed Thunk

Date: 2026-06-22 08:05:24 +08:00

## Scope

This slice extends the one-argument i64 typed direct-call route to subtraction by a signed integer constant.

Covered:

- The typed i64 thunk module recognizes typed one-parameter int64 functions that return `arg0 - K`.
- The recognizer handles `SUB_SIGNED_CONST`, `SUB_SIGNED_CONST_PLAIN_DEST`, `SUB_SIGNED_LOAD_STACK_CONST`, and materialized-constant subtract forms followed by `FUNCTION_RETURN`.
...
- The dedicated arithmetic shared-library smoke executes `decBy(value: int): int { return value - 8; }`, verifies runtime result `42`, and rejects `CallStaticDirect` / `CallStackValue` in this path.

Out of scope:

- General expression returns beyond this one-arg subtract-constant form.
...
- `zr_vm_aot_c_call_contracts_test`: missing `backend_aot_c_try_get_i64_arg0_subtract_constant_return(`.
...
- `zr_vm_aot_c_typed_direct_call_arithmetic_shared_library_smoke_test`: 3/0.
