# AOT M1.5 07-S5 static u64 two-arg equal bool typed thunk

Status: sub-slice complete; 07-S5 partial; M1.5/07 partial; stages 08-12 not started.

## Scope

This slice broadens the mixed `bool(u64, u64)` typed-to-typed direct-call route from unsigned less-than to unsigned equality.

## Baseline

- The previous u64-bool slice introduced the `TZrUInt64`-parameter bool thunk signature and caller-side `zr_aot_u*` argument route.
- The route already proved a bool destination local and two written u64 argument locals.
- A source function `same(left: uint, right: uint): bool { return left == right; }` still needed a `LOGICAL_EQUAL_UNSIGNED` recognizer and a `TZrUInt64`-parameter equality thunk writer.

## RED

- The typed call contract failed as expected because `backend_aot_c_try_get_bool_u64_arg0_arg1_equal_return(` was absent.
- The new bool shared-library smoke was added to prove generated C for `same(21u, 21u)` needs the u64 bool thunk and must stay off the VM call/value fallback path.

## Implementation

- `backend_aot_c_typed_bool_thunks.c` reuses the shared u64 bool-return compare helper.
- The equality wrapper accepts `LOGICAL_EQUAL_UNSIGNED` followed by `FUNCTION_RETURN`.
- `backend_aot_c_can_emit_typed_bool_u64_two_arg_thunk()` now accepts both `<` and `==` u64-parameter bool-return shapes.
...
- Focused RED result:
  - typed call contracts 3/1, missing `backend_aot_c_try_get_bool_u64_arg0_arg1_equal_return(`
- Focused GREEN result:
  - typed call contracts 4/0
  - bool typed direct-call smoke 15/0
...
- `backend_aot_c_typed_bool_thunks.c`: 686 physical / 598 non-empty lines.
...
- `test_aot_c_typed_call_contracts.c`: 747 physical / 709 non-empty lines.
- `test_aot_c_typed_direct_call_bool_shared_library_smoke.c`: 422 physical / 391 non-empty lines.

## Acceptance Decision

Accepted for the narrow mixed u64-parameter bool-return equality typed direct-call slice. This does not close 07-S5: remaining unsigned comparisons, inline structs, `in`/`out` writeback, deopt/dynamic bridges, broader ABI coverage, dynamic value access helpers, runtime-failure-capable division/modulo policy, and stages 08-12 remain open.
