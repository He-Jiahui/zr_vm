# AOT M1.5 / 07-S5 Static u64 Three-Arg Multiply Typed Thunk

时间：2026-06-23 09:24:25 +08:00
...
- 新增 `backend_aot_c_try_read_u64_multiply_operands()`，复用三参 u64
  binary-return helper 校验 `(arg0 * arg1) * arg2` 与最终 `FUNCTION_RETURN`。
- `backend_aot_c_can_emit_typed_u64_three_arg_thunk()` 现在覆盖 add 和 multiply。
- u64 thunk writer 对 multiply callee 发出：
  `return (TZrUInt64)(zr_aot_arg0 * zr_aot_arg1 * zr_aot_arg2);`
- 直接调用侧复用上一轮的 `zr_aot_static_u64_three_arg_direct_call` proof/writer
  与 scalar-only destination stack-sync elision。
- `tests/parser/test_aot_c_typed_direct_call_u64_shared_library_smoke.c` 新增
  `product3(2, 3, 7)` shared-library smoke，验证运行结果 42，且拒绝
...
- `zr_vm_aot_c_typed_call_contracts_test`：4 tests, 1 failure，缺少
  `return (TZrUInt64)(zr_aot_arg0 * zr_aot_arg1 * zr_aot_arg2);`。
- `zr_vm_aot_c_typed_direct_call_u64_shared_library_smoke_test`：16 tests,
  1 failure，three-arg multiply case 在 generated C 中找不到期望的三参 multiply
...
- 共享库烟测组：shared 8/0、call smoke 5/0、typed direct-call 5/0、i64 three-arg
  smoke 6/0、bool 19/0、u64 16/0、f64 13/0、typed arithmetic 5/0、typed bitwise
  6/0、value-type 1/0、float smoke 1/0、generic numeric smoke 1/0、global smoke
  9/0、logical smoke 4/0、power smoke 1/0。

## 文件规模

- `backend_aot_c_typed_u64_thunks.c`：798 physical / 715 non-empty lines。
- `backend_aot_c_typed_u64_thunk_shapes.c`：514 physical / 464 non-empty lines。
- `backend_aot_c_typed_u64_thunk_shapes.h`：15 physical / 12 non-empty lines。
- `tests/parser/test_aot_c_typed_call_contracts.c`：847 physical / 809 non-empty lines。
- `tests/parser/test_aot_c_typed_direct_call_u64_shared_library_smoke.c`：
  399 physical / 366 non-empty lines。

## 仍未完成

三参 u64 add/multiply 窄形态已覆盖。u64 三参 subtract/bitwise、
