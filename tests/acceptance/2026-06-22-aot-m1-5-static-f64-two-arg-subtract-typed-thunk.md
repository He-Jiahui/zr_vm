# AOT M1.5 / 07-S5 Static F64 Two-Arg Subtract Typed Thunk

- Timestamp: 2026-06-22 14:30:18 +08:00
...
- Adds a two-parameter floating-point subtract-return typed thunk direct-call route.
...
- Covered source shape: `func diff(left: float, right: float): float { return left - right; }`.
- Covered SemIR shape: direct `SUB_FLOAT` of parameter slots 0 and 1 followed by `FUNCTION_RETURN` of the subtract result.
- Generated thunk ABI: `static TZrFloat64 zr_aot_typed_f64_fn_N(struct SZrState *state, TZrFloat64 zr_aot_arg0, TZrFloat64 zr_aot_arg1)`.
- Generated thunk body returns `(TZrFloat64)(zr_aot_arg0 - zr_aot_arg1)`.
- Out of scope: f64 multiply/divide/constant expression returns, additional f64 arities, inline structs, `in` / `out` writeback, deopt/dynamic bridge boxing, and full 07-S5 completion.

## Baseline

- RED baseline: `zr_vm_aot_c_call_contracts_test` failed because the f64 typed thunk source had no `backend_aot_c_try_get_f64_arg0_arg1_subtract_return(` contract.
- The previous f64 slice only covered two-argument add returns and did not prove ordered two-argument f64 subtraction.
...
- Boundary cases:
  - Direct C call route for two `float` arguments.
  - Runtime result 42 from `diff(first, second)`, then final `<int> value + 1`.
...
- Route guard cases:
  ...
  - Both call-window argument slots must have f64 scalar-local coverage.
  - Both argument slots must be proven written before the static call.
  - Callee metadata must have float/double callable return and two float/double parameters.
...
- Failure: missing source contract text `backend_aot_c_try_get_f64_arg0_arg1_subtract_return(`.
...
- The first GREEN command attempt timed out before producing final pass/fail output.
- The same command was repeated and completed successfully.
...
- `zr_vm_aot_c_typed_direct_call_f64_shared_library_smoke_test`: 4 tests, 0 failures.
...
- `zr_vm_aot/zr_vm_parser/src/zr_vm_parser/backend_aot/backend_aot_c_typed_f64_thunks.c`: 248 physical / 215 non-empty lines.
...
- `tests/parser/test_aot_c_typed_direct_call_f64_shared_library_smoke.c`: 608 physical / 563 non-empty lines.
- `tests/parser/test_aot_c_call_contracts.c`: 875 physical / 818 non-empty lines.

## Acceptance Decision

- Accepted for this narrow 07-S5 sub-slice only.
- The accepted behavior is two-argument f64 direct typed calls where the callee returns the ordered subtraction of its two arguments.
