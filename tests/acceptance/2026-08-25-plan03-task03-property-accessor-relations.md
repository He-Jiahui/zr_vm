...
- An accessor TypeId mismatch, absent symbol, or non-function target fails
  closed without a property-name or hidden-accessor fallback, and leaves no
  partial edges for that contract set.
...
`5 Tests 0 Failures 0 Ignored`. The existing compiler-backed
...
- 完成项目：property accessor canonical relation producer、严格 target identity、
  端点范围、原子 fail-closed、幂等性与 compiler-backed consumer 验证。
