# 2026-06-23 AOT M1.5 07-S5 static f64 three-arg multiply typed thunk

## 状态

- 时间：2026-06-23 11:17:58 +08:00
- 状态：子切片完成；07-S5 / M1.5 / 07 仍部分完成；08-12 未开始

## 完成项目

- `backend_aot_c_typed_f64_thunk_shapes.{h,c}` 新增三参数 float 乘法识别：
  `backend_aot_c_try_get_f64_arg0_arg1_arg2_multiply_return()` 只接受三个 f64
  参数、两条 `MUL_FLOAT` 后接 `FUNCTION_RETURN` 的窄形态。
- `backend_aot_c_can_emit_typed_f64_three_arg_thunk()` 纳入三参 multiply shape。
...
- RED：typed call contracts 4 tests / 1 failure，缺
  `return (TZrFloat64)(zr_aot_arg0 * zr_aot_arg1 * zr_aot_arg2);`。
- RED：f64 shared-library smoke 15 tests / 1 failure，新增三参 multiply 用例为
  `Expected Non-NULL`。
- Focused GREEN：typed call contracts 4/0；f64 typed direct-call smoke 15/0。
...
- 共享库烟测组通过：shared 8/0、call smoke 5/0、typed direct-call 5/0、i64 three-arg 6/0、
  bool 19/0、u64 22/0、f64 15/0、typed arithmetic 5/0、typed bitwise 6/0、value-type 1/0、
  float smoke 1/0、generic numeric smoke 1/0、global smoke 9/0、logical smoke 4/0、
  power smoke 1/0。

## 文件规模

- `backend_aot_c_typed_f64_thunks.c`：231 physical / 212 non-empty lines
- `backend_aot_c_typed_f64_thunk_shapes.c`：796 / 690
- `backend_aot_c_typed_f64_thunk_shapes.h`：22 / 19
- `test_aot_c_typed_call_contracts.c`：896 / 858
- `test_aot_c_typed_direct_call_f64_shared_library_smoke.c`：374 / 343

## 未完成

This is narrow f64 three-arg multiply coverage only. inline structs、`in`/`out`
