# AOT M1.5 / 07-S5 Static U64 Two-Arg Subtract Typed Thunk

- Timestamp: 2026-06-22 13:31:10 +08:00
...
- Adds a two-parameter unsigned integer subtract-return typed thunk direct-call route.
...
- Covered source shape: `func diff(left: uint, right: uint): uint { return left - right; }`.
- Covered SemIR shapes:
  - direct `SUB_UNSIGNED` / `SUB_UNSIGNED_PLAIN_DEST` followed by `FUNCTION_RETURN`
  - `SUB_SIGNED` / `SUB_SIGNED_PLAIN_DEST` followed by `FUNCTION_RETURN`
  - narrow `TO_INT` conversion forms that preserve left/right operand order before signed subtract
  - narrow parameter-copy / `TO_INT` / signed-subtract forms that preserve left/right operand order before `FUNCTION_RETURN`
...
- Out of scope: arbitrary u64 expression trees, non-add/subtract two-arg u64 routes, f64 typed routes, inline structs, `in` / `out` writeback, deopt/dynamic bridge boxing, and full 07-S5 completion.

## Baseline

- RED baseline: `zr_vm_aot_c_call_contracts_test` failed because the u64 typed thunk source had no `backend_aot_c_try_get_u64_arg0_arg1_subtract_return(` contract.
- The previous u64 two-arg slice only covered ordered-independent addition. It did not prove ordered subtraction through the same two-argument typed ABI.
...
- Boundary cases:
  ...
  - Runtime result 42 from `50 - 9`, then final `+ 1`.
...
- Failure: missing source contract text `backend_aot_c_try_get_u64_arg0_arg1_subtract_return(`.
...
- `zr_vm_aot_c_typed_direct_call_u64_shared_library_smoke_test`: 5 tests, 0 failures.
...
- `zr_vm_aot/zr_vm_parser/src/zr_vm_parser/backend_aot/backend_aot_c_typed_u64_thunks.c`: 546 physical / 489 non-empty lines.
- `tests/parser/test_aot_c_typed_direct_call_u64_shared_library_smoke.c`: 758 physical / 702 non-empty lines.
- `tests/parser/test_aot_c_call_contracts.c`: 762 physical / 710 non-empty lines.
