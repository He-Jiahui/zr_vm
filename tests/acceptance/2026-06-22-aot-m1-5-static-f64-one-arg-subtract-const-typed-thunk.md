# AOT M1.5 / 07-S5 Static F64 One-Arg Subtract-Constant Typed Thunk

- Timestamp: 2026-06-22 15:09:08 +08:00
...
- Adds a one-parameter floating-point subtract-constant typed thunk direct-call route.
...
- Covered source shape: `func dec(value: float): float { return value - 8.0; }`.
- Covered SemIR shape: optional parameter copy from slot 0, then `GET_CONSTANT`, `SUB_FLOAT`, and `FUNCTION_RETURN` of the subtract result.
...
- Out of scope: f64 divide/modulo, multiply-constant, wider f64 expressions, additional f64 arities, inline structs, `in` / `out` writeback, deopt/dynamic bridge boxing, and full 07-S5 completion.

## Baseline

- RED baseline: `zr_vm_aot_c_call_contracts_test` failed because the f64 typed thunk source had no `backend_aot_c_try_get_f64_arg0_subtract_constant_return(` contract.
- The previous f64 behavior slices covered no-arg constants, one-arg identity, one-arg add-constant, and two-arg add/subtract/multiply returns.
...
- Boundary cases:
  - Direct C call route for one `float` argument minus a constant inside the callee.
  - Runtime result 42 from `dec(seed)` with `50.0 - 8.0`.
...
- Route guard cases:
  ...
  - Subtraction keeps operand order: argument left, constant right.
...
- Failure: missing source contract text `backend_aot_c_try_get_f64_arg0_subtract_constant_return(`.
...
- `zr_vm_aot_c_typed_direct_call_f64_shared_library_smoke_test`: 7 tests, 0 failures.
...
- `zr_vm_aot/zr_vm_parser/src/zr_vm_parser/backend_aot/backend_aot_c_typed_f64_thunks.c`: 470 physical / 410 non-empty lines.
...
- `tests/parser/test_aot_c_call_contracts.c`: 882 physical / 825 non-empty lines.
- `tests/parser/test_aot_c_typed_direct_call_f64_shared_library_smoke.c`: 176 physical / 161 non-empty lines.
...
- The accepted behavior is one-argument f64 direct typed calls where the callee returns the argument minus a float constant.
