# AOT M1.5 / 07-S5 Static i64 Three-Arg Bitwise-And Typed Thunk

时间：2026-06-23 08:24:03 +08:00

状态：子切片完成；07-S5、M1.5/07 仍为部分完成；08-12 未开始。

## 完成项目

- `backend_aot_c_typed_i64_thunk_shapes.{h,c}` 新增三参 i64 bitwise-and return
  shape：`backend_aot_c_try_get_i64_arg0_arg1_arg2_bitwise_and_return()`。
- 新增 `backend_aot_c_try_read_i64_bitwise_and_operands()`，读取 `BITWISE_AND`
  的两个 source slots。
- 三参 i64 add/multiply/bitwise-and 共享私有
...
- `backend_aot_c_can_emit_typed_i64_three_arg_thunk()` 现在覆盖 add、multiply 和
  bitwise-and 三种三参 i64 shape。
- i64 thunk writer 对 bitwise-and callee 发出：
...
- `tests/parser/test_aot_c_typed_direct_call_i64_three_arg_shared_library_smoke.c`
  新增 `mask3(58, 47, 62)` shared-library smoke，验证运行结果 42，且拒绝
...
- `zr_vm_aot_c_typed_direct_call_i64_three_arg_shared_library_smoke_test`：3 tests,
  1 failure，bitwise-and case 在 generated C 中找不到期望的三参 bitwise-and
...
- i64 three-arg shared-library smoke：3/0。
...
- 共享库烟测组：shared 8/0、call smoke 5/0、typed direct-call 5/0、i64 three-arg
  smoke 3/0、bool 19/0、u64 14/0、f64 13/0、typed arithmetic 5/0、typed bitwise
  6/0、value-type 1/0、float smoke 1/0、generic numeric smoke 1/0、global smoke
  9/0、logical smoke 4/0、power smoke 1/0。

## 文件规模

- `backend_aot_c_typed_i64_thunks.c`：828 physical / 740 non-empty lines。
- `backend_aot_c_typed_i64_thunk_shapes.c`：139 physical / 120 non-empty lines。
- `backend_aot_c_typed_i64_thunk_shapes.h`：10 physical / 7 non-empty lines。
- `tests/parser/test_aot_c_typed_call_contracts.c`：812 physical / 774 non-empty lines。
- `tests/parser/test_aot_c_typed_direct_call_i64_three_arg_shared_library_smoke.c`：
  104 physical / 96 non-empty lines。
- `tests/parser/aot_c_typed_direct_call_i64_smoke_support.h`：197 physical / 178
  non-empty lines。

## 仍未完成

这是三参 i64 bitwise-and 窄覆盖。07-S5 完整验收、inline structs、`in`/`out`
writeback、deopt/dynamic bridges、general multi-arg typed returns、dynamic value
access helpers、以及 08-12 阶段仍未完成。
