...
| 完成时间 | 状态 | 完成项目 |
|---|---|---|
| 2026-08-08 17:31 +08:00 | 已完成 | active `workspace/diagnostic`在reader观察到`didChange`或`didClose`后只返回`-32801`，v2更新后新的diagnostic request仍可成功返回。 |
...
- 同一generic document在测试末尾发送第二个active `workspace/diagnostic`与`textDocument/didClose`。该request也必须精确返回`-32801`，并保留既有空diagnostics close assertion。
