# 2026-06-23 AOT M1.5 07-S5 f64 thunk shape split

## 状态

- 时间：2026-06-23 11:07:26 +08:00
- 状态：子切片完成；07-S5 / M1.5 / 07 仍部分完成；08-12 未开始

## 完成项目

- 新增 `backend_aot_c_typed_f64_thunk_shapes.{h,c}`，集中承载 f64 typed thunk shape
  识别函数。
- `backend_aot_c_typed_f64_thunks.c` 只保留 can-emit gate、forward declarations 和
  thunk definition writer，接入新的 shape header。
- `tests/parser/test_aot_c_typed_call_contracts.c` 改为分别检查 f64 thunk writer 与
  f64 shape source，和现有 i64/u64 shape split 模式对齐。

## RED / GREEN

- RED：typed call contracts 4 tests / 1 failure，f64 测试读取
  `backend_aot_c_typed_f64_thunk_shapes.c` 时为 `Expected Non-NULL`。
...
- WSL GCC debug 目标构建通过；CMake 因新增 glob source 自动重新配置并编译
  `backend_aot_c_typed_f64_thunk_shapes.c`。
...
- `backend_aot_c_typed_f64_thunks.c`：225 physical / 206 non-empty lines
- `backend_aot_c_typed_f64_thunk_shapes.c`：748 / 646
- `backend_aot_c_typed_f64_thunk_shapes.h`：21 / 18
- `test_aot_c_typed_call_contracts.c`：894 / 856
