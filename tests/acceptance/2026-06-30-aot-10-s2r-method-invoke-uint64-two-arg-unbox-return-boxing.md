# AOT 10-S2R Method.Invoke UInt64 Two-Arg Unbox And Return Boxing

## Scope
- Generated AOT C reflection invoker bucket for `uint64(uint64, uint64)` calls.
- Plan coverage: `10-S2R` argument marshaling, `10-S3V` generated reflection invocation, and `11-S2D` MethodInfo/functionIndex binding consumption.
- Layers covered: AOT C backend reflection invoker emission, typed u64 two-arg thunk predicate exposure, parser source contracts, Unix shared-library smoke, and reflection/metadata adjacent runtime tests.

## Baseline
- Before this slice, generated `Method.Invoke` had no-arg i64/u64/bool/f64 return-boxing buckets, one-arg i64/u64/bool/f64 unbox buckets, and an i64 two-arg bucket, but no u64 two-argument bucket.
- RED command:
...
- RED result: the source contract failed on missing `static TZrBool backend_aot_c_method_metadata_has_u64_two_arg_reflection_case(`.
...
## Implementation
- `backend_aot_c_typed_u64_thunks.h` exposes `backend_aot_c_can_emit_typed_u64_two_arg_thunk(...)` and `backend_aot_c_can_emit_typed_u64_two_arg_state_free_thunk(...)`.
- `backend_aot_c_reflection_invokers.c` emits `zr_aot_try_invoke_u64_two_arg(...)`.
- The generated helper checks non-null method/signature/return type, uint64 return base type, `parameterCount == 2u`, non-null `parameterTypes`, uint64 parameter base type for both declared parameters, non-null args, `args[0].type == ZR_VALUE_TYPE_UINT64`, `args[1].type == ZR_VALUE_TYPE_UINT64`, and non-null `outReturn`.
- Matched state-free functionIndex cases unbox both `nativeUInt64` payloads, call `zr_aot_typed_u64_fn_<index>(zr_aot_arg0, zr_aot_arg1)`, and box through `ZrCore_Value_InitAsUInt(...)`.
- Matched stateful functionIndex cases call `zr_aot_typed_u64_fn_<index>(state, zr_aot_arg0, zr_aot_arg1)` so divide/modulo generated diagnostics keep their existing `state` path.
...
## Test Inventory
- `tests/parser/test_aot_c_frame_setup_contracts.c` asserts the u64 two-arg generated helper, broad and state-free predicates, case writer, second-parameter guard, second-argument type guard, second native uint64 unbox, state-free and stateful typed u64 call shapes, and dispatcher path.
- `tests/parser/test_aot_c_shared_library_smoke.c` adds `sum_unsigned(left: uint, right: uint): uint`, checks generated `case 10u`, invokes the generated reflection invoker with 100 and 23, and verifies boxed `ZR_VALUE_TYPE_UINT64` 123.
...
- Fixes made in response: added typed u64 two-arg predicate exposure and generated reflection invoker two-arg u64 marshaling.

## Acceptance Decision
- Accepted as a support sub-slice for `10-S2R / 10-S3V`.
- This closes only the generated u64 two-arg reflection bucket.
- Remaining work: bool/f64 two-arg buckets, three-arg and wider buckets, object/inline returns, numeric widening, instance receiver handling, public `MethodInfo` materialization, MethodSpec-specific generated function slots, cross-module token rewrite, trim diagnostics, and the full trim analyzer.
