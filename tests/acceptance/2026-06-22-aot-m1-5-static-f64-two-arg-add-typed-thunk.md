# AOT M1.5 / 07-S5 Static F64 Two-Arg Add Typed Thunk

- Timestamp: 2026-06-22 14:19:22 +08:00
...
- Adds a two-parameter floating-point add-return typed thunk direct-call route.
...
- Covered source shape: `func sum(left: float, right: float): float { return left + right; }`.
- Covered SemIR shape: direct `ADD_FLOAT` of parameter slots 0 and 1 followed by `FUNCTION_RETURN` of the add result.
...
- Out of scope: f64 non-add expression returns, additional f64 arities, inline structs, `in` / `out` writeback, deopt/dynamic bridge boxing, and full 07-S5 completion.

## Baseline

- RED baseline: `zr_vm_aot_c_call_contracts_test` failed because the f64 typed thunk source had no `backend_aot_c_can_emit_typed_f64_two_arg_thunk(const SZrFunction *function)` contract.
- The previous f64 slice only covered one-argument identity returns and did not prove a two-argument f64 call-window ABI.
...
- Boundary cases:
  ...
  - Runtime result 42 from `sum(first, second)`, then final `<int> value + 5`.
...
- Failure: missing source contract text `backend_aot_c_can_emit_typed_f64_two_arg_thunk(const SZrFunction *function)`.
...
- `zr_vm_aot_c_typed_direct_call_f64_shared_library_smoke_test`: 3 tests, 0 failures.
...
- `zr_vm_aot/zr_vm_parser/src/zr_vm_parser/backend_aot/backend_aot_c_typed_f64_thunks.c`: 200 physical / 171 non-empty lines.
...
- `tests/parser/test_aot_c_typed_direct_call_f64_shared_library_smoke.c`: 456 physical / 422 non-empty lines.
- `tests/parser/test_aot_c_call_contracts.c`: 872 physical / 815 non-empty lines.

## Acceptance Decision

- Accepted for this narrow 07-S5 sub-slice only.
- The accepted behavior is two-argument f64 direct typed calls where the callee returns the sum of its two arguments.
