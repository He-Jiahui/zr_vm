# AOT M1.5 07-S5 Static I64 Two-Arg Bitwise-Or Typed Thunk

Date: 2026-06-22 08:51:23 +08:00

## Scope

This slice extends the two-argument i64 typed direct-call route to signed integer bitwise-or.

Covered:

- The typed i64 thunk module recognizes typed two-parameter int64 functions that return `arg0 | arg1`.
- The recognizer handles `BITWISE_OR` followed by `FUNCTION_RETURN`.
...
- The dedicated bitwise shared-library smoke executes `join(left: int, right: int): int { return left | right; }`, verifies runtime result `42`, and rejects `CallStaticDirect` / `CallStackValue` in this path.

Out of scope:

- General expression returns beyond this two-arg bitwise-or form.
...
- `zr_vm_aot_c_call_contracts_test`: missing `backend_aot_c_try_get_i64_arg0_arg1_bitwise_or_return(`.
...
- `zr_vm_aot_c_typed_direct_call_bitwise_shared_library_smoke_test`: 1/0.
