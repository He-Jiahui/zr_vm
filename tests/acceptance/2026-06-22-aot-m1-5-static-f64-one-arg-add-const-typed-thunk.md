# AOT M1.5 / 07-S5 Static F64 One-Arg Add-Constant Typed Thunk

- Timestamp: 2026-06-22 14:53:25 +08:00
...
- Adds a one-parameter floating-point add-constant typed thunk direct-call route.
...
- Covered source shape: `func inc(value: float): float { return value + 37.0; }`.
- Covered SemIR shape: optional parameter copy from slot 0, then `GET_CONSTANT`, `ADD_FLOAT`, and `FUNCTION_RETURN` of the add result.
...
- Out of scope: f64 divide/modulo, subtract-constant, multiply-constant, wider f64 expressions, additional f64 arities, inline structs, `in` / `out` writeback, deopt/dynamic bridge boxing, and full 07-S5 completion.

## Baseline

- RED baseline: `zr_vm_aot_c_call_contracts_test` failed because the f64 typed thunk source had no `backend_aot_c_try_get_f64_arg0_add_constant_return(` contract.
- The previous f64 behavior slices covered no-arg constants, one-arg identity, and two-arg add/subtract/multiply returns.
- The previous support slice had already split the reusable f64 smoke harness into `tests/parser/aot_c_typed_direct_call_f64_smoke_support.h`.
...
- Boundary cases:
  - Direct C call route for one `float` argument plus a constant inside the callee.
  - Runtime result 42 from `inc(seed)` with `5.0 + 37.0`.
...
- Failure: missing source contract text `backend_aot_c_try_get_f64_arg0_add_constant_return(`.
...
- `zr_vm_aot_c_typed_direct_call_f64_shared_library_smoke_test`: 6 tests, 0 failures.
...
- `zr_vm_aot/zr_vm_parser/src/zr_vm_parser/backend_aot/backend_aot_c_typed_f64_thunks.c`: 385 physical / 336 non-empty lines.
...
- `tests/parser/test_aot_c_call_contracts.c`: 880 physical / 823 non-empty lines.
- `tests/parser/test_aot_c_typed_direct_call_f64_shared_library_smoke.c`: 152 physical / 139 non-empty lines.
...
- The accepted behavior is one-argument f64 direct typed calls where the callee returns the argument plus a float constant.
