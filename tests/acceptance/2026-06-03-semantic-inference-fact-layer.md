...
Accepted for the parser/LSP class setter/meta parameter-list close diagnostic slice:

- `tests/language_server/test_lsp_class_member_parser_diagnostics.c` is a new focused class-member parser diagnostics target, keeping additional class-member parameter-list coverage out of the already-near-threshold `test_lsp_parser_diagnostics.c`.
- `class Sized { set length(value: int { return value; } }` returns code `missing_parameter_list_close`, problem text `Missing closing ')' in function declaration parameters`, and a suggestion to insert `)` after the parameter list before continuing.
- `class Callable { @call(value: int: int { return value; } }` returns code `missing_parameter_list_close`, problem text `Missing closing ')' in function declaration parameters`, and a suggestion to insert `)` after the parameter list before continuing.
- The first RED attempt in `build-wsl-gcc` proved the new CMake target needed a build graph refresh: `gmake` reported no rule for `zr_vm_language_server_class_member_parser_diagnostics_test`. After `cmake -S . -B build-wsl-gcc`, RED WSL gcc failed only the class-setter parameter-list close case.
- GREEN implementation changes `parse_property_set` so a setter parameter list missing `)` reports the existing structured parameter-list close diagnostic before entering accessor body parsing.
- A second RED WSL gcc run added the class-meta case and failed only `expected class-meta missing-parameter-list-close diagnostic to carry code, problem text, and suggestion`; the setter case already passed.
- GREEN implementation changes `parse_class_meta_function` so its primary parameter list missing `)` reports the same structured diagnostic and frees parsed params before returning. Function calls remain on `missing_call_close`, and grouped expressions remain on `missing_group_close`.
- `parser_class.c` remains below the large-file threshold. `tests/CMakeLists.txt` is large, but this slice only registers the new focused target in the existing language-server test target block; broader test CMake decomposition remains outside this parser diagnostic slice.
- GREEN WSL gcc focused validation passed in `build-wsl-gcc`: `cmake --build build-wsl-gcc --target zr_vm_language_server_class_member_parser_diagnostics_test -j 8`, followed by `./build-wsl-gcc/bin/zr_vm_language_server_class_member_parser_diagnostics_test`.
- GREEN WSL gcc/clang/MSVC focused matrix validation passed afterward for `zr_vm_language_server_class_member_parser_diagnostics_test`, `zr_vm_language_server_parser_diagnostics_test`, `zr_vm_language_server_declaration_parser_diagnostics_test`, and `zr_vm_language_server_statement_parser_diagnostics_test` in `build-wsl-gcc`, `build/codex-semantic-wsl-clang-debug`, and `build\codex-semantic-msvc-debug`; existing compiler warnings remain outside this slice.
- This slice improves parser/LSP diagnostic expressiveness for class setter accessor and class meta function parameter-list edits only; it does not touch constructors, interface/extern parameter-list families, core runtime, AOT/value-type lowering behavior, Debug evaluator semantics, REPL execution, or whole-repository green status.
...
Accepted for the parser/LSP class/interface declaration body-close diagnostic slice:

- `tests/language_server/test_lsp_declaration_parser_diagnostics.c` now validates missing final body braces for class and interface declarations through the focused `zr_vm_language_server_declaration_parser_diagnostics_test` target.
- `class Box { var id: int;` returns code `missing_declaration_body_close`, problem text `Missing closing '}' for class declaration body`, and a suggestion to insert `}` to close the class declaration body.
- `interface Sized { get length: int;` returns code `missing_declaration_body_close`, problem text `Missing closing '}' for interface declaration body`, and a suggestion to insert `}` to close the interface declaration body.
- RED WSL gcc first failed after adding the class declaration body-close regression in `build-wsl-gcc`: the focused declaration diagnostics target reported `expected class declaration missing-body-close diagnostic to carry code, problem text, and suggestion`, while the existing declaration body-open, extern spec close, and test name close cases still passed.
- GREEN implementation adds `ZrParser_DiagnosticBuilder_BuildMissingDeclarationBodyClose` and `report_missing_declaration_body_close`, then uses them from `parse_class_declaration` and `parse_interface_declaration` when a declaration body has already consumed `{` but reaches end-of-input before the final `}`.
- GREEN implementation records the opening-body location and reports the close diagnostic only after a real `{`, so `class Box` / `interface Sized` continue to use `missing_declaration_body_open` and are not double-reported as missing body closes.
- GREEN WSL gcc focused validation passed in `build-wsl-gcc`: `cmake --build build-wsl-gcc --target zr_vm_language_server_declaration_parser_diagnostics_test -j 8`, followed by `./build-wsl-gcc/bin/zr_vm_language_server_declaration_parser_diagnostics_test`.
- GREEN WSL gcc/clang/MSVC focused matrix validation passed afterward for `zr_vm_language_server_statement_parser_diagnostics_test`, `zr_vm_language_server_declaration_parser_diagnostics_test`, and `zr_vm_language_server_parser_diagnostics_test` in `build-wsl-gcc`, `build/codex-semantic-wsl-clang-debug`, and `build\codex-semantic-msvc-debug`; existing compiler warnings remain outside this slice.
- This slice improves parser/LSP diagnostic expressiveness for class/interface declaration body-close edits only; it does not touch function/enum/extern/test body-close handling, constructor receiver handling, SemIR, AOT/value-type lowering behavior, Debug evaluator semantics, REPL execution, or whole-repository green status.

Accepted for the parser/LSP enum/extern declaration body-close diagnostic slice:

- `tests/language_server/test_lsp_declaration_parser_diagnostics.c` now also validates missing final body braces for enum declarations and extern blocks through the focused `zr_vm_language_server_declaration_parser_diagnostics_test` target.
- `enum Tone { warm,` returns code `missing_declaration_body_close`, problem text `Missing closing '}' for enum declaration body`, and a suggestion to insert `}` to close the enum declaration body.
- `%extern("fixture") { NativeAdd(value: int): int;` returns code `missing_declaration_body_close`, problem text `Missing closing '}' for extern block body`, and a suggestion to insert `}` to close the extern block body.
- RED WSL gcc first failed after adding the enum declaration body-close regression in `build-wsl-gcc`: the focused declaration diagnostics target reported `expected enum declaration missing-body-close diagnostic to carry code, problem text, and suggestion`, while the existing declaration diagnostics cases still passed.
- GREEN implementation reuses the existing `ZrParser_DiagnosticBuilder_BuildMissingDeclarationBodyClose` / `report_missing_declaration_body_close` path from class/interface body-close handling, then changes `parse_enum_declaration` and `parse_extern_block` to report it only after a real body-opening `{` reaches end-of-input before the final `}`.
- `parser_extern.c` remains oversized; this slice replaced an existing local expected-token fallback in enum/extern block close handling and records broader extern parser responsibility extraction as the cleanup boundary.
- GREEN WSL gcc focused validation passed in `build-wsl-gcc`: `cmake --build build-wsl-gcc --target zr_vm_language_server_declaration_parser_diagnostics_test -j 8`, followed by `./build-wsl-gcc/bin/zr_vm_language_server_declaration_parser_diagnostics_test`.
- GREEN WSL gcc/clang/MSVC focused matrix validation passed afterward for `zr_vm_language_server_statement_parser_diagnostics_test`, `zr_vm_language_server_declaration_parser_diagnostics_test`, and `zr_vm_language_server_parser_diagnostics_test` in `build-wsl-gcc`, `build/codex-semantic-wsl-clang-debug`, and `build\codex-semantic-msvc-debug`; existing compiler warnings remain outside this slice.
- This slice improves parser/LSP diagnostic expressiveness for enum/extern declaration body-close edits only; it does not touch function/test body-close handling, constructor receiver handling, SemIR, AOT/value-type lowering behavior, Debug evaluator semantics, REPL execution, or whole-repository green status.

Accepted for the parser/LSP function/test declaration body-close diagnostic slice:

- `tests/language_server/test_lsp_declaration_parser_diagnostics.c` now also validates missing final body braces for function declarations and test declarations through the focused `zr_vm_language_server_declaration_parser_diagnostics_test` target.
- `func pick(): int { return 1;` returns code `missing_declaration_body_close`, problem text `Missing closing '}' for function declaration body`, and a suggestion to insert `}` to close the function declaration body.
- `%test("smoke") { return 1;` returns code `missing_declaration_body_close`, problem text `Missing closing '}' for test declaration body`, and a suggestion to insert `}` to close the test declaration body.
- RED WSL gcc first failed after adding the function declaration body-close regression in `build-wsl-gcc`: the focused declaration diagnostics target reported `expected function declaration missing-body-close diagnostic to carry code, problem text, and suggestion`, while the existing declaration diagnostics cases still passed.
- GREEN implementation keeps `parse_block` as the generic block parser, adds `parse_declaration_body_block` as a narrow declaration-body wrapper, and switches `parse_function_declaration` / `parse_test_declaration` to that wrapper so EOF after a real body-opening `{` reports `missing_declaration_body_close` instead of the broader `missing_block_close`.
- Generic statement blocks, lambdas, method bodies, and control-flow blocks still use `parse_block` and keep the existing `missing_block_close` behavior.
- `parser_statements.c` remains oversized at 1446 lines; this slice extracted a small declaration-aware block wrapper instead of adding another duplicated close path, and records broader statement parser responsibility extraction as the cleanup boundary. `parser_extern.c` remains oversized; this slice only changed the existing `%test` body call point.
- GREEN WSL gcc focused validation passed in `build-wsl-gcc`: `cmake --build build-wsl-gcc --target zr_vm_language_server_declaration_parser_diagnostics_test -j 8`, followed by `./build-wsl-gcc/bin/zr_vm_language_server_declaration_parser_diagnostics_test`; the build still prints the existing `parser_types.c` unused helper warning.
- GREEN WSL gcc/clang/MSVC focused matrix validation passed afterward for `zr_vm_language_server_statement_parser_diagnostics_test`, `zr_vm_language_server_declaration_parser_diagnostics_test`, and `zr_vm_language_server_parser_diagnostics_test` in `build-wsl-gcc`, `build/codex-semantic-wsl-clang-debug`, and `build\codex-semantic-msvc-debug`; Clang still prints the existing `wrap_type_in_task_identifier` unused-function warning.
- This slice improves parser/LSP diagnostic expressiveness for function/test declaration body-close edits only; it does not touch constructor receiver handling, SemIR, AOT/value-type lowering behavior, Debug evaluator semantics, REPL execution, or whole-repository green status.
