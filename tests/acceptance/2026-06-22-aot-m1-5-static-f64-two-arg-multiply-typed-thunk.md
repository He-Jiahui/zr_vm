# AOT M1.5 / 07-S5 Static F64 Two-Arg Multiply Typed Thunk

- Timestamp: 2026-06-22 14:38:18 +08:00
...
- Adds a two-parameter floating-point multiply-return typed thunk direct-call route.
...
- Covered source shape: `func product(left: float, right: float): float { return left * right; }`.
- Covered SemIR shape: direct `MUL_FLOAT` of parameter slots 0 and 1 followed by `FUNCTION_RETURN` of the multiply result.
...
- Out of scope: f64 divide/mod/constant expression returns, additional f64 arities, inline structs, `in` / `out` writeback, deopt/dynamic bridge boxing, and full 07-S5 completion.

## Baseline

- RED baseline: `zr_vm_aot_c_call_contracts_test` failed because the f64 typed thunk source had no `backend_aot_c_try_get_f64_arg0_arg1_multiply_return(` contract.
- The previous f64 slices only covered no-arg constants, one-arg identity, and two-arg add/subtract returns.
...
- Boundary cases:
  ...
  - Runtime result 42 from `product(first, second)` with `6.0 * 7.0`.
...
- Failure: missing source contract text `backend_aot_c_try_get_f64_arg0_arg1_multiply_return(`.
...
- `zr_vm_aot/zr_vm_parser/src/zr_vm_parser/backend_aot/backend_aot_c_typed_f64_thunks.c`: 290 physical / 253 non-empty lines.
...
- `tests/parser/test_aot_c_typed_direct_call_f64_shared_library_smoke.c`: 760 physical / 704 non-empty lines.
- `tests/parser/test_aot_c_call_contracts.c`: 878 physical / 821 non-empty lines.

## Acceptance Decision

- Accepted for this narrow 07-S5 sub-slice only.
- The accepted behavior is two-argument f64 direct typed calls where the callee returns the product of its two arguments.
