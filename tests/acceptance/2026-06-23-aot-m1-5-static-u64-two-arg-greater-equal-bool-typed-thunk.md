# AOT M1.5 07-S5 static u64 two-arg greater-equal bool typed thunk

Status: sub-slice complete; 07-S5 partial; M1.5/07 partial; stages 08-12 not started.

## Scope

This slice broadens the mixed `bool(u64, u64)` typed-to-typed direct-call route from unsigned less-than-or-equal to unsigned greater-than-or-equal, completing the current unsigned comparison subset.
...
- A source function `at_least(left: uint, right: uint): bool { return left >= right; }` still needed a `LOGICAL_GREATER_EQUAL_UNSIGNED` recognizer and a `TZrUInt64`-parameter greater-than-or-equal thunk writer.

## RED

- The typed call contract failed as expected because `backend_aot_c_try_get_bool_u64_arg0_arg1_greater_equal_return(` was absent.
- The new bool shared-library smoke was added to prove generated C for `at_least(50u, 8u)` needs the u64 bool thunk and must stay off the VM call/value fallback path.

## Implementation

- `backend_aot_c_typed_bool_thunks.c` reuses the shared u64 bool-return compare helper.
- The greater-than-or-equal wrapper accepts `LOGICAL_GREATER_EQUAL_UNSIGNED` followed by `FUNCTION_RETURN`.
- `backend_aot_c_can_emit_typed_bool_u64_two_arg_thunk()` now accepts `<`, `==`, `!=`, `>`, `<=`, and `>=` u64-parameter bool-return shapes.
- The greater-than-or-equal thunk emits `return (TZrBool)(zr_aot_arg0 >= zr_aot_arg1);`.
...
- Focused RED result:
  - typed call contracts 3/1, missing `backend_aot_c_try_get_bool_u64_arg0_arg1_greater_equal_return(`
...
- `backend_aot_c_typed_bool_thunks.c`: 754 physical / 658 non-empty lines.
...
- `test_aot_c_typed_call_contracts.c`: 759 physical / 721 non-empty lines.
- `test_aot_c_typed_direct_call_bool_shared_library_smoke.c`: 534 physical / 495 non-empty lines.

## Acceptance Decision

Accepted for the narrow mixed u64-parameter bool-return greater-than-or-equal typed direct-call slice and current unsigned comparison subset. This does not close 07-S5: inline structs, `in`/`out` writeback, deopt/dynamic bridges, broader ABI coverage, dynamic value access helpers, runtime-failure-capable division/modulo policy, and stages 08-12 remain open.
