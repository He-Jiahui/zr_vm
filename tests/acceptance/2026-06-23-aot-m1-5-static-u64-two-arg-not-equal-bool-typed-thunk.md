# AOT M1.5 07-S5 static u64 two-arg not-equal bool typed thunk

Status: sub-slice complete; 07-S5 partial; M1.5/07 partial; stages 08-12 not started.

## Scope

This slice broadens the mixed `bool(u64, u64)` typed-to-typed direct-call route from unsigned equality to unsigned inequality.
...
- A source function `different(left: uint, right: uint): bool { return left != right; }` still needed a `LOGICAL_NOT_EQUAL_UNSIGNED` recognizer and a `TZrUInt64`-parameter inequality thunk writer.

## RED

- The typed call contract failed as expected because `backend_aot_c_try_get_bool_u64_arg0_arg1_not_equal_return(` was absent.
- The new bool shared-library smoke was added to prove generated C for `different(21u, 22u)` needs the u64 bool thunk and must stay off the VM call/value fallback path.

## Implementation

- `backend_aot_c_typed_bool_thunks.c` reuses the shared u64 bool-return compare helper.
- The inequality wrapper accepts `LOGICAL_NOT_EQUAL_UNSIGNED` followed by `FUNCTION_RETURN`.
...
- Focused RED result:
  - typed call contracts 3/1, missing `backend_aot_c_try_get_bool_u64_arg0_arg1_not_equal_return(`
- Focused GREEN result:
  - typed call contracts 4/0
  - bool typed direct-call smoke 16/0
...
- `backend_aot_c_typed_bool_thunks.c`: 703 physical / 613 non-empty lines.
...
- `test_aot_c_typed_call_contracts.c`: 750 physical / 712 non-empty lines.
- `test_aot_c_typed_direct_call_bool_shared_library_smoke.c`: 450 physical / 417 non-empty lines.

## Acceptance Decision

Accepted for the narrow mixed u64-parameter bool-return inequality typed direct-call slice. This does not close 07-S5: remaining unsigned comparisons, inline structs, `in`/`out` writeback, deopt/dynamic bridges, broader ABI coverage, dynamic value access helpers, runtime-failure-capable division/modulo policy, and stages 08-12 remain open.
