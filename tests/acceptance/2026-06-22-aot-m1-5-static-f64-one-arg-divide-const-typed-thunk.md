# AOT M1.5 Static F64 One-Arg Divide-Const Typed Thunk
...
## Baseline
- Before this slice, f64 one-arg typed thunks covered identity, add-constant, subtract-constant, and multiply-constant returns.
- RED baseline: `zr_vm_aot_c_call_contracts_test` failed because `backend_aot_c_try_get_f64_arg0_divide_constant_return(` was absent.
...
## Test Inventory
- Contract: `tests/parser/test_aot_c_call_contracts.c` now requires the divide-constant recognizer, `DIV_FLOAT`, the zero-constant rejection check, and the generated `zr_aot_arg0 / K` return expression.
- Integration: `tests/parser/test_aot_c_typed_direct_call_f64_shared_library_smoke.c` executes `func halve(value: float): float { return value / 2.0; }` through a generated shared library and expects result 42.
...
- Negative cases: zero constants are rejected by the recognizer; runtime dynamic denominator failure behavior and f64 modulo remain out of scope for this direct thunk slice.
...
- GREEN focused command: same command after implementation; observed call contracts 8/0 and f64 typed direct-call smoke 9/0.
- Broader AOT command:
...
## Results
- RED: call contracts reported 8 tests, 1 failure, missing `backend_aot_c_try_get_f64_arg0_divide_constant_return(`.
- GREEN focused: call contracts 8 tests / 0 failures; f64 typed direct-call smoke 9 tests / 0 failures.
...
## Acceptance Decision
- Accepted for the narrow 07-S5 sub-slice: static one-arg f64 nonzero divide-constant typed thunk direct-call.
- Not accepted as full 07-S5: f64 dynamic divide, f64 modulo, wider f64 expression routes, inline structs, in/out writeback, deopt/dynamic bridges, and full boundary marshaling remain open.
