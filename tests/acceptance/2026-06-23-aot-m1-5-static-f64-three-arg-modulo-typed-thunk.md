# 2026-06-23 AOT M1.5 07-S5 static f64 three-arg modulo typed thunk

## 状态

- 时间：2026-06-23 11:48:03 +08:00
- 状态：子切片完成；07-S5 / M1.5 / 07 仍部分完成；08-12 未开始

## 完成项目

- `backend_aot_c_typed_f64_thunk_shapes.{h,c}` 新增三参数 float 取模识别：
  `backend_aot_c_try_get_f64_arg0_arg1_arg2_modulo_return()` 只接受三个 f64
  参数、两条有序 `MOD_FLOAT` 后接 `FUNCTION_RETURN` 的窄形态。
- shape 识别保持非交换运算顺序：第一步必须为 `arg0 % arg1`，第二步必须为
  first-result `% arg2`。
- `backend_aot_c_can_emit_typed_f64_three_arg_thunk()` 纳入三参 modulo shape。
- `backend_aot_c_typed_f64_thunks.c` 新增三参 modulo thunk writer，在 `arg1` 或
...
- RED：f64 shared-library smoke 18 tests / 1 failure，新增三参 modulo 用例为
  `Expected Non-NULL`。
...
- 共享库烟测组通过：shared 8/0、call smoke 5/0、typed direct-call 5/0、i64 three-arg 6/0、
  bool 19/0、u64 22/0、f64 18/0、typed arithmetic 5/0、typed bitwise 6/0、value-type 1/0、
...
- `backend_aot_c_typed_f64_thunk_shapes.c`：943 / 822
- `backend_aot_c_typed_f64_thunk_shapes.h`：25 / 22
- `test_aot_c_typed_call_contracts.c`：907 / 869
- `test_aot_c_typed_direct_call_f64_shared_library_smoke.c`：452 / 415

大文件备注：f64 shape source 已接近 1000 行。后续再添加 f64 shape 前，应优先把三参数
shape 识别拆到独立模块，而不是继续扩张该文件。

## 未完成

This is narrow f64 three-arg modulo coverage only. inline structs、`in`/`out`
