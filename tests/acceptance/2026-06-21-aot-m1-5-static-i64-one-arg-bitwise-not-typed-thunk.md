# AOT M1.5 07-S5 Static I64 One-Arg Bitwise-Not Typed Thunk

Date: 2026-06-22 09:09:06 +08:00

## Scope

This slice extends the one-argument i64 typed direct-call route to signed integer bitwise-not.

Covered:

- The typed i64 thunk module recognizes typed one-parameter int64 functions that return `~arg0`.
- The recognizer handles `BITWISE_NOT` followed by `FUNCTION_RETURN`.
...
- The dedicated bitwise shared-library smoke executes `invert(value: int): int { return ~value; }`, verifies runtime result `42`, and rejects `CallStaticDirect` / `CallStackValue` in this path.

Out of scope:

- General expression returns beyond this one-arg bitwise-not form.
...
- `zr_vm_aot_c_call_contracts_test`: missing `backend_aot_c_try_get_i64_arg0_bitwise_not_return(`.
