# AOT M1.5 / 07-S5 Static u64 Three-Arg Bitwise-Or Typed Thunk

时间：2026-06-23 09:55:55 +08:00
...
- 新增 `backend_aot_c_try_read_u64_bitwise_or_operands()`，识别两条
  `BITWISE_OR` 指令后接 `FUNCTION_RETURN` 的三参数 unsigned return 形态。
- `backend_aot_c_can_emit_typed_u64_three_arg_thunk()` 现在覆盖 add、multiply、
  subtract、bitwise-and、bitwise-or。
- u64 thunk writer 对 bitwise-or callee 发出：
...
- `tests/parser/test_aot_c_typed_direct_call_u64_shared_library_smoke.c` 新增
  `flags3(40, 2, 0)` shared-library smoke，验证运行结果 42，且拒绝
...
- `zr_vm_aot_c_typed_call_contracts_test`：4 tests, 1 failure，缺少
  `return (TZrUInt64)(zr_aot_arg0 | zr_aot_arg1 | zr_aot_arg2);`。
- `zr_vm_aot_c_typed_direct_call_u64_shared_library_smoke_test`：19 tests,
  1 failure，three-arg bitwise-or case 在 generated C 中找不到期望的三参
  bitwise-or thunk/direct-call 文本，失败为 `Expected Non-NULL`。
...
- 共享库烟测组：shared 8/0、call smoke 5/0、typed direct-call 5/0、i64 three-arg
  smoke 6/0、bool 19/0、u64 19/0、f64 13/0、typed arithmetic 5/0、typed bitwise
  6/0、value-type 1/0、float smoke 1/0、generic numeric smoke 1/0、global smoke
  9/0、logical smoke 4/0、power smoke 1/0。

## 文件规模

- `backend_aot_c_typed_u64_thunks.c`：816 physical / 733 non-empty lines。
- `backend_aot_c_typed_u64_thunk_shapes.c`：595 physical / 534 non-empty lines。
- `backend_aot_c_typed_u64_thunk_shapes.h`：18 physical / 15 non-empty lines。
- `tests/parser/test_aot_c_typed_call_contracts.c`：854 physical / 816 non-empty lines。
- `tests/parser/test_aot_c_typed_direct_call_u64_shared_library_smoke.c`：
  477 physical / 438 non-empty lines。

## 仍未完成

三参 u64 add/multiply/subtract/bitwise-and/bitwise-or 窄形态已覆盖。u64 三参
XOR、runtime-failure-capable division/modulo、07-S5 完整验收、inline structs、
