# AOT M1.5 07-S5 static bool two-arg equality typed thunk
...
- The previous bool two-argument expression routes recognized `left && right` and `left || right`.
- Statically bool equality already lowered to `LOGICAL_EQUAL_BOOL`, but the generated callee body still used the frame-backed `zr_aot_bool_compare_exec` path with `SZrTypeValue` access.
...
- The typed call contract first failed with missing source text `backend_aot_c_try_get_bool_arg0_arg1_equal_return(`.
...
- `backend_aot_c_typed_bool_thunks.c` now recognizes a bool-return, two-bool-parameter callee whose body is `LOGICAL_EQUAL_BOOL` followed by `FUNCTION_RETURN`.
- The recognizer accepts the two parameter operands in either order and rejects non-bool signatures, varargs, and non-returned comparison results.
- The two-argument bool can-emit gate now accepts equality, logical-and, and logical-or returns.
- The equality thunk emits `return (TZrBool)(zr_aot_arg0 == zr_aot_arg1);`.
- The bool shared-library smoke executes `same(true, true)`, observes the expected `42` return path, and rejects `CallStaticDirect`, `CallStackValue`, and typed-destination stack sync for the scalar-only route.
...
Accepted for the narrow bool `left == right` typed direct-call slice. This does not close 07-S5: bool inequality, inline structs, `in`/`out` writeback, deopt/dynamic bridges, broader typed ABI coverage, and stages 08-12 remain open.
