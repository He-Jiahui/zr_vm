# AOT 07-S2/S4 Generic Numeric F64 ADD Local Fold

- 时间：2026-07-04 13:10:12 +08:00
- 状态：完成本子切片；07-S2/S4 部分完成；07~12 总目标继续进行中。

## 完成项目

- 覆盖手工 bytecode 形态：`GET_CONSTANT float -> GET_CONSTANT float -> ADD -> RETURN`。
- proven f64 operands 保留为 `zr_aot_scalar_constant_f64_local`，供 generic numeric `ADD` 直接读取。
- generic `ADD` 在 destination/left/right 均已证明为 f64 scalar local，且结果 value slot 可跳过时输出
  `zr_aot_generic_numeric_f64_add_scalar_local`。
- 本地 ADD 发射 `zr_aot_f2 = zr_aot_f0 + zr_aot_f1;`，不再调用
...
- RED：focused WSL GCC smoke 新增 ADD 用例后失败于缺少 `zr_aot_generic_numeric_f64_add_scalar_local`；
  生成 C 仍走 `ZrLibrary_AotRuntime_GenericNumericAdd(state, &frame, 2, 0, 1)` boundary。
- GREEN：`backend_aot_c_lowering_generic_numeric_arithmetic.c` 在 ADD runtime fallback 前尝试 f64 binary
  scalar-local helper；`backend_aot_c_function_body.c` / `backend_aot_c_emitter.h` 将 `instructionIndex`
  传入 ADD lowering；`backend_aot_c_scalar_locals.c` 将 generic ADD 纳入 f64 operand、destination、
  exec-write 和 f64 consumer proof。
- GREEN 目标项目不包含 `zr_aot_arith_exec_generic_numeric_binary_boundary`、
  `GenericNumericAdd(state, &frame, 2, 0, 1)`、`SyncFloatLocal(state, &frame, 2, &zr_aot_f2)` 或
  `ZR_VALUE_IS_TYPE_FLOAT(zr_aot_left->type)`。

## 验证

- WSL GCC：generic numeric smoke 2/0；generic numeric contracts 1/0；source contracts 24/0；
  power smoke 1/0；power contracts 2/0；generic LOGICAL_NOT 8/0；generic equality stack-copy 4/0。
- WSL Clang：generic numeric smoke 2/0；generic numeric contracts 1/0；source contracts 24/0；
...
- Windows MSVC Debug（`build-msvc-aot-stack-copy`）：构建 focused smoke/contract/regression targets；
  generic numeric shared-library smoke 0 failures / 2 expected ignores；generic numeric contracts 1/0；
...
- generic numeric SUB/MUL/DIV 及 dynamic/unproven numeric operands 的本地化。
