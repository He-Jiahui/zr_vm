...
- The `finally` body contains only nested blocks and linear expression
  ...
  ownership/reference/GC-neutral `int`, `bool`, or `float` identifiers or
  literals by value.
...
- The focused source cleanup suite passes 47/47 on Windows MSVC and WSL GCC and
...
- GCC ASan+UBSan passes the focused suite 47/47 five consecutive times, with
