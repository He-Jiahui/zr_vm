...
- For the terminal-return form, the return operand is captured before cleanup,
  cleanup instead targets a dedicated RETURN block, and that block consumes the
  pre-cleanup ValueId without reloading a local changed by `finally`.
...
The same test keeps nonlinear return, `throw`, and a combined catch-plus-finally
statement on the legacy path. Calls, declarations, nested nonlinear control
flow, ownership cleanup, multiple completion kinds, and exceptional cleanup
entry are also outside this slice. Those shapes must not publish a partial
cleanup graph or restart a detached CFG after rejection.
...
- The focused source cleanup suite passes 5/5 on Windows MSVC and WSL GCC and
