...
- The `finally` body contains only nested blocks and linear expression
  ...
  when a normal sibling path remains, or one resolved direct call outside
...
- For a terminal return or throw, the abrupt operand is captured before
  ...
  `finally`. Multiple same-kind return or throw sites with a normal sibling
  reuse the same payload merge and completion block.
...
The same test keeps nonlinear return/throw payloads, multiple return/throw
sites without a normal sibling, two protected calls, and a combined
catch-plus-finally statement on the legacy path. Literal, converting, non-value,
...
and therefore retains its direct cleanup edge. When a normal sibling remains,
same-kind return/throw sites use the same pending selector and payload merge.
One or more same-kind,
...
- The focused source cleanup suite passes 31/31 on Windows MSVC and WSL GCC and
...
- GCC ASan+UBSan passes the focused suite 31/31 five consecutive times, with
