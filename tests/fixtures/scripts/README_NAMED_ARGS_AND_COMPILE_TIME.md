...
### 编译测试
```bash
cmake -S . -B out/build/msvc/debug
...
```bash
# 直接运行某个 fixture
zr_vm_cli tests/fixtures/scripts/named_arguments.zr
```

### 产物位置
```text
输入 fixture:  tests/fixtures/scripts/
golden 快照:   tests/golden/
构建期输出:   out/build/msvc/debug/tests_generated/scripts/
```
