...
static void zr_vm_task_worker_run_launch(ZrVmTaskWorkerLaunch *launch) {
    ...
    if (!ZrVmTask_Register(workerGlobal)) {
        zr_vm_task_worker_queue_error_message(launch->ownerRuntime, launch->ownerHandle, "Failed to register zr.task in worker isolate");
        ZrCore_GlobalState_Free(workerGlobal);
        return;
    }
...

    {
        ...
        if (!zr_vm_task_transport_encode_value(workerState,
                                               &resultValue,
                                               &payload,
                                               "spawnThread only returns sendable scalar values or Channel handles")) {
            zr_vm_task_worker_queue_error_message(launch->ownerRuntime,
                                                  launch->ownerHandle,
                                                  "spawnThread only returns sendable scalar values or Channel handles");
            goto cleanup;
...
TZrBool zr_vm_task_spawn_thread_worker(ZrLibCallContext *context, SZrTypeValue *result, SZrObject *mainScheduler) {
    ...
    if (context == ZR_NULL || context->state == ZR_NULL || result == ZR_NULL || mainScheduler == ZR_NULL ||
    ...
    if (captureCount > 0u) {
        ...
        for (captureIndex = 0; captureIndex < captureCount; captureIndex++) {
            ...
            if (!zr_vm_task_transport_encode_value(context->state,
                                                   ...
                                                   "spawnThread only captures sendable scalar values or Channel handles")) {
                zr_vm_task_worker_launch_free(launch);
                remove(tempPath);
                return ZR_FALSE;
            }
