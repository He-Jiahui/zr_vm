...
SZrObject *zr_vm_task_new_typed_object(SZrState *state, const TZrChar *typeName) {
    ...
    if (ZrLib_Module_GetLoaded(state, kTaskModuleName) == ZR_NULL) {
        if (ZrLib_Module_GetExport(state, kTaskModuleName, "spawn") == ZR_NULL) {
...
static TZrBool zr_vm_task_scheduler_step_internal(SZrState *state, SZrObject *scheduler) {
    ...
    if (queuedValue == ZR_NULL) {
        if (zr_vm_task_scheduler_has_pending_workers(state, scheduler) &&
            zr_vm_task_scheduler_wait_for_external(state, scheduler, kTaskSchedulerExternalWaitMs) &&
            zr_vm_task_scheduler_process_external(state, scheduler)) {
            return ZR_TRUE;
        }
...
static TZrBool zr_vm_task_spawn_thread(ZrLibCallContext *context, SZrTypeValue *result) {
    ...
    scheduler = zr_vm_task_main_scheduler(context->state);
    if (!zr_vm_task_create_async_handle(context->state, scheduler, callable, result) ||
        result->type != ZR_VALUE_TYPE_OBJECT || result->value.object == ZR_NULL) {
        return ZR_FALSE;
    }
    ...
    zr_vm_task_set_int_field(context->state, handle, kTaskStatusField, ZR_VM_TASK_STATUS_QUEUED);
    return zr_vm_task_spawn_thread_worker(context, result, scheduler);
}
...
static const ZrLibTypeDescriptor g_task_types[] = {
        ...
        ZR_LIB_TYPE_DESCRIPTOR_INIT("Channel", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_channel_methods,
                                    ...
                                    "Non-blocking FIFO channel wrapper for same-isolate message passing.", ZR_NULL,
                                    ZR_NULL, 0, ZR_NULL, 0, ZR_NULL, ZR_TRUE, ZR_TRUE, "Channel()",
                                    g_task_single_generic_parameter, ZR_ARRAY_COUNT(g_task_single_generic_parameter)),
        ZR_LIB_TYPE_DESCRIPTOR_INIT("Mutex", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_mutex_methods,
                                    ZR_ARRAY_COUNT(g_mutex_methods), g_mutex_meta_methods,
                                    ZR_ARRAY_COUNT(g_mutex_meta_methods),
                                    "Cooperative mutex wrapper with explicit lock and unlock operations.", ZR_NULL,
                                    ZR_NULL, 0, ZR_NULL, 0, ZR_NULL, ZR_TRUE, ZR_TRUE, "Mutex(value: T)",
                                    g_task_single_generic_parameter, ZR_ARRAY_COUNT(g_task_single_generic_parameter)),
        ZR_LIB_TYPE_DESCRIPTOR_INIT("Shared", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_shared_methods,
                                    ...
                                    "Shared wrapper with strong and weak handle semantics for task runtime cells.",
        ...
        ZR_LIB_TYPE_DESCRIPTOR_INIT("WeakShared", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_weak_shared_methods,
                                    ...
                                    g_task_single_generic_parameter, ZR_ARRAY_COUNT(g_task_single_generic_parameter)),
        ZR_LIB_TYPE_DESCRIPTOR_INIT("AtomicBool", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_atomic_bool_methods,
                                    ZR_ARRAY_COUNT(g_atomic_bool_methods), g_atomic_bool_meta_methods,
                                    ZR_ARRAY_COUNT(g_atomic_bool_meta_methods),
                                    "Atomic bool wrapper with load, store, and compareExchange.", ZR_NULL, ZR_NULL, 0,
                                    ZR_NULL, 0, ZR_NULL, ZR_TRUE, ZR_TRUE, "AtomicBool(value: bool)", ZR_NULL, 0),
        ZR_LIB_TYPE_DESCRIPTOR_INIT("AtomicInt", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_atomic_int_methods,
                                    ZR_ARRAY_COUNT(g_atomic_int_methods), g_atomic_int_meta_methods,
                                    ZR_ARRAY_COUNT(g_atomic_int_meta_methods),
                                    "Atomic integer wrapper with load, store, compareExchange, and fetch RMW ops.",
                                    ZR_NULL, ZR_NULL, 0, ZR_NULL, 0, ZR_NULL, ZR_TRUE, ZR_TRUE, "AtomicInt(value: T)",
                                    g_task_single_generic_parameter,
                                    ZR_ARRAY_COUNT(g_task_single_generic_parameter)),
        ZR_LIB_TYPE_DESCRIPTOR_INIT("AtomicUInt", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_atomic_uint_methods,
                                    ZR_ARRAY_COUNT(g_atomic_uint_methods), g_atomic_uint_meta_methods,
                                    ZR_ARRAY_COUNT(g_atomic_uint_meta_methods),
                                    "Atomic unsigned integer wrapper with load, store, compareExchange, and fetch RMW ops.",
                                    ZR_NULL, ZR_NULL, 0, ZR_NULL, 0, ZR_NULL, ZR_TRUE, ZR_TRUE,
                                    "AtomicUInt(value: T)", g_task_single_generic_parameter,
                                    ZR_ARRAY_COUNT(g_task_single_generic_parameter)),
};
...
static const ZrLibTypeHintDescriptor g_task_hints[] = {
        ...
        {"Channel", "type", "class Channel<T>", "Non-blocking FIFO channel wrapper."},
        {"Mutex", "type", "class Mutex<T>", "Task mutex wrapper with explicit lock and unlock."},
        ...
        {"AtomicBool", "type", "class AtomicBool", "Atomic bool wrapper with compareExchange."},
        {"AtomicInt", "type", "class AtomicInt<T>", "Atomic signed integer wrapper."},
        {"AtomicUInt", "type", "class AtomicUInt<T>", "Atomic unsigned integer wrapper."},
};
