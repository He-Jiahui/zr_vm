set -e
cd /mnt/e/Git/zr_vm
mkdir -p /tmp/zrvm-cases
cat > /tmp/zrvm-cases/case1.zr <<'EOF'
class Loop {
...
return directValue + metaValue;
EOF
cat > /tmp/zrvm-cases/case2.zr <<'EOF'
class Loop {
    pub @call(n: int, acc: int): int {
        if (n == 0) { return acc; }
        return this(n - 1, acc + 1);
...
EOF
cat > /tmp/zrvm-cases/case3.zr <<'EOF'
class Loop {
    pub @call(n: int, acc: int): int {
        if (n == 0) { return acc; }
        return this(n - 1, acc + 1);
...
return directValue + metaValue + guardedValue;
EOF
cat > /tmp/zrvm-cases/case4.zr <<'EOF'
class Loop {
    pub @call(n: int, acc: int): int {
        if (n == 0) { return acc; }
        return this(n - 1, acc + 1);
...
EOF
for f in /tmp/zrvm-cases/case*.zr; do
  echo "==== $(basename "$f")"
