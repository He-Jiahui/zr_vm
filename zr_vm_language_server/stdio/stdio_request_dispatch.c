...
int dispatch_request_method(SZrStdioServer *server,
    ...
    *outStatus = ZR_LSP_HANDLER_OK;
    if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_COMPLETION) == 0) {
        return dispatch_handler_result(handle_completion_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_COMPLETION_ITEM_RESOLVE) == 0) {
        return dispatch_handler_result(handle_completion_item_resolve_request(server, params), outResult, outStatus);
        ...
        return dispatch_handler_result(handle_formatting_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_RANGE_FORMATTING) == 0) {
        return dispatch_handler_result(handle_range_formatting_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_RANGES_FORMATTING) == 0) {
        if (!server->supportsRangesFormatting) {
            return 0;
        }
        return dispatch_handler_result(handle_ranges_formatting_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_ON_TYPE_FORMATTING) == 0) {
        return dispatch_handler_result(handle_on_type_formatting_request(server, params), outResult, outStatus);
        ...
        return dispatch_handler_result(handle_code_action_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_CODE_ACTION_RESOLVE) == 0) {
        return dispatch_handler_result(handle_code_action_resolve_request(server, params), outResult, outStatus);
        ...
        return dispatch_handler_result(handle_linked_editing_range_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_MONIKER) == 0) {
        return dispatch_handler_result(handle_moniker_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_INLINE_VALUE) == 0) {
        return dispatch_handler_result(handle_inline_value_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_INLINE_COMPLETION) == 0) {
        if (!server->supportsInlineCompletion) {
            return 0;
        }
        return dispatch_handler_result(handle_inline_completion_request(server, params), outResult, outStatus);
        ...
        return dispatch_handler_result(handle_text_document_diagnostic_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_WORKSPACE_DIAGNOSTIC) == 0) {
        return dispatch_handler_result(handle_workspace_diagnostic_request(server, params), outResult, outStatus);
        ...
        return dispatch_handler_result(handle_will_rename_files_request(server, params), outResult, outStatus);
        ...
        return dispatch_handler_result(handle_semantic_tokens_full_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_SEMANTIC_TOKENS_FULL_DELTA) == 0) {
        return dispatch_handler_result(handle_semantic_tokens_full_delta_request(server, params), outResult, outStatus);
    } else if (strcmp(method, ZR_LSP_METHOD_TEXT_DOCUMENT_SEMANTIC_TOKENS_RANGE) == 0) {
        return dispatch_handler_result(handle_semantic_tokens_range_request(server, params), outResult, outStatus);
        ...
        return dispatch_handler_result(handle_project_modules_request(server, params), outResult, outStatus);
    } else {
        return 0;
    }
