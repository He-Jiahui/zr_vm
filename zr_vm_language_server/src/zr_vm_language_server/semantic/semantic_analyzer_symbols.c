...
static SZrString *semantic_extract_generic_argument_name_string(SZrSemanticAnalyzer *analyzer,
                                                                SZrAstNode *node) {
    SZrInferredType inferredType;
    SZrString *typeName = ZR_NULL;
    SZrState *state;

    if (analyzer == ZR_NULL || analyzer->compilerState == ZR_NULL || node == ZR_NULL || node->type != ZR_AST_TYPE) {
        return ZR_NULL;
    }
        ...
        if (parameter->genericTypeConstraints != ZR_NULL) {
            for (TZrSize constraintIndex = 0;
                ...
                SZrString *constraintName =
                    semantic_extract_generic_argument_name_string(analyzer, constraintNode);
...
static SZrInferredType *create_type_info_from_type_node_with_callable_context(SZrState *state,
    ...
    if (state == ZR_NULL || analyzer == ZR_NULL || typeNode == ZR_NULL) {
        return ZR_NULL;
    }
...
static SZrFileRange semantic_parameter_diagnostic_location(
...
static TZrBool infer_symbol_expression_type(SZrState *state,
    ...
    if (node->type == ZR_AST_IDENTIFIER_LITERAL && node->data.identifier.name != ZR_NULL) {
        SZrSymbol *symbol =
            ZrLanguageServer_SymbolTable_LookupAtPosition(analyzer->symbolTable, node->data.identifier.name, node->location);
        if (symbol != ZR_NULL &&
...
static SZrInferredType *create_type_info_from_inferred_type(SZrState *state,
                                                            const SZrInferredType *inferredType) {
    SZrInferredType *typeInfo = allocate_object_type_info(state);

    if (typeInfo == ZR_NULL || inferredType == ZR_NULL) {
        return typeInfo;
    }
...
static SZrInferredType *create_type_info_for_variable(SZrState *state,
    ...
    if (varDecl->typeInfo != ZR_NULL) {
        typeInfo = create_type_info_from_type_node(state, analyzer, varDecl->typeInfo);
        if (typeInfo == ZR_NULL) {
            ZrLanguageServer_SemanticAnalyzer_ReportCannotInferExactType(
                    state,
                    analyzer,
                    varDecl->typeInfo->name != ZR_NULL ? varDecl->typeInfo->name->location : varDecl->pattern->location);
        }
        return typeInfo;
    }
    ...
    if (varDecl->value != ZR_NULL) {
        ...
        if (infer_symbol_expression_type(state, analyzer, varDecl->value, &inferredType)) {
            typeInfo = create_type_info_from_inferred_type(state, &inferredType);
        } else {
            if (analyzer != ZR_NULL && analyzer->compilerState != ZR_NULL && analyzer->compilerState->hasError) {
...
static SZrInferredType *create_type_info_for_callable_return(SZrState *state,
    ...
    if (returnTypeNode != ZR_NULL) {
        typeInfo = create_type_info_from_type_node_with_callable_context(state,
                                                                         analyzer,
                                                                         returnTypeNode,
                                                                         callableDeclarationNode);
        if (typeInfo == ZR_NULL) {
    ...
    ZrParser_InferredType_Init(state, &inferredReturnType, ZR_VALUE_TYPE_NULL);
    savedEnv = push_runtime_type_binding_scope(state, analyzer);
    if (params != ZR_NULL && params->nodes != ZR_NULL) {
        for (TZrSize index = 0; index < params->count; index++) {
            ...
            paramTypeInfo = create_type_info_from_type_node_with_callable_context(state,
                                                                                  analyzer,
                                                                                  parameter->typeInfo,
                                                                                  callableDeclarationNode);
    ...
    ZrParser_InferredType_Free(state, &inferredReturnType);
    return typeInfo;
}
...
static void register_function_type_binding_in_env(SZrState *state,
    ...
    SZrCompilerState *compilerState;
    SZrInferredType *resolvedReturnTypeInfo = ZR_NULL;
    SZrInferredType returnType;
    SZrArray paramTypes;
    SZrArray genericParameters;
    SZrArray parameterPassingModes;
    SZrAstNode *declarationNode;
    TZrBool hasUnresolvedParameter = ZR_FALSE;
    ...
    resolvedReturnTypeInfo =
        create_type_info_for_callable_return(state,
                                             ...
                                             declarationNode != ZR_NULL
                                                 ...
                                                 : funcDecl->nameLocation);
    if (resolvedReturnTypeInfo == ZR_NULL) {
    ...
    ZrParser_InferredType_Copy(state, &returnType, resolvedReturnTypeInfo);
        ...
        for (TZrSize index = 0; index < funcDecl->params->count; index++) {
            ...
            if (paramNode->data.parameter.typeInfo != ZR_NULL) {
                if (!ZrLanguageServer_SemanticAnalyzer_BuildDeclaredTypeInferredType(
                    ...
                    hasUnresolvedParameter = ZR_TRUE;
                    continue;
                }
            } else {
                ZrLanguageServer_SemanticAnalyzer_ReportCannotInferExactType(
    ...
    if (hasUnresolvedParameter) {
        ...
        ZrParser_InferredType_Free(state, resolvedReturnTypeInfo);
        ZrCore_Memory_RawFree(state->global, resolvedReturnTypeInfo, sizeof(SZrInferredType));
        semantic_free_collected_generic_parameters(state, &genericParameters);
        if (parameterPassingModes.isValid && parameterPassingModes.head != ZR_NULL) {
            ZrCore_Array_Free(state, &parameterPassingModes);
        }
...
static void collect_single_parameter_symbol(SZrState *state,
    ...
    if (typeInfo == ZR_NULL) {
        ZrLanguageServer_SemanticAnalyzer_ReportCannotInferExactType(state, analyzer, ownerNode->location);
    }
    ...
    switch (node->type) {
        ...
        case ZR_AST_FUNCTION_DECLARATION: {
            ...
            if (name != ZR_NULL) {
                ...
                returnType = create_type_info_for_callable_return(state,
                                                                  analyzer,
                                                                  funcDecl->returnType,
                                                                  funcDecl->params,
                                                                  node,
        ...
        case ZR_AST_EXTERN_FUNCTION_DECLARATION: {
            ...
            const SZrSemanticSymbolRecord *canonicalSymbol;
            ...
            ZrLanguageServer_SymbolTable_AddSymbolEx(state,
                                                     ...
                                                     &symbol);
            if (symbol != ZR_NULL) {
                symbol->selectionRange = canonicalFunction != ZR_NULL &&
                                                 canonicalFunction->hasDeclarationRange
                                         ? canonicalFunction->declarationRange
                                         : compute_extern_callable_name_range(node, name);
            }
            canonicalSymbol = canonicalFunction != ZR_NULL &&
                                      analyzer->semanticContext != ZR_NULL
                              ? ZrParser_Semantic_FindSymbolById(
                                      analyzer->semanticContext,
                                      canonicalFunction->symbolId)
                              : ZR_NULL;
            if (symbol != ZR_NULL && canonicalFunction != ZR_NULL &&
                canonicalSymbol != ZR_NULL &&
                canonicalFunction->typeId != ZR_SEMANTIC_ID_INVALID) {
                symbol->semanticId = canonicalFunction->symbolId;
                symbol->semanticTypeId = canonicalFunction->typeId;
                symbol->overloadSetId = canonicalSymbol->overloadSetId;
            } else {
                ZrLanguageServer_SemanticAnalyzer_RegisterSymbolSemantics(
                        analyzer,
                        symbol,
