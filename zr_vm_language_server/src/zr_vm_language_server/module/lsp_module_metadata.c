...
static TZrBool module_metadata_resolve_descriptor_plugin_file_uri(SZrState *state,
    ...
    if (module_metadata_resolve_native_module_descriptor_internal(state, projectIndex, moduleName, &sourceKind) == ZR_NULL ||
    ...
    if (!ZrLibrary_NativeRegistry_GetModuleInfo(state->global, moduleName, &moduleInfo) ||
        moduleInfo.sourcePath == ZR_NULL ||
        moduleInfo.sourcePath[0] == '\0' ||
...
TZrBool ZrLanguageServer_LspModuleMetadata_LoadBinaryModuleSource(SZrState *state,
    ...
    SZrLibrary_File_Reader *reader = ZR_NULL;
    SZrIo io;
    TZrBool loaded = ZR_FALSE;
    ...
    io.isBinary = ZR_TRUE;
    loaded = ZrParser_ModuleInitAnalysis_TryLoadBinaryMetadataSourceFromIo(state, &io, outSource);
    if (io.close != ZR_NULL) {
...
static TZrBool module_metadata_binary_export_symbol_try_range(SZrString *uri,
    ...
    if (symbol == ZR_NULL || symbol->columnInSourceStart == 0 || symbol->columnInSourceEnd == 0 || outRange == ZR_NULL) {
        return ZR_FALSE;
    }

    startLine = module_metadata_binary_export_normalize_line(symbol->lineInSourceStart, symbol->columnInSourceStart);
    endLine = module_metadata_binary_export_normalize_line(symbol->lineInSourceEnd, symbol->columnInSourceEnd);
...
TZrBool ZrLanguageServer_LspModuleMetadata_ResolveBinaryExportDeclaration(SZrState *state,
    ...
    if (module_metadata_binary_export_symbol_try_range(*outUri, symbol, outRange)) {
        resolved = ZR_TRUE;
    }
...
TZrBool ZrLanguageServer_LspModuleMetadata_ResolveNativeModuleUri(SZrState *state,
    ...
    if (sourceKind == ZR_LSP_IMPORTED_MODULE_SOURCE_NATIVE_DESCRIPTOR_PLUGIN &&
        module_metadata_resolve_descriptor_plugin_file_uri(state, projectIndex, moduleText, outUri)) {
        return ZR_TRUE;
    }
