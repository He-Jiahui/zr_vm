...
static inline SZrTypeValue *execution_frame_value_slot_signed_fast_inline(
        ...
        const SZrFunction *function,
        TZrStackValuePointer frameBase,
        TZrStackValuePointer packedDirectValueBase,
        TZrUInt32 stackSlot,
        SZrProfileRuntime *profileRuntime,
        TZrBool recordHelpers) {
    if (ZR_LIKELY(packedDirectValueBase != ZR_NULL &&
        ...
        return ZrCore_Stack_GetValueNoProfile(
                packedDirectValueBase + stackSlot);
    }

    return execution_frame_value_slot_dispatch_fast_inline(
            state,
            function,
            frameBase,
            stackSlot,
...
#if defined(_MSC_VER)
#define ZR_EXECUTION_FRAME_BASE_NO_INLINE __declspec(noinline)
#elif defined(__GNUC__) || defined(__clang__)
#define ZR_EXECUTION_FRAME_BASE_NO_INLINE __attribute__((noinline))
#else
#define ZR_EXECUTION_FRAME_BASE_NO_INLINE
#endif

static ZR_EXECUTION_FRAME_BASE_NO_INLINE TZrStackValuePointer
execution_packed_direct_value_frame_base(const SZrFunction *function,
                                         TZrStackValuePointer frameBase) {
    if (!ZrCore_Function_HasDirectValueFrameSlotSummary(function) ||
        frameBase == ZR_NULL) {
        return ZR_NULL;
    }
    return (TZrStackValuePointer)((TZrByte *)frameBase +
                                  function->stackSize *
...
static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_known_vm_fast(SZrState *state,
                                                                     ...
                                                                     SZrProfileRuntime *profileRuntime,
                                                                     TZrBool recordHelpers);
static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_known_vm_with_argument_source_fast(
        SZrState *state,
        TZrStackValuePointer stackPointer,
        SZrTypeValue *callableValue,
        TZrSize argumentsCount,
...
static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_prepared_resolved_vm_fast(
        ...
        TZrSize resultCount,
        TZrStackValuePointer returnDestination);
static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_prepared_resolved_vm_with_argument_source_fast(
        SZrState *state,
        TZrStackValuePointer stackPointer,
        SZrFunction *resolvedFunction,
        TZrSize argumentsCount,
        ...
        TZrStackValuePointer argumentSourceFrameBase,
        TZrUInt32 argumentSourceStartSlot);
static ZR_FORCE_INLINE void execution_prepare_frame_staging_destination_no_profile(SZrState *state,
                                                                                   SZrTypeValue *destination);
...
static ZR_FORCE_INLINE void execution_prepare_stack_destination_for_overwrite_no_profile(SZrState *state,
                                                                                         SZrTypeValue *destination) {
...
static ZR_FORCE_INLINE void execution_copy_stack_value_to_stack_fast_no_profile(SZrState *state,
    ...
    execution_prepare_stack_destination_for_overwrite_no_profile(state, destination);
...
static ZR_FORCE_INLINE void execution_sanitize_non_owning_value_no_profile(SZrTypeValue *value) {
...
static ZR_FORCE_INLINE void execution_copy_frame_staging_value_no_profile(SZrState *state,
                                                                          SZrTypeValue *destination,
                                                                          const SZrTypeValue *source) {
    ...
    execution_sanitize_non_owning_value_no_profile(&sanitizedSource);
    execution_prepare_frame_staging_destination_no_profile(state, destination);
...
static ZR_FORCE_INLINE void execution_copy_frame_staging_value_to_fresh_slot_no_profile(
        SZrState *state,
        SZrTypeValue *destination,
        const SZrTypeValue *source) {
    SZrTypeValue sanitizedSource;
...
static ZR_FORCE_INLINE void execution_copy_stack_value_allowing_overlap_no_profile(SZrState *state,
                                                                                   SZrTypeValue *destination,
                                                                                   const SZrTypeValue *source) {
...
static ZR_FORCE_INLINE void execution_drop_call_info_inline_frame_values_if_available(SZrState *state,
    ...
    (void)ZrCore_Function_DropInlineFrameValues(state,
                                                function,
                                                frameBase,
...
static ZR_FORCE_INLINE SZrTypeValue *execution_get_call_return_destination_value_slot(
        SZrState *state,
        SZrCallInfo *callInfo,
    ...
    TZrMemoryOffset destinationStackSlot;
    const SZrFunctionFrameSlotLayout *slotLayout;
    SZrStackFramePlace place;

    if (state == ZR_NULL || callInfo == ZR_NULL || destination == ZR_NULL ||
        callInfo->previous == ZR_NULL) {
        return destination != ZR_NULL ? ZrCore_Stack_GetValueNoProfile(destination) : ZR_NULL;
    }

    callerCallInfo = callInfo->previous;
    callerFrameBase = callerCallInfo->functionBase.valuePointer != ZR_NULL
                              ? callerCallInfo->functionBase.valuePointer + 1
                              : ZR_NULL;
    callerFunction = ZrCore_Closure_GetMetadataFunctionFromCallInfo(state, callerCallInfo);
    if (callerFrameBase == ZR_NULL || callerFunction == ZR_NULL ||
        callerFunction->frameSlotLayouts == ZR_NULL ||
        callerFunction->frameSlotLayoutLength == 0u ||
        destination < callerFrameBase) {
        return ZrCore_Stack_GetValueNoProfile(destination);
    }

    destinationStackSlot = destination - callerFrameBase;
    if (destinationStackSlot < 0 || destinationStackSlot > UINT32_MAX) {
        return ZrCore_Stack_GetValueNoProfile(destination);
    }

    slotLayout = ZrCore_Function_FindFrameSlotLayout(callerFunction, (TZrUInt32)destinationStackSlot);
...
static ZR_FORCE_INLINE void execution_post_call_single_result_resolved_source_fast(
    ...
    TZrBool copiedInlineReturn = ZR_FALSE;
    TZrBool frameOverlapsStack = ZR_FALSE;
    ...
    destinationFrameValue = execution_get_call_return_destination_value_slot(state, callInfo, destination);
    frameOverlapsStack = (TZrBool)(destinationFrameValue != ZR_NULL &&
                                   destinationFrameValue != destinationValue &&
                                   ZrCore_Value_SlotsOverlapNoProfile(destinationFrameValue, destinationValue));
    (void)ZrCore_Function_TryCopyInlineReceiverParameterBack(state, callInfo);
    ZrCore_Function_TryCopyInlineConstructorReceiverBack(state, callInfo);

    if (returnSource != ZR_NULL) {
        copiedInlineReturn = ZrCore_Function_TryCopyInlineFrameReturnValue(
                state,
                callInfo,
                returnSource,
                destination,
    ...
    if (returnSource == ZR_NULL || returnSourceValue == ZR_NULL) {
        ZrCore_Value_ResetAsNullNoProfile(destinationValue);
    } else if (!copiedInlineReturn && returnSource != destination) {
        TZrBool copiedObjectInline = ZrCore_Function_TryCopyObjectReturnValueToInlineDestination(
                state, callInfo, destination, returnSourceValue);
        if (copiedObjectInline) {
        ...
        } else if (frameOverlapsStack && destinationFrameValue != ZR_NULL) {
    ...
    execution_drop_call_info_inline_frame_values_if_available(state, callInfo);
    if (returnSource == ZR_NULL || returnSourceValue == ZR_NULL) {
        if (destinationFrameValue != ZR_NULL && destinationFrameValue != destinationValue) {
            ZrCore_Value_ResetAsNullNoProfile(destinationFrameValue);
        }
...
static ZR_FORCE_INLINE TZrBool execution_frame_slot_is_inline_struct_destination(const SZrFunction *function,
                                                                                 TZrUInt32 stackSlot) {
    const SZrFunctionFrameSlotLayout *slotLayout;
...
static ZR_FORCE_INLINE TZrBool execution_store_result_to_inline_struct_destination_if_needed(
        SZrState *state,
        const SZrFunction *function,
        TZrStackValuePointer frameBase,
        TZrUInt32 stackSlot,
        const SZrTypeValue *source) {
    if (!execution_frame_slot_is_inline_struct_destination(function, stackSlot)) {
        return ZR_FALSE;
    }

    return ZrCore_Function_CopyObjectValueToFrameSlotInline(state, function, frameBase, stackSlot, source);
}

static ZR_FORCE_INLINE void execution_store_or_copy_result_to_frame_slot(
        SZrState *state,
        const SZrFunction *function,
        TZrStackValuePointer frameBase,
        TZrUInt32 stackSlot,
        ...
        SZrProfileRuntime *profileRuntime,
        TZrBool recordHelpers) {
    if (execution_store_result_to_inline_struct_destination_if_needed(state, function, frameBase, stackSlot, source)) {
        return;
    }

    execution_copy_value_fast(
            state,
            execution_inline_frame_get_value_slot(state, function, frameBase, stackSlot),
            source,
            profileRuntime,
            recordHelpers);
}

static ZR_FORCE_INLINE TZrBool execution_frame_value_slot_allows_plain_metadata_overwrite(
        SZrState *state,
        const SZrFunction *function,
        TZrStackValuePointer frameBase,
        SZrTypeValue *destination) {
    ...
    for (TZrUInt32 index = 0u; index < function->frameSlotLayoutLength; index++) {
        ...
        if ((SZrTypeValue *)place.address == destination) {
            return ZR_TRUE;
        }
    }

    for (TZrUInt32 stackSlot = 0u; stackSlot < function->stackSize; stackSlot++) {
        TZrStackValuePointer physicalSlot = frameBase + stackSlot;
        if (&physicalSlot->value != destination) {
            continue;
        }
        return (TZrBool)!ZrCore_Function_FrameStackSlotIntersectsInlineStruct(function, frameBase, physicalSlot);
    }
...
static ZR_FORCE_INLINE void execution_prepare_frame_staging_destination_no_profile(SZrState *state,
    ...
    /*
     * Frame-layout call staging overwrites dense stack mirrors that may overlap
     * previously used inline-frame byte storage. Such slots are not live
     * ownership roots for the new call window, so normalize stale metadata
     * before copying the staged value in.
     */
    ZrCore_Value_ResetAsNullNoProfile(destination);
}

static ZR_FORCE_INLINE void execution_push_closure_to_effective_destination_no_profile(
        SZrState *state,
        SZrFunction *function,
        SZrClosureValue **parentClosureValues,
        TZrStackValuePointer base,
        TZrStackValuePointer physicalDestination,
        SZrTypeValue *destination) {
    ...
    ZR_ASSERT(physicalDestination != ZR_NULL);
    ZR_ASSERT(destination != ZR_NULL);

    execution_prepare_destination_for_direct_store_no_profile(state, destination);
    ZrCore_Closure_PushToStack(state, function, parentClosureValues, base, physicalDestination);
    physicalValue = ZrCore_Stack_GetValueNoProfile(physicalDestination);
    physicalValue->type = ZR_VALUE_TYPE_CLOSURE;
    physicalValue->isGarbageCollectable = ZR_TRUE;
    physicalValue->isNative = ZR_FALSE;

    if (destination != physicalValue) {
        if (ZrCore_Value_SlotsOverlapNoProfile(destination, physicalValue)) {
            SZrTypeValue snapshot = *physicalValue;
            *destination = snapshot;
        } else {
            execution_copy_stack_value_to_stack_fast_no_profile(state, destination, physicalValue);
            execution_reset_stack_value_to_null_fast_no_profile(state, physicalValue);
        }
    }
}
...
static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_known_vm_with_argument_source_fast(
    ...
    if (argumentSourceFrameBase == ZR_NULL) {
    ...
    return ZrCore_Function_PreCallKnownVmValueWithArgumentSource(state,
                                                                 stackPointer,
                                                                 callableValue,
                                                                 resultCount,
                                                                 returnDestination,
...
static ZR_FORCE_INLINE TZrBool execution_call_window_value_is_callable(const SZrTypeValue *value) {
    return (TZrBool)(value != ZR_NULL &&
...
static ZR_FORCE_INLINE void execution_stage_frame_layout_call_values(SZrState *state,
    ...
    SZrTypeValue stackSnapshot[16];
    SZrTypeValue *snapshot = stackSnapshot;
    TZrSize valueCount;

    if (state == ZR_NULL || function == ZR_NULL || frameBase == ZR_NULL ||
        function->frameSlotLayouts == ZR_NULL || function->frameSlotLayoutLength == 0u) {
        return;
    }

    valueCount = parametersCount + 1u;
    if (valueCount > (sizeof(stackSnapshot) / sizeof(stackSnapshot[0]))) {
        snapshot = (SZrTypeValue *)malloc(sizeof(*snapshot) * valueCount);
        if (snapshot == ZR_NULL) {
            return;
        }
    }

    for (TZrSize offset = 0u; offset < valueCount; offset++) {
        TZrUInt32 logicalSlot = functionSlot + (TZrUInt32)offset;
        SZrTypeValue *physicalSource = ZrCore_Stack_GetValueNoProfile(frameBase + logicalSlot);
        ...
        if (offset == 0u && execution_call_window_value_is_callable(layoutSource)) {
            ...
            snapshot[offset] = *physicalSource;
        } else if (layoutSource != ZR_NULL && !ZR_VALUE_IS_TYPE_NULL(layoutSource->type)) {
            snapshot[offset] = *layoutSource;
        } else if (physicalSource != ZR_NULL && !ZR_VALUE_IS_TYPE_NULL(physicalSource->type)) {
            snapshot[offset] = *physicalSource;
        } else if (!execution_inline_frame_try_materialize_stack_slot_value(state,
                                                                            function,
                                                                            frameBase,
                                                                            logicalSlot,
                                                                            &snapshot[offset])) {
            if (layoutSource != ZR_NULL) {
                snapshot[offset] = *layoutSource;
            } else if (physicalSource != ZR_NULL) {
                snapshot[offset] = *physicalSource;
            }
        }
        execution_sanitize_non_owning_value_no_profile(&snapshot[offset]);
    }

    for (TZrSize offset = 0u; offset < valueCount; offset++) {
        TZrUInt32 logicalSlot = functionSlot + (TZrUInt32)offset;
        SZrTypeValue *destination = &frameBase[logicalSlot].value;
        const SZrFunctionFrameSlotLayout *slotLayout = ZrCore_Function_FindFrameSlotLayout(function, logicalSlot);
        ...
        execution_copy_frame_staging_value_no_profile(state, destination, &snapshot[offset]);
    }

    if (snapshot != stackSnapshot) {
        free(snapshot);
    }
}

static ZR_FORCE_INLINE TZrStackValuePointer execution_prepare_frame_layout_call_window_with_extra(
        SZrState *state,
        SZrCallInfo *callInfo,
        SZrFunction *function,
    ...
    if (state == ZR_NULL || callInfo == ZR_NULL || function == ZR_NULL ||
        ioFrameBase == ZR_NULL || *ioFrameBase == ZR_NULL) {
        return ZR_NULL;
    }

    frameBase = *ioFrameBase;
    valueCount = parametersCount + 1u;
    allocationCount = valueCount + extraSlotCount;
    if (allocationCount < valueCount) {
        return ZR_NULL;
    }
    if (valueCount > (sizeof(stackSnapshot) / sizeof(stackSnapshot[0]))) {
        snapshot = (SZrTypeValue *)malloc(sizeof(*snapshot) * valueCount);
    ...
    ZrCore_Function_StackAnchorInit(state, frameBase, &frameBaseAnchor);
    callWindowBase = ZrCore_Function_GetCallInfoFrameStorageTop(state, callInfo);
    if (callWindowBase == ZR_NULL || callWindowBase < state->stackTop.valuePointer) {
        callWindowBase = state->stackTop.valuePointer;
    }
    callWindow = ZrCore_Function_CheckStackAndGc(
            state,
            allocationCount,
            callWindowBase);
    frameBase = ZrCore_Function_StackAnchorRestore(state, &frameBaseAnchor);
    *ioFrameBase = frameBase;
    ...
    for (TZrSize offset = 0u; offset < valueCount; offset++) {
        execution_copy_frame_staging_value_to_fresh_slot_no_profile(state, &callWindow[offset].value, &snapshot[offset]);
    }
    for (TZrSize offset = valueCount; offset < allocationCount; offset++) {
        ZrCore_Value_ResetAsNullNoProfile(&callWindow[offset].value);
    }
...
static ZR_FORCE_INLINE TZrStackValuePointer execution_prepare_frame_layout_call_window(
        SZrState *state,
        SZrCallInfo *callInfo,
        SZrFunction *function,
        TZrStackValuePointer *ioFrameBase,
        TZrUInt32 functionSlot,
        TZrSize parametersCount) {
    return execution_prepare_frame_layout_call_window_with_extra(
            state,
            callInfo,
            function,
            ioFrameBase,
...
static ZR_FORCE_INLINE TZrStackValuePointer execution_prepare_tail_call_fallback_window(
    ...
    callWindow = ZrCore_Function_CheckStackAndGc(state, valueCount, callWindowBase);
    *ioFrameBase = ZrCore_Function_StackAnchorRestore(state, &frameBaseAnchor);
    sourceWindow = ZrCore_Function_StackAnchorRestore(state, &sourceWindowAnchor);
    if (callWindow == ZR_NULL || sourceWindow == ZR_NULL) {
        return ZR_NULL;
    }

    for (TZrSize offset = 0u; offset < valueCount; offset++) {
        execution_copy_frame_staging_value_to_fresh_slot_no_profile(
                state,
                &callWindow[offset].value,
                ZrCore_Stack_GetValueNoProfile(sourceWindow + offset));
    }

    state->stackTop.valuePointer = callWindow + valueCount;
    return callWindow;
}

static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_frame_layout_generic_single_result(
        SZrState *state,
        SZrCallInfo *callInfo,
        SZrFunction *function,
        TZrStackValuePointer *ioFrameBase,
    ...
    callWindow = execution_prepare_frame_layout_call_window(
            state,
            callInfo,
            function,
            ioFrameBase,
    ...
    *ioFrameBase = ZrCore_Function_StackAnchorRestore(state, &frameBaseAnchor);
    ...
    if (nextCallInfo == ZR_NULL && resultSlot != ZR_INSTRUCTION_USE_RET_FLAG) {
        const SZrTypeValue *resultSource =
                effectiveReturnDestination != ZR_NULL
                        ? execution_inline_frame_get_value_slot(state, function, *ioFrameBase, resultSlot)
                        : ZrCore_Stack_GetValueNoProfile(callWindow);
        execution_store_or_copy_result_to_frame_slot(state,
                                                     function,
                                                     *ioFrameBase,
                                                     resultSlot,
                                                     resultSource,
...
static ZR_FORCE_INLINE const SZrTypeValue *execution_effective_member_store_value(SZrState *state,
                                                                                  SZrFunction *function,
                                                                                  TZrStackValuePointer frameBase,
                                                                                  TZrUInt32 sourceSlot,
                                                                                  const SZrTypeValue *assignedValue,
                                                                                  SZrTypeValue *materializedValue) {
    if (execution_inline_frame_try_materialize_stack_slot_value(state,
                                                               function,
                                                               frameBase,
                                                               sourceSlot,
                                                               materializedValue)) {
...
static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_prepared_resolved_vm_with_argument_source_fast(
    ...
    return ZrCore_Function_PreCallPreparedResolvedVmFunctionWithArgumentSource(state,
                                                                               stackPointer,
                                                                               resolvedFunction,
                                                                               argumentsCount,
                                                                               resultCount,
...
static ZR_FORCE_INLINE SZrCallInfo *execution_pre_call_known_vm_member_fast(
    ...
    return execution_pre_call_known_vm_with_argument_source_fast(state,
                                                                stackPointer,
                                                                callableValue,
                                                                cachedArgumentCount,
                                                                resultCount,
...
static ZR_FORCE_INLINE SZrFunctionCallSiteCacheEntry *execution_member_try_get_single_slot_cache_entry_dispatch_fast(
        SZrFunction *function,
        TZrUInt16 cacheIndex,
        EZrFunctionCallSiteCacheKind expectedKind) {
    SZrFunctionCallSiteCacheEntry *entry;

    if (ZR_UNLIKELY(function == ZR_NULL ||
                    function->callSiteCaches == ZR_NULL ||
                    cacheIndex >= function->callSiteCacheLength ||
                    execution_callsite_sanitize_enabled())) {
        return ZR_NULL;
    }

    entry = &function->callSiteCaches[cacheIndex];
    if (ZR_UNLIKELY((EZrFunctionCallSiteCacheKind)entry->kind != expectedKind || entry->picSlotCount != 1u)) {
        return ZR_NULL;
    }
...
static ZR_FORCE_INLINE TZrBool execution_member_try_dispatch_exact_receiver_pair_get_from_entry_fast(
        SZrState *state,
        SZrFunctionCallSiteCacheEntry *entry,
...
static ZR_FORCE_INLINE TZrBool execution_member_try_dispatch_same_prototype_instance_field_get_from_entry_fast(
    ...
    pair = object_get_own_string_pair_by_name_cached_unchecked(state, receiverObject, memberName);
    if (pair == ZR_NULL) {
        return ZR_FALSE;
    }
...
static ZR_FORCE_INLINE TZrBool execution_member_try_dispatch_exact_receiver_pair_set_from_entry_fast(
        SZrState *state,
        SZrFunctionCallSiteCacheEntry *entry,
        const SZrTypeValue *receiver,
        const SZrTypeValue *assignedValue,
...
static TZrBool execution_try_reuse_preinstalled_top_level_closure(SZrState *state,
    ...
    for (index = 0; index < ownerClosure->function->exportedVariableLength; ++index) {
        ...
        existingValue =
                execution_inline_frame_get_value_slot(state, ownerClosure->function, base, binding->stackSlot);
...
void ZrCore_Execute(SZrState *state, SZrCallInfo *callInfo) {
    ...
    static void *const fastDispatchTable[ZR_INSTRUCTION_ENUM(ENUM_MAX)] = {
            ...
            [ZR_INSTRUCTION_ENUM(RESET_STACK_NULL2)] = &&LZrFastInstruction_RESET_STACK_NULL2,
            [ZR_INSTRUCTION_ENUM(ADD)] = &&LZrFastInstruction_ADD,
            ...
            [ZR_INSTRUCTION_ENUM(ADD_SIGNED_MOD_CONST)] = &&LZrFastInstruction_ADD_SIGNED_MOD_CONST,
            ...
            [ZR_INSTRUCTION_ENUM(MUL)] = &&LZrFastInstruction_MUL,
            ...
            [ZR_INSTRUCTION_ENUM(MUL_SIGNED_LOAD_STACK)] = &&LZrFastInstruction_MUL_SIGNED_LOAD_STACK,
            ...
            [ZR_INSTRUCTION_ENUM(TO_FLOAT_SIGNED)] = &&LZrFastInstruction_TO_FLOAT_SIGNED,
            [ZR_INSTRUCTION_ENUM(TO_FLOAT_UNSIGNED)] = &&LZrFastInstruction_TO_FLOAT_UNSIGNED,
            [ZR_INSTRUCTION_ENUM(TO_INT_FLOAT)] = &&LZrFastInstruction_TO_INT_FLOAT,
            [ZR_INSTRUCTION_ENUM(TO_INT_UNSIGNED)] = &&LZrFastInstruction_TO_INT_UNSIGNED,
            [ZR_INSTRUCTION_ENUM(TO_UINT_FLOAT)] = &&LZrFastInstruction_TO_UINT_FLOAT,
            [ZR_INSTRUCTION_ENUM(TO_UINT_SIGNED)] = &&LZrFastInstruction_TO_UINT_SIGNED,
            [ZR_INSTRUCTION_ENUM(NEG_SIGNED)] = &&LZrFastInstruction_NEG_SIGNED,
            [ZR_INSTRUCTION_ENUM(NEG_FLOAT)] = &&LZrFastInstruction_NEG_FLOAT,
            ...
            [ZR_INSTRUCTION_ENUM(MOD)] = &&LZrFastInstruction_MOD,
            ...
            [ZR_INSTRUCTION_ENUM(LOGICAL_NOT_BOOL)] = &&LZrFastInstruction_LOGICAL_NOT_BOOL,
            ...
            [ZR_INSTRUCTION_ENUM(BITWISE_XOR)] = &&LZrFastInstruction_BITWISE_XOR,
            ...
            [ZR_INSTRUCTION_ENUM(SUPER_ARRAY_ADD_INT)] = &&LZrFastInstruction_SUPER_ARRAY_ADD_INT,
            [ZR_INSTRUCTION_ENUM(SUPER_ARRAY_ADD_INT4)] = &&LZrFastInstruction_SUPER_ARRAY_ADD_INT4,
            [ZR_INSTRUCTION_ENUM(SUPER_ARRAY_ADD_INT4_CONST)] = &&LZrFastInstruction_SUPER_ARRAY_ADD_INT4_CONST,
            [ZR_INSTRUCTION_ENUM(SUPER_ARRAY_FILL_INT4_CONST)] = &&LZrFastInstruction_SUPER_ARRAY_FILL_INT4_CONST,
            [ZR_INSTRUCTION_ENUM(ITER_INIT)] = &&LZrFastInstruction_ITER_INIT,
            [ZR_INSTRUCTION_ENUM(ITER_MOVE_NEXT)] = &&LZrFastInstruction_ITER_MOVE_NEXT,
            [ZR_INSTRUCTION_ENUM(DYN_ITER_INIT)] = &&LZrFastInstruction_DYN_ITER_INIT,
            [ZR_INSTRUCTION_ENUM(DYN_ITER_MOVE_NEXT)] = &&LZrFastInstruction_DYN_ITER_MOVE_NEXT,
            [ZR_INSTRUCTION_ENUM(ITER_CURRENT)] = &&LZrFastInstruction_ITER_CURRENT,
            ...
            [ZR_INSTRUCTION_ENUM(JUMP_IF_BOOL_FALSE)] = &&LZrFastInstruction_JUMP_IF_BOOL_FALSE,
            [ZR_INSTRUCTION_ENUM(JUMP_IF_GREATER_SIGNED)] = &&LZrFastInstruction_JUMP_IF_GREATER_SIGNED,
            [ZR_INSTRUCTION_ENUM(JUMP_IF_LESS_EQUAL_SIGNED)] = &&LZrFastInstruction_JUMP_IF_LESS_EQUAL_SIGNED,
            ...
            [ZR_INSTRUCTION_ENUM(JUMP_IF_NULL)] = &&LZrFastInstruction_JUMP_IF_NULL,
            [ZR_INSTRUCTION_ENUM(FUNCTION_CALL)] = &&LZrFastInstruction_FUNCTION_CALL,
            [ZR_INSTRUCTION_ENUM(KNOWN_VM_CALL)] = &&LZrFastInstruction_KNOWN_VM_CALL,
            [ZR_INSTRUCTION_ENUM(KNOWN_VM_MEMBER_CALL)] = &&LZrFastInstruction_KNOWN_VM_MEMBER_CALL,
            [ZR_INSTRUCTION_ENUM(KNOWN_VM_MEMBER_CALL_LOAD1_U8)] =
                    &&LZrFastInstruction_KNOWN_VM_MEMBER_CALL_LOAD1_U8,
            [ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_CALL)] = &&LZrFastInstruction_KNOWN_NATIVE_CALL,
            [ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_MEMBER_CALL)] =
                    &&LZrFastInstruction_KNOWN_NATIVE_MEMBER_CALL,
            [ZR_INSTRUCTION_ENUM(KNOWN_NATIVE_MEMBER_CALL_RECV_U8)] =
                    &&LZrFastInstruction_KNOWN_NATIVE_MEMBER_CALL_RECV_U8,
            [ZR_INSTRUCTION_ENUM(SUPER_FUNCTION_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_FUNCTION_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_KNOWN_VM_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_KNOWN_VM_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_KNOWN_NATIVE_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_KNOWN_NATIVE_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_DYN_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_DYN_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_DYN_CALL_CACHED)] =
                    &&LZrFastInstruction_SUPER_DYN_CALL_CACHED,
            [ZR_INSTRUCTION_ENUM(DYN_CALL)] = &&LZrFastInstruction_DYN_CALL,
            [ZR_INSTRUCTION_ENUM(SUPER_META_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_META_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_META_CALL_CACHED)] =
                    &&LZrFastInstruction_SUPER_META_CALL_CACHED,
            [ZR_INSTRUCTION_ENUM(META_CALL)] = &&LZrFastInstruction_META_CALL,
            ...
            [ZR_INSTRUCTION_ENUM(SUPER_FUNCTION_TAIL_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_FUNCTION_TAIL_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_KNOWN_VM_TAIL_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_KNOWN_VM_TAIL_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_KNOWN_NATIVE_TAIL_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_KNOWN_NATIVE_TAIL_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_DYN_TAIL_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_DYN_TAIL_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_DYN_TAIL_CALL_CACHED)] =
                    &&LZrFastInstruction_SUPER_DYN_TAIL_CALL_CACHED,
            [ZR_INSTRUCTION_ENUM(DYN_TAIL_CALL)] = &&LZrFastInstruction_DYN_TAIL_CALL,
            [ZR_INSTRUCTION_ENUM(SUPER_META_TAIL_CALL_NO_ARGS)] =
                    &&LZrFastInstruction_SUPER_META_TAIL_CALL_NO_ARGS,
            [ZR_INSTRUCTION_ENUM(SUPER_META_TAIL_CALL_CACHED)] =
                    &&LZrFastInstruction_SUPER_META_TAIL_CALL_CACHED,
            [ZR_INSTRUCTION_ENUM(META_TAIL_CALL)] = &&LZrFastInstruction_META_TAIL_CALL,
            [ZR_INSTRUCTION_ENUM(FUNCTION_RETURN)] = &&LZrFastInstruction_FUNCTION_RETURN,
    };
...
#define FETCH_DEBUG_BASE_SYNC()                                                                                         \
    do {                                                                                                               \
        if (ZR_UNLIKELY(base != callInfo->functionBase.valuePointer + 1)) {                                            \
...
#define FAST_PREPARE_DESTINATION_FROM_OFFSET(DESTINATION_OFFSET)                                                        \
    do {                                                                                                               \
        ...
        if (ZR_UNLIKELY(fastDestinationOffset__ == ZR_INSTRUCTION_USE_RET_FLAG)) {                                     \
            ...
            destination = FRAME_VALUE_SLOT(fastDestinationOffset__);                                                   \
        }                                                                                                              \
    } while (0)
#define FAST_PREPARE_DESTINATION() FAST_PREPARE_DESTINATION_FROM_OFFSET(E(instruction))
#define FAST_PREPARE_SIGNED_DESTINATION_FROM_OFFSET(DESTINATION_OFFSET)                                                 \
    do {                                                                                                               \
        TZrUInt16 fastDestinationOffset__ = (DESTINATION_OFFSET);                                                      \
        if (ZR_UNLIKELY(fastDestinationOffset__ == ZR_INSTRUCTION_USE_RET_FLAG)) {                                     \
            destination = &ret;                                                                                         \
        } else {                                                                                                       \
            destination = SIGNED_FRAME_VALUE_SLOT(fastDestinationOffset__);                                            \
        }                                                                                                              \
    } while (0)
#define FAST_PREPARE_SIGNED_DESTINATION() FAST_PREPARE_SIGNED_DESTINATION_FROM_OFFSET(E(instruction))
...
#define EXECUTION_STORE_PLAIN_DIRECT_TO(DESTINATION_VALUE, REGION, DATA, TYPE)                                         \
    do {                                                                                                               \
        ...
        TZrBool executionPlainDestinationHasDirtyMetadata__ =                                                           \
                (TZrBool)(executionPlainDestination__->ownershipKind != ZR_OWNERSHIP_VALUE_KIND_NONE ||                \
                          executionPlainDestination__->ownershipControl != ZR_NULL ||                                  \
                          executionPlainDestination__->ownershipWeakRef != ZR_NULL ||                                  \
                          executionPlainDestination__->isGarbageCollectable ||                                         \
                          !executionPlainDestination__->isNative);                                                     \
        TZrBool executionPlainDestinationAllowsMetadata__ = ZR_FALSE;                                                   \
        if (executionPlainDestinationHasDirtyMetadata__) {                                                             \
            executionPlainDestinationAllowsMetadata__ =                                                                 \
                    execution_frame_value_slot_allows_plain_metadata_overwrite(                                        \
                            state, currentFunction, base, executionPlainDestination__);                                \
        }                                                                                                              \
        if (executionPlainDestinationAllowsMetadata__) {                                                               \
            execution_prepare_stack_destination_for_overwrite_no_profile(state, executionPlainDestination__);           \
        } else {                                                                                                       \
        ...
        if (executionPlainDestinationAllowsMetadata__) {                                                               \
            executionPlainDestination__->isGarbageCollectable = ZR_FALSE;                                              \
            executionPlainDestination__->isNative = ZR_TRUE;                                                           \
            executionPlainDestination__->ownershipKind = ZR_OWNERSHIP_VALUE_KIND_NONE;                                 \
            executionPlainDestination__->ownershipControl = ZR_NULL;                                                   \
            executionPlainDestination__->ownershipWeakRef = ZR_NULL;                                                   \
        }                                                                                                              \
    } while (0)
...
#define EXECUTION_INTEGRAL_VALUE_AS_U64(VALUE)                                                                         \
    (ZR_VALUE_IS_TYPE_UNSIGNED_INT((VALUE)->type) ? (VALUE)->value.nativeObject.nativeUInt64                          \
                                                  : (TZrUInt64)(VALUE)->value.nativeObject.nativeInt64)
#define EXECUTE_BITWISE_NOT_BODY()                                                                                     \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_INT(opA->type));                                                                    \
        if (ZR_VALUE_IS_TYPE_UNSIGNED_INT(opA->type)) {                                                                \
            ALGORITHM_1(nativeUInt64, ~, ZR_VALUE_TYPE_UINT64);                                                        \
        } else {                                                                                                       \
            ALGORITHM_1(nativeInt64, ~, ZR_VALUE_TYPE_INT64);                                                          \
        }                                                                                                              \
    } while (0)
#define EXECUTE_BITWISE_BINARY_BODY(OP)                                                                                \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ...
        if (ZR_VALUE_IS_TYPE_UNSIGNED_INT(opA->type) || ZR_VALUE_IS_TYPE_UNSIGNED_INT(opB->type)) {                  \
            EXECUTION_STORE_PLAIN_REUSE(                                                                               \
                    nativeUInt64,                                                                                      \
                    EXECUTION_INTEGRAL_VALUE_AS_U64(opA) OP EXECUTION_INTEGRAL_VALUE_AS_U64(opB),                    \
                    ZR_VALUE_TYPE_UINT64);                                                                             \
        } else {                                                                                                       \
            ALGORITHM_2(nativeInt64, OP, ZR_VALUE_TYPE_INT64);                                                         \
        }                                                                                                              \
    } while (0)
...
#define EXECUTE_GET_MEMBER_SLOT_BODY()                                                                                 \
    do {                                                                                                               \
        ...
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        if (opA != destination && execution_inline_frame_try_get_member_to_slot(                                        \
                    state, currentFunction, base, A1(instruction), B1(instruction), E(instruction), destination)) {    \
            break;                                                                                                     \
        }                                                                                                              \
        entry__ = (opA->type == ZR_VALUE_TYPE_OBJECT || opA->type == ZR_VALUE_TYPE_ARRAY)                            \
                          ? execution_member_try_get_single_slot_cache_entry_dispatch_fast(                            \
        ...
        if (opA->type != ZR_VALUE_TYPE_OBJECT && opA->type != ZR_VALUE_TYPE_ARRAY &&                                  \
            ...
            ZrCore_Debug_RunError(state, "GET_MEMBER: receiver must be an object, array, or string");                \
        } else if (!(((opA != destination) &&                                                                           \
                      (opA->type == ZR_VALUE_TYPE_OBJECT || opA->type == ZR_VALUE_TYPE_ARRAY) &&                      \
                      execution_member_try_dispatch_exact_receiver_pair_get_hot_fast_checked_object_profiled(          \
                              state,                                                                                  \
                              currentFunction,                                                                        \
                              B1(instruction),                                                                        \
                              opA,                                                                                    \
                     ...
                     ((opA != destination) &&                                                                           \
                      (opA->type == ZR_VALUE_TYPE_OBJECT || opA->type == ZR_VALUE_TYPE_ARRAY) &&                      \
                      execution_member_try_dispatch_same_prototype_hot_field_get_fast_checked_object(                  \
                              state,                                                                                  \
                              currentFunction,                                                                        \
                              B1(instruction),                                                                        \
                              opA,                                                                                    \
                              ...
                              profileRuntime,                                                                         \
                              recordHelpers)) ||                                                                      \
                     ((opA != destination) && entry__ != ZR_NULL &&                                                    \
...
#define EXECUTE_GET_MEMBER_SLOT_BODY_FAST()                                                                            \
    do {                                                                                                               \
        SZrFunctionCallSiteCacheEntry *entry__;                                                                        \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        if (opA != destination && execution_inline_frame_try_get_member_to_slot(                                        \
        ...
        entry__ = (opA->type == ZR_VALUE_TYPE_OBJECT || opA->type == ZR_VALUE_TYPE_ARRAY)                            \
                          ? execution_member_try_get_single_slot_cache_entry_dispatch_fast(                            \
                                    currentFunction, B1(instruction), ZR_FUNCTION_CALLSITE_CACHE_KIND_MEMBER_GET)     \
                          : ZR_NULL;                                                                                  \
        if (ZR_UNLIKELY(!(opA != destination && entry__ != ZR_NULL &&                                                  \
...
#define EXECUTE_SET_MEMBER_SLOT_BODY()                                                                                 \
    do {                                                                                                               \
        ...
        if (opA->type != ZR_VALUE_TYPE_OBJECT && opA->type != ZR_VALUE_TYPE_ARRAY) {                                  \
            ZrCore_Debug_RunError(state, "SET_MEMBER: receiver must be a writable object member");                   \
        } else if (!(execution_member_try_dispatch_exact_receiver_pair_set_hot_fast_checked_object_profiled(          \
                             state,                                                                                   \
                             currentFunction,                                                                         \
                             B1(instruction),                                                                         \
                             opA,                                                                                     \
                             ...
                             profileRuntime,                                                                          \
                             recordHelpers) ||                                                                        \
                     execution_member_try_dispatch_same_prototype_hot_field_set_fast_checked_object(                  \
                             state,                                                                                   \
                             currentFunction,                                                                         \
                             B1(instruction),                                                                         \
                             opA,                                                                                     \
                     ...
                     (entry__ != ZR_NULL &&                                                                            \
                      execution_member_try_dispatch_same_prototype_instance_field_set_from_entry_fast(                 \
                              state, currentFunction, entry__, opA, effectiveAssignedValue__, profileRuntime, recordHelpers)) || \
                     execution_member_set_cached_stack_receiver(                                                       \
                             state, programCounter, currentFunction, B1(instruction), opA, effectiveAssignedValue__))) { \
    ...
    do {                                                                                                               \
        ...
        entry__ = (opA->type == ZR_VALUE_TYPE_OBJECT || opA->type == ZR_VALUE_TYPE_ARRAY)                            \
                          ? execution_member_try_get_single_slot_cache_entry_dispatch_fast(                            \
                                    currentFunction, B1(instruction), ZR_FUNCTION_CALLSITE_CACHE_KIND_MEMBER_SET)     \
                          : ZR_NULL;                                                                                  \
        if (ZR_UNLIKELY(!(entry__ != ZR_NULL &&                                                                       \
                          (execution_member_try_dispatch_exact_receiver_pair_set_from_entry_fast(                      \
                                   state, entry__, opA, effectiveAssignedValue__, profileRuntime, recordHelpers) ||   \
...
#define EXECUTE_SET_MEMBER_SLOT_NULL_BODY()                                                                            \
    do {                                                                                                               \
        ...
        entry__ = (opA->type == ZR_VALUE_TYPE_OBJECT || opA->type == ZR_VALUE_TYPE_ARRAY)                            \
                          ...
                          : ZR_NULL;                                                                                  \
        if (opA->type != ZR_VALUE_TYPE_OBJECT && opA->type != ZR_VALUE_TYPE_ARRAY) {                                  \
            ZrCore_Debug_RunError(state, "SET_MEMBER_SLOT_NULL_A: receiver must be a writable object member");       \
        } else if (!(execution_member_try_dispatch_exact_receiver_pair_set_hot_fast_checked_object_profiled(          \
                             state,                                                                                   \
                             currentFunction,                                                                         \
                             B1(instruction),                                                                         \
                     ...
                     (entry__ != ZR_NULL &&                                                                            \
                      execution_member_try_dispatch_same_prototype_instance_field_set_from_entry_fast(                 \
                              state, currentFunction, entry__, opA, &nullValue__, profileRuntime, recordHelpers)) ||  \
                     execution_member_set_cached_stack_receiver(                                                       \
                             state, programCounter, currentFunction, B1(instruction), opA, &nullValue__))) {          \
            if (execution_resolve_cached_member_symbol(                                                                \
                        currentFunction, B1(instruction), ZR_FUNCTION_CALLSITE_CACHE_KIND_MEMBER_SET) == ZR_NULL) {   \
                ZrCore_Debug_RunError(state, "SET_MEMBER_SLOT_NULL: cached member store failed");                    \
            } else {                                                                                                   \
                ZrCore_Debug_RunError(state, "SET_MEMBER_SLOT_NULL_B: receiver must be a writable object member");   \
            }                                                                                                          \
        }                                                                                                              \
    } while (0)
#define EXECUTE_SET_MEMBER_SLOT_NULL_BODY_FAST()                                                                       \
    do {                                                                                                               \
        ...
        if (ZR_UNLIKELY(!(entry__ != ZR_NULL &&                                                                       \
                          (execution_member_try_dispatch_exact_receiver_pair_set_from_entry_fast(                      \
                                   state, entry__, opA, &nullValue__, profileRuntime, recordHelpers) ||               \
                           execution_member_try_dispatch_same_prototype_hot_field_set_from_entry_fast(                 \
                                   state, currentFunction, entry__, opA, &nullValue__, profileRuntime, recordHelpers) || \
                           execution_member_try_dispatch_same_prototype_instance_field_set_from_entry_fast(            \
                                   state, currentFunction, entry__, opA, &nullValue__, profileRuntime, recordHelpers))))) { \
...
#define EXECUTE_GET_BY_INDEX_BODY()                                                                                    \
    do {                                                                                                               \
        SZrTypeValue stableResult__;                                                                                   \
        TZrBool resolved__ = ZR_FALSE;                                                                                 \
        TZrBool readonlyFastTried__ = ZR_FALSE;                                                                        \
        TZrBool directResultStored__ = ZR_FALSE;                                                                       \
        TZrBool inlineStructDestination__ =                                                                            \
                (TZrBool)(E(instruction) != ZR_INSTRUCTION_USE_RET_FLAG &&                                            \
                          execution_frame_slot_is_inline_struct_destination(currentFunction, E(instruction)));         \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        opB = FRAME_VALUE_SLOT(B1(instruction));                                                                       \
        if (opA->type != ZR_VALUE_TYPE_OBJECT && opA->type != ZR_VALUE_TYPE_ARRAY) {                                  \
            ...
            if (!inlineStructDestination__ && E(instruction) != ZR_INSTRUCTION_USE_RET_FLAG &&                       \
                destination != opA && destination != opB) {                                                           \
                readonlyFastTried__ = ZR_TRUE;                                                                        \
                execution_prepare_destination_for_direct_store_no_profile(state, destination);                         \
                ZrCore_Value_ResetAsNullNoProfile(destination);                                                        \
                resolved__ = ZrCore_Object_TryGetByIndexReadonlyInlineFastStackOperands(                              \
                        state, opA, opB, destination);                                                                \
                if (resolved__) {                                                                                      \
                    if (ZR_UNLIKELY(recordHelpers)) {                                                                  \
                        profileRuntime->helperCounts[ZR_PROFILE_HELPER_GET_BY_INDEX]++;                               \
                    }                                                                                                  \
                    directResultStored__ = ZR_TRUE;                                                                    \
            ...
            if (!directResultStored__) {                                                                               \
                ZrCore_Value_ResetAsNull(&stableResult__);                                                             \
                if (!readonlyFastTried__) {                                                                            \
                    resolved__ = ZrCore_Object_TryGetByIndexReadonlyInlineFastStackOperands(                          \
                            state, opA, opB, &stableResult__);                                                        \
                }                                                                                                      \
            }                                                                                                          \
            if (!directResultStored__ && resolved__) {                                                                 \
                if (ZR_UNLIKELY(recordHelpers)) {                                                                      \
                    profileRuntime->helperCounts[ZR_PROFILE_HELPER_GET_BY_INDEX]++;                                   \
                }                                                                                                      \
                UPDATE_BASE(callInfo);                                                                                 \
                destination = E(instruction) == ZR_INSTRUCTION_USE_RET_FLAG ? &ret : FRAME_VALUE_SLOT(E(instruction)); \
            } else if (!directResultStored__ && state->threadStatus != ZR_THREAD_STATUS_FINE) {                       \
                SAVE_PC(state, callInfo);                                                                              \
                goto LZrReturning;                                                                                     \
            } else if (!directResultStored__ && !resolved__) {                                                         \
                PROTECT_E(state, callInfo, {                                                                           \
                    resolved__ = ZrCore_Object_GetByIndexUncheckedStackOperands(                                      \
                            state, opA, opB, &stableResult__);                                                        \
                });                                                                                                    \
            ...
            if (!directResultStored__) {                                                                               \
                if (resolved__) {                                                                                      \
                    if (!execution_store_result_to_inline_struct_destination_if_needed(                                \
                                state, currentFunction, base, E(instruction), &stableResult__)) {                     \
                        execution_copy_value_fast(state, destination, &stableResult__, profileRuntime, recordHelpers); \
                    }                                                                                                  \
                } else {                                                                                               \
                    ZrCore_Debug_RunError(state, "GET_BY_INDEX: receiver must be an object or array");               \
                }                                                                                                      \
...
#define EXECUTE_SET_BY_INDEX_BODY()                                                                                    \
    do {                                                                                                               \
        ...
        if (opA->type != ZR_VALUE_TYPE_OBJECT && opA->type != ZR_VALUE_TYPE_ARRAY) {                                  \
            ...
            resolved__ = ZrCore_Object_TrySetByIndexReadonlyInlineFastStackOperands(                                  \
                    state, opA, opB, destination);                                                                     \
            if (resolved__) {                                                                                          \
                if (ZR_UNLIKELY(recordHelpers)) {                                                                      \
                ...
                PROTECT_E(state, callInfo, {                                                                           \
                    resolved__ = ZrCore_Object_SetByIndexUncheckedStackOperands(state, opA, opB, destination);         \
                });                                                                                                    \
                RELOAD_DESTINATION_AFTER_PROTECT(callInfo, instruction);                                              \
            }                                                                                                          \
            if (!resolved__) {                                                                                         \
...
#define EXECUTE_ITER_INIT_BODY(ERROR_MESSAGE__)                                                                        \
    do {                                                                                                               \
        SZrTypeValue stableReceiver__;                                                                                 \
        SZrTypeValue stableResult__;                                                                                   \
        TZrBool resolved__ = ZR_FALSE;                                                                                 \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        stableReceiver__ = *opA;                                                                                       \
        ZrCore_Value_ResetAsNull(&stableResult__);                                                                     \
        PROTECT_E(state, callInfo, {                                                                                   \
            resolved__ = ZrCore_Object_IterInit(state, &stableReceiver__, &stableResult__);                            \
        });                                                                                                            \
        RELOAD_DESTINATION_AFTER_PROTECT(callInfo, instruction);                                                       \
        if (resolved__) {                                                                                              \
            ZrCore_Value_Copy(state, destination, &stableResult__);                                                    \
        } else {                                                                                                       \
            ZrCore_Debug_RunError(state, ERROR_MESSAGE__);                                                             \
        }                                                                                                              \
    } while (0)
#define EXECUTE_ITER_MOVE_NEXT_BODY(ERROR_MESSAGE__, DONE_ON_FAST__)                                                   \
    do {                                                                                                               \
        SZrTypeValue stableReceiver__;                                                                                 \
        SZrTypeValue stableResult__;                                                                                   \
        TZrBool resolved__ = ZR_FALSE;                                                                                 \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        if (ZrCore_Object_TryIterMoveNextCachedArrayFast(state, opA, destination)) {                                  \
            DONE_ON_FAST__(1);                                                                                         \
        }                                                                                                              \
        stableReceiver__ = *opA;                                                                                       \
        ZrCore_Value_ResetAsNull(&stableResult__);                                                                     \
        PROTECT_E(state, callInfo, {                                                                                   \
            resolved__ = ZrCore_Object_IterMoveNext(state, &stableReceiver__, &stableResult__);                        \
        });                                                                                                            \
        RELOAD_DESTINATION_AFTER_PROTECT(callInfo, instruction);                                                       \
        if (resolved__) {                                                                                              \
            ZrCore_Value_Copy(state, destination, &stableResult__);                                                    \
...
#define EXECUTE_ITER_CURRENT_BODY(DONE_ON_FAST__)                                                                      \
    do {                                                                                                               \
        SZrTypeValue stableReceiver__;                                                                                 \
        SZrTypeValue stableResult__;                                                                                   \
        TZrBool resolved__ = ZR_FALSE;                                                                                 \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        if (ZrCore_Object_TryIterCurrentCachedMemberFastStackResult(state, opA, destination)) {                       \
            DONE_ON_FAST__(1);                                                                                         \
        }                                                                                                              \
        stableReceiver__ = *opA;                                                                                       \
        ZrCore_Value_ResetAsNull(&stableResult__);                                                                     \
        PROTECT_E(state, callInfo, {                                                                                   \
            resolved__ = ZrCore_Object_IterCurrent(state, &stableReceiver__, &stableResult__);                         \
        });                                                                                                            \
        RELOAD_DESTINATION_AFTER_PROTECT(callInfo, instruction);                                                       \
        if (resolved__) {                                                                                              \
            ZrCore_Value_Copy(state, destination, &stableResult__);                                                    \
...
#define EXECUTE_KNOWN_VM_MEMBER_CALL_BODY()                                                                            \
    do {                                                                                                               \
        TZrSize resultSlot__ = E(instruction);                                                                         \
        TZrUInt16 cacheIndex__ = (TZrUInt16)A1(instruction);                                                           \
        SZrFunctionCallSiteCacheEntry *cacheEntry__;                                                                   \
        TZrSize parametersCount__ = 0u;                                                                                \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        TZrUInt32 argumentSourceStartSlot__;                                                                           \
        SZrCallInfo *nextCallInfo__;                                                                                   \
                                                                                                                       \
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        cacheEntry__ = execution_member_get_cache_entry_dispatch_fast(                                                 \
                currentFunction, cacheIndex__, ZR_FUNCTION_CALLSITE_CACHE_KIND_MEMBER_GET);                           \
        parametersCount__ = cacheEntry__ != ZR_NULL ? cacheEntry__->argumentCount : 0u;                               \
        callWindow__ = execution_prepare_frame_layout_call_window(                                                     \
                state, callInfo, currentFunction, &base, (TZrUInt32)resultSlot__, parametersCount__);                 \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "KNOWN_VM_MEMBER_CALL: failed to prepare call frame");                      \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + parametersCount__ + 1;                                          \
        argumentSourceStartSlot__ = (TZrUInt32)(resultSlot__ + 1u);                                                    \
        nextCallInfo__ = execution_pre_call_known_vm_member_fast(state,                                                \
                                                                 programCounter,                                      \
                                                                 currentFunction,                                     \
                                                                 cacheIndex__,                                        \
                                                                 cacheEntry__,                                        \
                                                                 callWindow__,                                        \
                                                                 expectedReturnCount__,                               \
                                                                 BASE(resultSlot__),                                  \
                                                                 base,                                                \
...
#define EXECUTE_KNOWN_VM_MEMBER_CALL_LOAD1_U8_BODY()                                                                   \
    do {                                                                                                               \
        TZrSize resultSlot__ = E(instruction);                                                                         \
        TZrUInt16 cacheIndex__ = (TZrUInt16)instruction.instruction.operand.operand0[0];                              \
        TZrUInt16 receiverSourceSlot__ = (TZrUInt16)instruction.instruction.operand.operand0[1];                      \
        TZrUInt16 argumentSourceSlot__ = (TZrUInt16)instruction.instruction.operand.operand0[2];                      \
        SZrFunctionCallSiteCacheEntry *cacheEntry__;                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        TZrUInt32 argumentSourceStartSlot__;                                                                           \
        SZrCallInfo *nextCallInfo__;                                                                                   \
                                                                                                                       \
        execution_copy_stack_value_to_stack_fast_no_profile(                                                           \
                state, FRAME_VALUE_SLOT(resultSlot__ + 1u), FRAME_VALUE_SLOT(receiverSourceSlot__));                  \
        execution_copy_stack_value_to_stack_fast_no_profile(                                                           \
                state, FRAME_VALUE_SLOT(resultSlot__ + 2u), FRAME_VALUE_SLOT(argumentSourceSlot__));                  \
        if (ZR_UNLIKELY(recordHelpers)) {                                                                              \
        ...
        cacheEntry__ = execution_member_get_cache_entry_dispatch_fast(                                                 \
                currentFunction, cacheIndex__, ZR_FUNCTION_CALLSITE_CACHE_KIND_MEMBER_GET);                           \
        callWindow__ = execution_prepare_frame_layout_call_window(                                                     \
                state, callInfo, currentFunction, &base, (TZrUInt32)resultSlot__, 2u);                                \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "KNOWN_VM_MEMBER_CALL_LOAD1_U8: failed to prepare call frame");             \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + 3u;                                                             \
        argumentSourceStartSlot__ = (TZrUInt32)(resultSlot__ + 1u);                                                    \
        nextCallInfo__ = execution_pre_call_known_vm_member_fast(state,                                                \
                                                                 programCounter,                                      \
                                                                 currentFunction,                                     \
...
#define EXECUTE_KNOWN_VM_CALL_BODY()                                                                                   \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrFunction *resolvedFunction__;                                                                               \
        SZrCallInfo *nextCallInfo__;                                                                                   \
                                                                                                                       \
        ...
        if (resolvedFunction__ == ZR_NULL) {                                                                           \
            resolvedFunction__ = execution_try_resolve_vm_metadata_function_fast(state, opA, ZR_NULL);                 \
            if (opA->type == ZR_VALUE_TYPE_FUNCTION &&                                                                 \
                (resolvedFunction__ == ZR_NULL || resolvedFunction__->closureValueLength != 0)) {                     \
                resolvedFunction__ = ZR_NULL;                                                                         \
            }                                                                                                          \
        }                                                                                                              \
        if (resolvedFunction__ == ZR_NULL) {                                                                           \
        ...
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "KNOWN_VM_CALL: failed to prepare call frame");                             \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + parametersCount__ + 1;                                          \
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        nextCallInfo__ = execution_pre_call_prepared_resolved_vm_with_argument_source_fast(                           \
                state,                                                                                                \
                callWindow__,                                                                                         \
                resolvedFunction__,                                                                                   \
                parametersCount__,                                                                                    \
...
#define EXECUTE_KNOWN_NATIVE_CALL_BODY()                                                                               \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrFunctionStackAnchor callWindowAnchor__;                                                                     \
                                                                                                                       \
        opA = FRAME_VALUE_SLOT(functionSlot__);                                                                        \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window(                                                     \
                state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, parametersCount__);               \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "KNOWN_NATIVE_CALL: failed to prepare call frame");                         \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + parametersCount__ + 1;                                          \
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        ...
        callWindow__ = ZrCore_Function_StackAnchorRestore(state, &callWindowAnchor__);                                \
        execution_store_or_copy_result_to_frame_slot(state,                                                            \
                                                      currentFunction,                                                  \
                                                      base,                                                             \
                                                      E(instruction),                                                   \
...
#define EXECUTE_KNOWN_NATIVE_MEMBER_CALL_BODY()                                                                        \
    do {                                                                                                               \
        TZrSize resultSlot__ = E(instruction);                                                                         \
        TZrUInt16 cacheIndex__ = (TZrUInt16)A1(instruction);                                                           \
        SZrTypeValue *callableValue__;                                                                                 \
        SZrTypeValue *receiverValue__;                                                                                 \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrFunctionStackAnchor callWindowAnchor__;                                                                     \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window(                                                     \
                state, callInfo, currentFunction, &base, (TZrUInt32)resultSlot__, parametersCount__);                 \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "KNOWN_NATIVE_MEMBER_CALL: failed to prepare call frame");                  \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + parametersCount__ + 1;                                          \
        callableValue__ = ZrCore_Stack_GetValueNoProfile(callWindow__);                                               \
        receiverValue__ = ZrCore_Stack_GetValueNoProfile(callWindow__ + 1);                                           \
        if (callableValue__ == ZR_NULL || receiverValue__ == ZR_NULL ||                                               \
            !execution_member_get_cached(                                                                              \
                    state, programCounter, currentFunction, cacheIndex__, receiverValue__, callableValue__)) {        \
        ...
        execution_store_or_copy_result_to_frame_slot(state,                                                            \
                                                      currentFunction,                                                  \
                                                      base,                                                             \
                                                      (TZrUInt32)resultSlot__,                                          \
                                                      ZrCore_Stack_GetValueNoProfile(callWindow__),                    \
                                                      profileRuntime,                                                   \
                                                      recordHelpers);                                                   \
    } while (0)
#define EXECUTE_KNOWN_NATIVE_MEMBER_CALL_RECV_U8_BODY()                                                                \
    do {                                                                                                               \
        TZrSize resultSlot__ = E(instruction);                                                                         \
        TZrUInt16 cacheIndex__ = (TZrUInt16)instruction.instruction.operand.operand0[0];                              \
        TZrUInt16 receiverSourceSlot__ = (TZrUInt16)instruction.instruction.operand.operand0[1];                      \
        TZrSize parametersCount__ = (TZrSize)instruction.instruction.operand.operand0[2];                             \
        SZrTypeValue *callableValue__;                                                                                 \
        SZrTypeValue *receiverValue__;                                                                                 \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window(                                                     \
                state, callInfo, currentFunction, &base, (TZrUInt32)resultSlot__, parametersCount__);                 \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "KNOWN_NATIVE_MEMBER_CALL_RECV_U8: failed to prepare call frame");          \
        }                                                                                                              \
        execution_copy_stack_value_to_stack_fast_no_profile(                                                           \
                state, ZrCore_Stack_GetValueNoProfile(callWindow__ + 1), FRAME_VALUE_SLOT(receiverSourceSlot__));     \
        if (ZR_UNLIKELY(recordHelpers)) {                                                                              \
            profileRuntime->helperCounts[ZR_PROFILE_HELPER_VALUE_COPY]++;                                             \
        }                                                                                                              \
                                                                                                                       \
        ...
        if (callableValue__ == ZR_NULL || receiverValue__ == ZR_NULL ||                                               \
            !execution_member_get_cached(                                                                              \
                    state, programCounter, currentFunction, cacheIndex__, receiverValue__, callableValue__)) {        \
            ZrCore_Debug_RunError(state, "KNOWN_NATIVE_MEMBER_CALL_RECV_U8: invalid cached native member callable"); \
        }                                                                                                              \
        ZrCore_Function_StackAnchorInit(state, callWindow__, &callWindowAnchor__);                                    \
        execution_pre_call_known_native_fast(state,                                                                    \
                                             callWindow__,                                                             \
...
#define EXECUTE_SUPER_FUNCTION_CALL_NO_ARGS_BODY()                                                                     \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        SZrCallInfo *nextCallInfo__;                                                                                   \
        ...
        nextCallInfo__ = execution_pre_call_frame_layout_generic_single_result(                                        \
                ...
                recordHelpers);                                                                                        \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_FUNCTION_CALL_NO_ARGS: failed to prepare call frame");               \
        }                                                                                                              \
        if (nextCallInfo__ == ZR_NULL) {                                                                               \
            RESUME_AFTER_NATIVE_CALL(state, callInfo);                                                                 \
        } else {                                                                                                       \
...
#define EXECUTE_SUPER_KNOWN_VM_CALL_NO_ARGS_BODY()                                                                     \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrCallInfo *nextCallInfo__;                                                                                   \
                                                                                                                       \
        if (currentFunction != ZR_NULL && currentFunction->frameSlotLayouts != ZR_NULL &&                              \
            currentFunction->frameSlotLayoutLength != 0u) {                                                            \
            callWindow__ = execution_prepare_frame_layout_call_window(                                                 \
                    state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, 0u);                           \
            if (callWindow__ == ZR_NULL) {                                                                             \
                ZrCore_Debug_RunError(state, "SUPER_KNOWN_VM_CALL_NO_ARGS: failed to prepare call frame");            \
            }                                                                                                          \
            opA = ZrCore_Stack_GetValueNoProfile(callWindow__);                                                        \
        } else {                                                                                                       \
            opA = FRAME_VALUE_SLOT(functionSlot__);                                                                    \
            execution_stage_frame_layout_call_values(state, currentFunction, base, (TZrUInt32)functionSlot__, 0u);     \
            callWindow__ = BASE(functionSlot__);                                                                       \
        }                                                                                                              \
        ZR_ASSERT(!ZR_VALUE_IS_TYPE_NULL(opA->type) &&                                                                \
        ...
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        nextCallInfo__ = execution_pre_call_known_vm_fast(state,                                                       \
                                                          callWindow__,                                                \
                                                          opA,                                                         \
                                                          0,                                                           \
                                                          expectedReturnCount__,                                        \
...
#define EXECUTE_SUPER_KNOWN_NATIVE_CALL_NO_ARGS_BODY()                                                                 \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window(                                                     \
                state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, 0u);                              \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_KNOWN_NATIVE_CALL_NO_ARGS: failed to prepare call frame");           \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + 1;                                                               \
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        ZrCore_Function_StackAnchorInit(state, callWindow__, &callWindowAnchor__);                                    \
...
#define EXECUTE_FUNCTION_CALL_BODY()                                                                                   \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
...
#define EXECUTE_SUPER_DYN_CALL_NO_ARGS_BODY()                                                                          \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        SZrCallInfo *nextCallInfo__;                                                                                   \
...
#define EXECUTE_SUPER_DYN_CALL_CACHED_BODY()                                                                           \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        const SZrFunctionCallSiteCacheEntry *cacheEntry__ = execution_get_callsite_cache_entry(                       \
                currentFunction, B1(instruction), ZR_FUNCTION_CALLSITE_CACHE_KIND_DYN_CALL);                          \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrFunctionStackAnchor callWindowAnchor__;                                                                     \
        TZrStackValuePointer functionPointer__;                                                                        \
        TZrUInt32 argumentSourceStartSlot__;                                                                           \
        SZrTypeValue *callableValue__;                                                                                 \
        TZrBool preparedDynCallTarget__;                                                                               \
        SZrCallInfo *nextCallInfo__;                                                                                   \
                                                                                                                       \
        if (cacheEntry__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_DYN_CALL_CACHED: invalid callsite cache");                           \
        }                                                                                                              \
                                                                                                                       \
        callWindow__ = execution_prepare_frame_layout_call_window_with_extra(                                          \
                state,                                                                                                 \
                callInfo,                                                                                              \
                currentFunction,                                                                                       \
                &base,                                                                                                 \
                (TZrUInt32)functionSlot__,                                                                             \
                cacheEntry__->argumentCount,                                                                           \
                1u);                                                                                                   \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_DYN_CALL_CACHED: failed to prepare call frame");                      \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + cacheEntry__->argumentCount + 1;                                \
        preparedDynCallTarget__ = execution_try_prepare_dyn_call_target_cached(                                        \
                state,                                                                                                \
                currentFunction,                                                                                      \
                B1(instruction),                                                                                      \
                callWindow__,                                                                                         \
                ZR_FUNCTION_CALLSITE_CACHE_KIND_DYN_CALL);                                                            \
        functionPointer__ = callWindow__;                                                                              \
        argumentSourceStartSlot__ =                                                                                   \
        ...
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        ZrCore_Function_StackAnchorInit(state, callWindow__, &callWindowAnchor__);                                    \
        nextCallInfo__ = callableValue__ != ZR_NULL &&                                                                 \
                                  !callableValue__->isNative &&                                                        \
                                  (callableValue__->type == ZR_VALUE_TYPE_FUNCTION ||                                  \
                                   callableValue__->type == ZR_VALUE_TYPE_CLOSURE)                                     \
                                 ? ZrCore_Function_PreCallKnownVmValueWithArgumentSource(                              \
                                           state,                                                                      \
                                           functionPointer__,                                                          \
                                           callableValue__,                                                            \
                                           expectedReturnCount__,                                                      \
                                           ...
                                           base,                                                                       \
                                           argumentSourceStartSlot__)                                                  \
                                 : ZrCore_Function_PreCall(                                                           \
                                           state, functionPointer__, expectedReturnCount__, BASE(E(instruction)));     \
        callWindow__ = ZrCore_Function_StackAnchorRestore(state, &callWindowAnchor__);                                \
        if (nextCallInfo__ == ZR_NULL) {                                                                               \
            RESUME_AFTER_NATIVE_CALL(state, callInfo);                                                                 \
        } else {                                                                                                       \
...
#define EXECUTE_DYN_CALL_BODY()                                                                                        \
    do {                                                                                                               \
        ...
        nextCallInfo__ = execution_pre_call_frame_layout_generic_single_result(                                        \
                ...
                recordHelpers);                                                                                        \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "DYN_CALL: failed to prepare call frame");                                  \
        }                                                                                                              \
        if (nextCallInfo__ == ZR_NULL) {                                                                               \
            RESUME_AFTER_NATIVE_CALL(state, callInfo);                                                                 \
        } else {                                                                                                       \
...
#define EXECUTE_SUPER_META_CALL_NO_ARGS_BODY()                                                                         \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        ...
        SZrCallInfo *nextCallInfo__;                                                                                   \
                                                                                                                       \
        callWindow__ = execution_prepare_frame_layout_call_window_with_extra(                                          \
                state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, 0u, 1u);                          \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_META_CALL_NO_ARGS: failed to prepare call frame");                    \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + 1;                                                              \
        if (!execution_prepare_meta_call_target(state, callWindow__)) {                                                \
        ...
        nextCallInfo__ = ZrCore_Function_PreCall(                                                                      \
                state, callWindow__, expectedReturnCount__, BASE(E(instruction)));                                    \
        callWindow__ = ZrCore_Function_StackAnchorRestore(state, &callWindowAnchor__);                                \
        if (nextCallInfo__ == ZR_NULL) {                                                                               \
            RESUME_AFTER_NATIVE_CALL(state, callInfo);                                                                 \
        } else {                                                                                                       \
...
#define EXECUTE_SUPER_META_CALL_CACHED_BODY()                                                                          \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        const SZrFunctionCallSiteCacheEntry *cacheEntry__ = execution_get_callsite_cache_entry(                       \
                currentFunction, B1(instruction), ZR_FUNCTION_CALLSITE_CACHE_KIND_META_CALL);                         \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrFunctionStackAnchor callWindowAnchor__;                                                                     \
        SZrCallInfo *nextCallInfo__;                                                                                   \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window_with_extra(                                          \
                ...
                1u);                                                                                                   \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_META_CALL_CACHED: failed to prepare call frame");                    \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + cacheEntry__->argumentCount + 1;                                \
...
#define EXECUTE_META_CALL_BODY()                                                                                       \
    do {                                                                                                               \
        ...
        SZrCallInfo *nextCallInfo__;                                                                                   \
                                                                                                                       \
        callWindow__ = execution_prepare_frame_layout_call_window_with_extra(                                          \
                state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, parametersCount__, 1u);           \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "META_CALL: failed to prepare call frame");                                 \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + parametersCount__ + 1;                                          \
        if (!execution_prepare_meta_call_target(state, callWindow__)) {                                                \
...
#define EXECUTE_FUNCTION_TAIL_CALL_BODY()                                                                              \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        ...
        state->stackTop.valuePointer = BASE(functionSlot__) + parametersCount__ + 1;                                  \
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        callInfo->callStatus |= ZR_CALL_STATUS_TAIL_CALL;                                                             \
        functionPointer__ = BASE(functionSlot__);                                                                      \
        if (execution_try_reuse_tail_call_frame(state, callInfo, functionPointer__)) {                                \
            callInfo->callStatus &= ~ZR_CALL_STATUS_TAIL_CALL;                                                        \
            goto LZrStart;                                                                                             \
        }                                                                                                              \
...
#define EXECUTE_KNOWN_VM_TAIL_CALL_BODY()                                                                              \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        ...
        if ((currentFunction == ZR_NULL || currentFunction->frameSlotLayouts == ZR_NULL ||                            \
             currentFunction->frameSlotLayoutLength == 0u) &&                                                          \
            execution_try_reuse_tail_call_frame(state, callInfo, functionPointer__)) {                                \
            callInfo->callStatus &= ~ZR_CALL_STATUS_TAIL_CALL;                                                        \
            goto LZrStart;                                                                                             \
        }                                                                                                              \
        opA = ZrCore_Stack_GetValueNoProfile(functionPointer__);                                                       \
        nextCallInfo__ = execution_pre_call_known_vm_fast(state,                                                       \
                                                          functionPointer__,                                           \
                                                          opA,                                                         \
                                                          parametersCount__,                                           \
...
#define EXECUTE_KNOWN_NATIVE_TAIL_CALL_BODY()                                                                          \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
...
#define EXECUTE_SUPER_FUNCTION_TAIL_CALL_NO_ARGS_BODY()                                                                \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer functionPointer__;                                                                        \
...
#define EXECUTE_SUPER_KNOWN_VM_TAIL_CALL_NO_ARGS_BODY()                                                                \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer functionPointer__;                                                                        \
...
#define EXECUTE_SUPER_KNOWN_NATIVE_TAIL_CALL_NO_ARGS_BODY()                                                            \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer functionPointer__;                                                                        \
...
#define EXECUTE_SUPER_DYN_TAIL_CALL_NO_ARGS_BODY()                                                                     \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer functionPointer__;                                                                        \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window(                                                     \
                state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, 0u);                              \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_DYN_TAIL_CALL_NO_ARGS: failed to prepare call frame");               \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + 1;                                                               \
        callInfo->context.context.programCounter = programCounter + 1;                                                 \
        callInfo->callStatus |= ZR_CALL_STATUS_TAIL_CALL;                                                             \
...
#define EXECUTE_SUPER_DYN_TAIL_CALL_CACHED_BODY()                                                                      \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        const SZrFunctionCallSiteCacheEntry *cacheEntry__ = execution_get_callsite_cache_entry(                       \
                currentFunction, B1(instruction), ZR_FUNCTION_CALLSITE_CACHE_KIND_DYN_TAIL_CALL);                     \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer functionPointer__;                                                                        \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrCallInfo *nextCallInfo__;                                                                                   \
...
#define EXECUTE_DYN_TAIL_CALL_BODY()                                                                                   \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
...
#define EXECUTE_SUPER_META_TAIL_CALL_NO_ARGS_BODY()                                                                    \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer functionPointer__;                                                                        \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window_with_extra(                                          \
                state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, 0u, 1u);                          \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_META_TAIL_CALL_NO_ARGS: failed to prepare call frame");              \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + 1;                                                              \
        if (!execution_prepare_meta_call_target(state, callWindow__)) {                                                \
...
#define EXECUTE_SUPER_META_TAIL_CALL_CACHED_BODY()                                                                     \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        const SZrFunctionCallSiteCacheEntry *cacheEntry__ = execution_get_callsite_cache_entry(                       \
                currentFunction, B1(instruction), ZR_FUNCTION_CALLSITE_CACHE_KIND_META_TAIL_CALL);                    \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        TZrStackValuePointer functionPointer__;                                                                        \
        TZrStackValuePointer callWindow__;                                                                             \
        SZrFunctionStackAnchor callWindowAnchor__;                                                                     \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window_with_extra(                                          \
                ...
                1u);                                                                                                   \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "SUPER_META_TAIL_CALL_CACHED: failed to prepare call frame");               \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + cacheEntry__->argumentCount + 1;                                \
        if (!execution_prepare_meta_call_target_cached(state,                                                          \
                                                       currentFunction,                                                \
...
#define EXECUTE_META_TAIL_CALL_BODY()                                                                                  \
    do {                                                                                                               \
        TZrSize functionSlot__ = A1(instruction);                                                                      \
        TZrSize parametersCount__ = B1(instruction);                                                                   \
        TZrSize expectedReturnCount__ = 1;                                                                             \
        ...
        callWindow__ = execution_prepare_frame_layout_call_window_with_extra(                                          \
                state, callInfo, currentFunction, &base, (TZrUInt32)functionSlot__, parametersCount__, 1u);           \
        if (callWindow__ == ZR_NULL) {                                                                                 \
            ZrCore_Debug_RunError(state, "META_TAIL_CALL: failed to prepare call frame");                            \
        }                                                                                                              \
        state->stackTop.valuePointer = callWindow__ + parametersCount__ + 1;                                          \
        if (!execution_prepare_meta_call_target(state, callWindow__)) {                                                \
...
#define EXECUTE_FUNCTION_RETURN_BODY()                                                                                 \
    do {                                                                                                               \
        TZrSize returnCount__ = E(instruction);                                                                        \
        TZrSize resultSlot__ = A1(instruction);                                                                        \
        TZrSize variableArguments__ = B1(instruction);                                                                 \
                                                                                                                        \
        callInfo->context.context.programCounter = programCounter;                                                     \
        if (ZR_LIKELY(returnCount__ == 1u && variableArguments__ == 0u &&                                             \
                      state->exceptionHandlerStackLength == 0u &&                                                      \
                      (currentFunction == ZR_NULL || currentFunction->returnEscapeSlotCount == 0u) &&                 \
                      state->debugHookSignal == 0u && callInfo->expectedReturnCount == 1u &&                          \
                      callInfo->callStatus == ZR_CALL_STATUS_NONE && callInfo->hasReturnDestination &&                \
                      !execution_callinfo_has_pending_close_work(state, callInfo))) {                                  \
            TZrStackValuePointer returnSource__ = BASE(resultSlot__);                                                  \
            const SZrTypeValue *returnSourceValue__ = FRAME_VALUE_SLOT(resultSlot__);                                  \
            SZrCallInfo *previousCallInfo__ = callInfo->previous;                                                     \
            ...
            UPDATE_BASE(callInfo);                                                                                     \
            execution_post_call_single_result_resolved_source_fast(                                                   \
                    state, callInfo, returnSource__, returnSourceValue__);                                            \
            trap = callInfo->context.context.trap;                                                                     \
            callInfo = previousCallInfo__;                                                                             \
            goto LZrReturning;                                                                                         \
        }                                                                                                              \
        ...
        if (currentFunction != ZR_NULL && currentFunction->returnEscapeSlotCount > 0u) {                              \
            for (TZrSize returnIndex__ = 0; returnIndex__ < returnCount__; returnIndex__++) {                         \
                const SZrTypeValue *returnEscapeValue__ =                                                             \
                        FRAME_VALUE_SLOT(resultSlot__ + returnIndex__);                                                \
                if (returnEscapeValue__ != ZR_NULL) {                                                                  \
                    ZrCore_Function_ApplyReturnEscape(state,                                                          \
                                                      currentFunction,                                                 \
                                                      (TZrUInt32)(resultSlot__ + returnIndex__),                      \
                                                      returnEscapeValue__);                                            \
                }                                                                                                      \
            }                                                                                                          \
        }                                                                                                              \
        ...
        UPDATE_BASE(callInfo);                                                                                         \
                                                                                                                        \
        if (variableArguments__ > 0) {                                                                                 \
            callInfo->functionBase.valuePointer -=                                                                     \
                    callInfo->context.context.variableArgumentCount + variableArguments__;                            \
        }                                                                                                              \
        if (ZR_LIKELY(state->debugHookSignal == 0u &&                                                                  \
                      callInfo->expectedReturnCount == 1u &&                                                           \
                      callInfo->callStatus == ZR_CALL_STATUS_NONE)) {                                                  \
            TZrStackValuePointer returnSource__ = returnCount__ > 0u ? BASE(resultSlot__) : ZR_NULL;                  \
            const SZrTypeValue *returnSourceValue__ = returnCount__ > 0u ? FRAME_VALUE_SLOT(resultSlot__) : ZR_NULL;  \
            execution_post_call_single_result_resolved_source_fast(                                                   \
                    state, callInfo, returnSource__, returnSourceValue__);                                            \
        } else {                                                                                                       \
            state->stackTop.valuePointer = BASE(resultSlot__) + returnCount__;                                        \
            ZrCore_Function_PostCall(state, callInfo, returnCount__);                                                 \
        }                                                                                                              \
        trap = callInfo->context.context.trap;                                                                         \
        goto LZrReturn;                                                                                                \
    } while (0)
#define EXECUTE_GET_STACK_BODY_FAST()                                                                                  \
    do {                                                                                                               \
        TZrUInt16 destinationOffset__ = E(instruction);                                                                \
        const SZrTypeValue *source = FRAME_VALUE_SLOT(A2(instruction));                                                \
        if (ZR_UNLIKELY(recordHelpers)) {                                                                              \
...
#define EXECUTE_MATERIALIZE_CONSTANT_SLOT(CONSTANT_INDEX, MATERIALIZED_SLOT)                                           \
    do {                                                                                                               \
        ...
        SZrTypeValue *materializedDestination__ = FRAME_VALUE_SLOT(MATERIALIZED_SLOT);                                  \
        if (ZR_UNLIKELY(recordHelpers)) {                                                                              \
...
#define EXECUTE_MATERIALIZE_STACK_SLOT(SOURCE_SLOT, MATERIALIZED_SLOT)                                                 \
    do {                                                                                                               \
        const SZrTypeValue *materializedSource__ = FRAME_VALUE_SLOT(SOURCE_SLOT);                                       \
        SZrTypeValue *materializedDestination__ = FRAME_VALUE_SLOT(MATERIALIZED_SLOT);                                  \
        if (ZR_UNLIKELY(recordHelpers)) {                                                                              \
            execution_copy_value_fast(state,                                                                            \
                                      materializedDestination__,                                                        \
...
#define EXECUTE_ADD_INT_BODY_PLAIN_DEST()                                                                              \
    do {                                                                                                               \
        SZrTypeValue *plainDestination__ = FRAME_VALUE_SLOT(E(instruction));                                           \
        const SZrTypeValue *leftValue__ = FRAME_VALUE_SLOT(A1(instruction));                                           \
        const SZrTypeValue *rightValue__ = FRAME_VALUE_SLOT(B1(instruction));                                          \
...
#define EXECUTE_TYPED_NEG_SIGNED_BODY()                                                                                \
    do {                                                                                                               \
        opA = SIGNED_FRAME_VALUE_SLOT(A1(instruction));                                                                \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_SIGNED_INT(opA->type));                                                            \
        ZR_VALUE_FAST_SET(destination, nativeInt64, -opA->value.nativeObject.nativeInt64, opA->type);                 \
    } while (0)
#define EXECUTE_TYPED_NEG_FLOAT_BODY()                                                                                 \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_FLOAT(opA->type));                                                                 \
        ZR_VALUE_FAST_SET(destination, nativeDouble, -opA->value.nativeObject.nativeDouble, opA->type);               \
    } while (0)
#define EXECUTE_TYPED_TO_FLOAT_SIGNED_BODY()                                                                           \
    do {                                                                                                               \
        opA = SIGNED_FRAME_VALUE_SLOT(A1(instruction));                                                                \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_SIGNED_INT(opA->type));                                                            \
        ZR_VALUE_FAST_SET(destination,                                                                                 \
                          nativeDouble,                                                                                \
                          (TZrFloat64)opA->value.nativeObject.nativeInt64,                                            \
                          ZR_VALUE_TYPE_DOUBLE);                                                                       \
    } while (0)
#define EXECUTE_TYPED_TO_FLOAT_UNSIGNED_BODY()                                                                         \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_UNSIGNED_INT(opA->type));                                                          \
        ZR_VALUE_FAST_SET(destination,                                                                                 \
                          nativeDouble,                                                                                \
                          (TZrFloat64)opA->value.nativeObject.nativeUInt64,                                           \
                          ZR_VALUE_TYPE_DOUBLE);                                                                       \
    } while (0)
#define EXECUTE_TYPED_TO_INT_FLOAT_BODY()                                                                              \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_FLOAT(opA->type));                                                                 \
        ZR_VALUE_FAST_SET(destination,                                                                                 \
                          nativeInt64,                                                                                 \
                          (TZrInt64)opA->value.nativeObject.nativeDouble,                                             \
                          ZR_VALUE_TYPE_INT64);                                                                        \
    } while (0)
#define EXECUTE_TYPED_TO_INT_UNSIGNED_BODY()                                                                           \
    do {                                                                                                               \
        TZrUInt64 zr_vm_source_scalar__;                                                                               \
        TZrUInt64 zr_vm_unsigned_to_signed_limit__;                                                                    \
        TZrInt64 zr_vm_converted_scalar__;                                                                             \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_UNSIGNED_INT(opA->type));                                                          \
        zr_vm_source_scalar__ = opA->value.nativeObject.nativeUInt64;                                                  \
        zr_vm_unsigned_to_signed_limit__ = (TZrUInt64)ZR_TYPE_RANGE_INT64_MAX + (TZrUInt64)1u;                        \
        if (zr_vm_source_scalar__ >= zr_vm_unsigned_to_signed_limit__) {                                               \
            zr_vm_converted_scalar__ = ZR_TYPE_RANGE_INT64_MIN +                                                       \
                                      (TZrInt64)(zr_vm_source_scalar__ - zr_vm_unsigned_to_signed_limit__);            \
        } else {                                                                                                       \
            zr_vm_converted_scalar__ = (TZrInt64)zr_vm_source_scalar__;                                                \
        }                                                                                                              \
        ZR_VALUE_FAST_SET(destination,                                                                                 \
                          nativeInt64,                                                                                 \
                          zr_vm_converted_scalar__,                                                                    \
                          ZR_VALUE_TYPE_INT64);                                                                        \
    } while (0)
#define EXECUTE_TYPED_TO_UINT_FLOAT_BODY()                                                                             \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_FLOAT(opA->type));                                                                 \
        ZR_VALUE_FAST_SET(destination,                                                                                 \
                          nativeUInt64,                                                                                \
                          (TZrUInt64)opA->value.nativeObject.nativeDouble,                                            \
                          ZR_VALUE_TYPE_UINT64);                                                                       \
    } while (0)
#define EXECUTE_TYPED_TO_UINT_SIGNED_BODY()                                                                            \
    do {                                                                                                               \
        opA = SIGNED_FRAME_VALUE_SLOT(A1(instruction));                                                                \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_SIGNED_INT(opA->type));                                                            \
        ZR_VALUE_FAST_SET(destination,                                                                                 \
                          nativeUInt64,                                                                                \
                          (TZrUInt64)opA->value.nativeObject.nativeInt64,                                             \
                          ZR_VALUE_TYPE_UINT64);                                                                       \
    } while (0)
...
#define EXECUTE_ADD_SIGNED_MOD_CONST_BODY()                                                                            \
    do {                                                                                                               \
        const SZrTypeValue *constOpC__ = CONST(instruction.instruction.operand.operand1[1]);                          \
        TZrInt64 divisor__;                                                                                            \
        opA = SIGNED_FRAME_VALUE_SLOT(instruction.instruction.operand.operand0[0]);                                     \
        opB = SIGNED_FRAME_VALUE_SLOT(instruction.instruction.operand.operand0[1]);                                     \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_SIGNED_INT(opA->type) && ZR_VALUE_IS_TYPE_SIGNED_INT(opB->type));                  \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_INT(constOpC__->type));                                                             \
        divisor__ = constOpC__->value.nativeObject.nativeInt64;                                                        \
        if (ZR_UNLIKELY(divisor__ == 0)) {                                                                             \
        ...
        EXECUTION_STORE_PLAIN_REUSE(                                                                                   \
                nativeInt64,                                                                                           \
                (opA->value.nativeObject.nativeInt64 + opB->value.nativeObject.nativeInt64) % divisor__,               \
                ZR_VALUE_TYPE_INT64);                                                                                  \
    } while (0)
...
#define EXECUTE_TYPED_LOGICAL_NOT_BOOL_BODY()                                                                          \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_BOOL(opA->type));                                                                   \
        ZR_VALUE_FAST_SET(destination,                                                                                  \
                          nativeBool,                                                                                   \
                          !opA->value.nativeObject.nativeBool,                                                          \
                          ZR_VALUE_TYPE_BOOL);                                                                          \
    } while (0)
...
#define EXECUTE_JUMP_IF_GREATER_SIGNED_BODY(JUMP_IMPL, CALL_INFO)                                                     \
    do {                                                                                                               \
        const SZrTypeValue *leftValue__ = SIGNED_FRAME_VALUE_SLOT(E(instruction));                                     \
        const SZrTypeValue *rightValue__ = SIGNED_FRAME_VALUE_SLOT(A1(instruction));                                   \
...
#define EXECUTE_JUMP_IF_LESS_EQUAL_SIGNED_BODY(JUMP_IMPL, CALL_INFO)                                                  \
    do {                                                                                                               \
        const SZrTypeValue *leftValue__ = SIGNED_FRAME_VALUE_SLOT(E(instruction));                                     \
        const SZrTypeValue *rightValue__ = SIGNED_FRAME_VALUE_SLOT(A1(instruction));                                   \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_INT(leftValue__->type) && ZR_VALUE_IS_TYPE_INT(rightValue__->type));               \
...
#define EXECUTE_JUMP_IF_NULL_BODY(JUMP_IMPL, CALL_INFO)                                                               \
    do {                                                                                                               \
        const SZrTypeValue *value__ = FRAME_VALUE_SLOT(E(instruction));                                                \
        if (ZR_VALUE_IS_TYPE_NULL(value__->type)) {                                                                    \
            JUMP_IMPL(CALL_INFO, instruction, 0);                                                                      \
        }                                                                                                              \
    } while (0)
#define EXECUTE_JUMP_IF_BOOL_FALSE_BODY(JUMP_IMPL, CALL_INFO)                                                         \
    do {                                                                                                               \
        const SZrTypeValue *conditionValue__ = FRAME_VALUE_SLOT(E(instruction));                                       \
        ZR_ASSERT(conditionValue__ != ZR_NULL && ZR_VALUE_IS_TYPE_BOOL(conditionValue__->type));                      \
        if (!conditionValue__->value.nativeObject.nativeBool) {                                                        \
            JUMP_IMPL(CALL_INFO, instruction, 0);                                                                      \
        }                                                                                                              \
    } while (0)
#define EXECUTE_SUPER_ARRAY_BIND_ITEMS_BODY()                                                                          \
    do {                                                                                                               \
        SZrTypeValue *itemsDestination__ = FRAME_VALUE_SLOT(E(instruction));                                            \
        SZrTypeValue *receiverValue__ = FRAME_VALUE_SLOT(A2(instruction));                                                  \
...
#define EXECUTE_SUPER_ARRAY_GET_INT_BODY()                                                                             \
    do {                                                                                                               \
        SZrTypeValue stableResult__;                                                                                   \
        TZrBool inlineStructDestination__ =                                                                            \
                (TZrBool)execution_frame_slot_is_inline_struct_destination(currentFunction, E(instruction));           \
        SZrTypeValue *resultValue = inlineStructDestination__ ? &stableResult__ : FRAME_VALUE_SLOT(E(instruction));   \
        SZrTypeValue *receiverValue = FRAME_VALUE_SLOT(A1(instruction));                                                   \
        const SZrTypeValue *indexValue = FRAME_VALUE_SLOT(B1(instruction));                                                \
        ZR_ASSERT(E(instruction) != ZR_INSTRUCTION_USE_RET_FLAG);                                                      \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_SIGNED_INT(indexValue->type));                                                      \
        if (inlineStructDestination__) {                                                                               \
            ZrCore_Value_ResetAsNullNoProfile(&stableResult__);                                                        \
        }                                                                                                              \
        ...
        } else if (inlineStructDestination__ &&                                                                        \
                   !execution_store_result_to_inline_struct_destination_if_needed(                                     \
                           state, currentFunction, base, E(instruction), &stableResult__)) {                          \
            execution_copy_value_fast(                                                                                 \
                    state, FRAME_VALUE_SLOT(E(instruction)), &stableResult__, profileRuntime, recordHelpers);         \
        }                                                                                                              \
    } while (0)
#define EXECUTE_SUPER_ARRAY_GET_INT_ITEMS_BODY()                                                                       \
    do {                                                                                                               \
        ...
        TZrBool inlineStructDestination__ =                                                                            \
                (TZrBool)execution_frame_slot_is_inline_struct_destination(currentFunction, E(instruction));           \
        SZrTypeValue *resultValue__ = inlineStructDestination__ ? &stableResult__ : FRAME_VALUE_SLOT(E(instruction)); \
        const SZrTypeValue *itemsValue__ = FRAME_VALUE_SLOT(A1(instruction));                                               \
        const SZrTypeValue *indexValue__ = FRAME_VALUE_SLOT(B1(instruction));                                              \
...
#define EXECUTE_SUPER_ARRAY_GET_INT_PLAIN_DEST_BODY_TO(RESULT_VALUE)                                                   \
    do {                                                                                                               \
        ...
        SZrTypeValue *receiverValue__ = FRAME_VALUE_SLOT(A1(instruction));                                                 \
        const SZrTypeValue *indexValue__ = FRAME_VALUE_SLOT(B1(instruction));                                              \
        SZrObject *itemsObject__ = ZR_NULL;                                                                            \
...
#define EXECUTE_SUPER_ARRAY_GET_INT_PLAIN_DEST_BODY()                                                                  \
    EXECUTE_SUPER_ARRAY_GET_INT_PLAIN_DEST_BODY_TO(FRAME_VALUE_SLOT(E(instruction)))
...
#define EXECUTE_SUPER_ARRAY_SET_INT_BODY()                                                                             \
    do {                                                                                                               \
        SZrTypeValue *receiverValue = FRAME_VALUE_SLOT(A1(instruction));                                                   \
        const SZrTypeValue *indexValue = FRAME_VALUE_SLOT(B1(instruction));                                                \
        const SZrTypeValue *storedValue = FRAME_VALUE_SLOT(E(instruction));                                            \
        ZR_ASSERT(E(instruction) != ZR_INSTRUCTION_USE_RET_FLAG);                                                      \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_SIGNED_INT(indexValue->type));                                                      \
...
#define EXECUTE_SUPER_ARRAY_SET_INT_ITEMS_BODY()                                                                       \
    do {                                                                                                               \
        const SZrTypeValue *itemsValue__ = FRAME_VALUE_SLOT(A1(instruction));                                               \
        const SZrTypeValue *indexValue__ = FRAME_VALUE_SLOT(B1(instruction));                                              \
        const SZrTypeValue *storedValue__ = FRAME_VALUE_SLOT(E(instruction));                                           \
        SZrObject *itemsObject__;                                                                                       \
        ZR_ASSERT(E(instruction) != ZR_INSTRUCTION_USE_RET_FLAG);                                                       \
        ZR_ASSERT(ZR_VALUE_IS_TYPE_SIGNED_INT(indexValue__->type));                                                     \
...
#define EXECUTE_SUPER_ARRAY_ADD_INT_BODY()                                                                             \
    do {                                                                                                               \
        opA = FRAME_VALUE_SLOT(A1(instruction));                                                                           \
        opB = FRAME_VALUE_SLOT(B1(instruction));                                                                           \
        if (ZR_UNLIKELY(!(E(instruction) == ZR_INSTRUCTION_USE_RET_FLAG                                               \
                                  ? ZrCore_Object_SuperArrayAddIntDiscardResultAssumeFast(state, opA, opB)            \
                                  : ZrCore_Object_SuperArrayAddIntAssumeFast(state, opA, opB, destination)))) {       \
            ZrCore_Debug_RunError(state,                                                                               \
...
#define UPDATE_BASE(CALL_INFO)                                                                                        \
    do {                                                                                                               \
        base = (CALL_INFO)->functionBase.valuePointer + 1;                                                            \
        packedDirectValueBase =                                                                                       \
                (CALL_INFO)->functionBase.valuePointer == frameFunctionBase                                            \
                        ? execution_packed_direct_value_frame_base(                                                    \
                                  currentFunction, base)                                                               \
                        : ZR_NULL;                                                                                     \
    } while (0)
...
LZrReturning: {
    ...
    for (;;) {
        ...
        TZrStackValuePointer packedDirectValueBase = ZR_NULL;
        ...
        packedDirectValueBase =
                execution_packed_direct_value_frame_base(currentFunction, base);
...
#endif
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_RESET_STACK_NULL2: {
                EXECUTE_RESET_STACK_NULL2_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
            ZR_INSTRUCTION_LABEL(RESET_STACK_NULL2) {
                EXECUTE_RESET_STACK_NULL2_BODY();
            }
            DONE(1);
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_TO_FLOAT_SIGNED: {
                FAST_PREPARE_SIGNED_DESTINATION();
                EXECUTE_TYPED_TO_FLOAT_SIGNED_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(TO_FLOAT_SIGNED) {
                EXECUTE_TYPED_TO_FLOAT_SIGNED_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_TO_FLOAT_UNSIGNED: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_TYPED_TO_FLOAT_UNSIGNED_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(TO_FLOAT_UNSIGNED) {
                EXECUTE_TYPED_TO_FLOAT_UNSIGNED_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_TO_INT_FLOAT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_TYPED_TO_INT_FLOAT_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(TO_INT_FLOAT) {
                EXECUTE_TYPED_TO_INT_FLOAT_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_TO_INT_UNSIGNED: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_TYPED_TO_INT_UNSIGNED_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(TO_INT_UNSIGNED) {
                EXECUTE_TYPED_TO_INT_UNSIGNED_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_TO_UINT_FLOAT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_TYPED_TO_UINT_FLOAT_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(TO_UINT_FLOAT) {
                EXECUTE_TYPED_TO_UINT_FLOAT_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_TO_UINT_SIGNED: {
                FAST_PREPARE_SIGNED_DESTINATION();
                EXECUTE_TYPED_TO_UINT_SIGNED_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(TO_UINT_SIGNED) {
                EXECUTE_TYPED_TO_UINT_SIGNED_BODY();
            }
            DONE(1);
            ...
            ZR_INSTRUCTION_LABEL(TO_STRUCT) {
                ...
                if (E(instruction) != ZR_INSTRUCTION_USE_RET_FLAG) {
            ...
            ZR_INSTRUCTION_LABEL(TO_OBJECT) {
                opA = FRAME_VALUE_SLOT(A1(instruction));
                destination = E(instruction) == ZR_INSTRUCTION_USE_RET_FLAG ? &ret : FRAME_VALUE_SLOT(E(instruction));
                if (currentFunction != ZR_NULL && B1(instruction) < currentFunction->constantValueLength) {
                    (void)ZrCore_Execution_ToObject(state, callInfo, destination, opA, CONST(B1(instruction)));
                } else {
                    ZrCore_Value_ResetAsNull(destination);
                }
                if (E(instruction) != ZR_INSTRUCTION_USE_RET_FLAG) {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_ADD: {
                FAST_PREPARE_DESTINATION();
                goto LZrInstruction_ADD;
            }
#endif
                ...
                if (execution_try_builtin_add_exact_numeric_fast(destination, opA, opB)) {
                    ...
                    if (destination == &ret || !builtinNeedsTemporaryResult) {
                        if (try_builtin_add(state, destination, opA, opB)) {
                            ...
                            SZrTypeValue metaArg0Value;
                            SZrTypeValue metaArg1Value;
                            SZrTypeValue *metaReceiver = opA;
                            const SZrTypeValue *metaArg0 = opA;
                            const SZrTypeValue *metaArg1 = opB;
                            SZrMeta *meta;
                            ZrCore_Value_ResetAsNull(&metaArg0Value);
                            ZrCore_Value_ResetAsNull(&metaArg1Value);
                            if (ZrCore_Function_CopyFrameSlotInlineToObjectValue(
                                        state, currentFunction, base, A1(instruction), &metaArg0Value)) {
                                metaReceiver = &metaArg0Value;
                                metaArg0 = &metaArg0Value;
                            }
                            if (ZrCore_Function_CopyFrameSlotInlineToObjectValue(
                                        state, currentFunction, base, B1(instruction), &metaArg1Value)) {
                                metaArg1 = &metaArg1Value;
                            }
                            meta = ZrCore_Value_GetMeta(state, metaReceiver, ZR_META_ADD);
                            if (meta != ZR_NULL && meta->function != ZR_NULL) {
                            // 调用元方法
                                TZrStackValuePointer savedStackTop = state->stackTop.valuePointer;
                                SZrCallInfo *savedCallInfo = state->callInfoList;
                                PROTECT_E(state, callInfo, {
                                    TZrStackValuePointer metaBase = ZR_NULL;
                                    TZrStackValuePointer restoredStackTop = savedStackTop;
                                    TZrStackValuePointer metaReturnDestination =
                                            E(instruction) == ZR_INSTRUCTION_USE_RET_FLAG ? ZR_NULL : BASE(E(instruction));
                                    TZrBool metaCallSucceeded = execution_invoke_meta_call_to_destination(
                                            state,
                                            savedCallInfo,
                                            savedStackTop,
                                            savedStackTop,
                                    ...
                                    if (metaCallSucceeded) {
                                        SZrTypeValue *returnValue = ZrCore_Stack_GetValue(metaBase);
                                        if (metaReturnDestination == ZR_NULL && returnValue != ZR_NULL) {
                                            ZrCore_Value_Copy(state, destination, returnValue);
                                        }
                                    } else {
                                        ZrCore_Value_ResetAsNull(destination);
                        ...
                        if (try_builtin_add(state, &builtinResult, opA, opB)) {
                            ...
                            // 调用元方法
                                ...
                                PROTECT_E(state, callInfo, {
                                    TZrStackValuePointer metaBase = ZR_NULL;
                                    TZrStackValuePointer restoredStackTop = savedStackTop;
                                    TZrStackValuePointer metaReturnDestination = BASE(E(instruction));
                                    TZrBool metaCallSucceeded = execution_invoke_meta_call_to_destination(
                                            state,
                                            savedCallInfo,
                                            savedStackTop,
                                    ...
                                    destination = FRAME_VALUE_SLOT(E(instruction));
                                    if (metaCallSucceeded) {
                                        /* Return-destination calls copy object and inline frame results during call
                                           post-processing. Re-copying from the physical slot here can overwrite a
                                           freshly copied inline struct when the callee returned from inline storage. */
                                    } else {
                                        ZrCore_Value_ResetAsNull(destination);
                                    }
                                    state->stackTop.valuePointer = restoredStackTop;
...
LZrFastInstruction_ADD_SIGNED_LOAD_STACK: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_ADD_SIGNED_MOD_CONST: {
                FAST_PREPARE_SIGNED_DESTINATION();
                EXECUTE_ADD_SIGNED_MOD_CONST_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(ADD_SIGNED_MOD_CONST) {
                EXECUTE_ADD_SIGNED_MOD_CONST_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
...
LZrFastInstruction_SUB_UNSIGNED_CONST_PLAIN_DEST: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_MUL: {
                FAST_PREPARE_DESTINATION();
                goto LZrInstruction_MUL;
            }
#endif
...
LZrFastInstruction_MUL_SIGNED_LOAD_STACK_CONST: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_MUL_SIGNED_LOAD_STACK: {
                FAST_PREPARE_SIGNED_DESTINATION();
                EXECUTE_MUL_SIGNED_LOAD_STACK_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(MUL_SIGNED_LOAD_STACK) {
                EXECUTE_MUL_SIGNED_LOAD_STACK_BODY();
            }
            DONE(1);
...
LZrFastInstruction_MUL_UNSIGNED_CONST_PLAIN_DEST: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_NEG_SIGNED: {
                FAST_PREPARE_SIGNED_DESTINATION();
                EXECUTE_TYPED_NEG_SIGNED_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(NEG_SIGNED) {
                EXECUTE_TYPED_NEG_SIGNED_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_NEG_FLOAT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_TYPED_NEG_FLOAT_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(NEG_FLOAT) {
                EXECUTE_TYPED_NEG_FLOAT_BODY();
            }
            DONE(1);
...
LZrFastInstruction_DIV_UNSIGNED_CONST_PLAIN_DEST: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_MOD: {
                FAST_PREPARE_DESTINATION();
                goto LZrInstruction_MOD;
            }
#endif
...
LZrFastInstruction_MOD_UNSIGNED_CONST_PLAIN_DEST: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_LOGICAL_NOT_BOOL: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_TYPED_LOGICAL_NOT_BOOL_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(LOGICAL_NOT_BOOL) {
                EXECUTE_TYPED_LOGICAL_NOT_BOOL_BODY();
            }
            DONE(1);
...
LZrFastInstruction_LOGICAL_LESS_EQUAL_SIGNED: {
            ...
            ZR_INSTRUCTION_LABEL(BITWISE_NOT) {
                EXECUTE_BITWISE_NOT_BODY();
            }
            DONE(1);
            ZR_INSTRUCTION_LABEL(BITWISE_AND) {
                EXECUTE_BITWISE_BINARY_BODY(&);
            }
            DONE(1);
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_BITWISE_XOR: {
                FAST_PREPARE_DESTINATION();
                goto LZrInstruction_BITWISE_XOR;
            }
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_FUNCTION_CALL_NO_ARGS: {
                EXECUTE_SUPER_FUNCTION_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_KNOWN_VM_CALL_NO_ARGS: {
                EXECUTE_SUPER_KNOWN_VM_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_KNOWN_NATIVE_CALL_NO_ARGS: {
                EXECUTE_SUPER_KNOWN_NATIVE_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_FUNCTION_TAIL_CALL_NO_ARGS: {
                EXECUTE_SUPER_FUNCTION_TAIL_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_KNOWN_VM_TAIL_CALL_NO_ARGS: {
                EXECUTE_SUPER_KNOWN_VM_TAIL_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_KNOWN_NATIVE_TAIL_CALL_NO_ARGS: {
                EXECUTE_SUPER_KNOWN_NATIVE_TAIL_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_FUNCTION_CALL: {
                EXECUTE_FUNCTION_CALL_BODY();
            }
            DONE_FAST(1);
#endif
...
LZrFastInstruction_KNOWN_VM_CALL: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_KNOWN_VM_MEMBER_CALL: {
                EXECUTE_KNOWN_VM_MEMBER_CALL_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_KNOWN_VM_MEMBER_CALL_LOAD1_U8: {
                EXECUTE_KNOWN_VM_MEMBER_CALL_LOAD1_U8_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_KNOWN_NATIVE_CALL: {
                EXECUTE_KNOWN_NATIVE_CALL_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_KNOWN_NATIVE_MEMBER_CALL: {
                EXECUTE_KNOWN_NATIVE_MEMBER_CALL_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_KNOWN_NATIVE_MEMBER_CALL_RECV_U8: {
                EXECUTE_KNOWN_NATIVE_MEMBER_CALL_RECV_U8_BODY();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(KNOWN_NATIVE_MEMBER_CALL_RECV_U8) {
                EXECUTE_KNOWN_NATIVE_MEMBER_CALL_RECV_U8_BODY();
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_DYN_CALL_NO_ARGS: {
                EXECUTE_SUPER_DYN_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_DYN_CALL_CACHED: {
                EXECUTE_SUPER_DYN_CALL_CACHED_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_DYN_TAIL_CALL_NO_ARGS: {
                EXECUTE_SUPER_DYN_TAIL_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_DYN_TAIL_CALL_CACHED: {
                EXECUTE_SUPER_DYN_TAIL_CALL_CACHED_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_DYN_CALL: {
                EXECUTE_DYN_CALL_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_DYN_TAIL_CALL: {
                EXECUTE_DYN_TAIL_CALL_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_META_CALL_NO_ARGS: {
                EXECUTE_SUPER_META_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_META_CALL_CACHED: {
                EXECUTE_SUPER_META_CALL_CACHED_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_META_TAIL_CALL_NO_ARGS: {
                EXECUTE_SUPER_META_TAIL_CALL_NO_ARGS_BODY();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_META_TAIL_CALL_CACHED: {
                EXECUTE_SUPER_META_TAIL_CALL_CACHED_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_META_CALL: {
                EXECUTE_META_CALL_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_META_TAIL_CALL: {
                EXECUTE_META_TAIL_CALL_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_FUNCTION_TAIL_CALL: {
                EXECUTE_FUNCTION_TAIL_CALL_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_KNOWN_VM_TAIL_CALL: {
                EXECUTE_KNOWN_VM_TAIL_CALL_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_KNOWN_NATIVE_TAIL_CALL: {
                EXECUTE_KNOWN_NATIVE_TAIL_CALL_BODY();
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_FUNCTION_RETURN: {
                EXECUTE_FUNCTION_RETURN_BODY();
            }
            DONE_FAST(1);
#endif
        ...
        LZrReturn: {
            ...
            ZR_INSTRUCTION_LABEL(GETUPVAL) {
                ...
                SZrTypeValue *upvalueDestination = FRAME_VALUE_SLOT(E(instruction));
                ...
                ZrCore_Value_Copy(state, upvalueDestination, execution_get_closure_value_no_profile(closureValue));
            }
            DONE(1);
            ...
            ZR_INSTRUCTION_LABEL(SETUPVAL) {
                ...
                SZrTypeValue *source = FRAME_VALUE_SLOT(E(instruction));
                ZrCore_Value_Copy(state, target, source);
                ...
                if (parentFunction != ZR_NULL) {
                    ...
                    if (childFunctionIndex < parentFunction->childFunctionLength) {
                        ...
                        if (childFunction != ZR_NULL &&
                            ...
                            execution_push_closure_to_effective_destination_no_profile(state,
                                                                                       childFunction,
                                                                                       parentClosureValues,
                                                                                       base,
                                                                                       BASE(E(instruction)),
            ...
            ZR_INSTRUCTION_LABEL(GET_MEMBER) {
                ...
                } else if (execution_inline_frame_try_get_member_by_name(
                                   state, currentFunction, base, A1(instruction), memberName, destination)) {
...
LZrFastInstruction_GET_MEMBER_SLOT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_GET_MEMBER_SLOT_BODY_FAST();
            }
            DONE_FAST(1);
#endif
...
LZrFastInstruction_SET_MEMBER_SLOT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_SET_MEMBER_SLOT_BODY_FAST();
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SET_MEMBER_SLOT_NULL: {
                EXECUTE_SET_MEMBER_SLOT_NULL_BODY_FAST();
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(SET_MEMBER_SLOT_NULL) {
                EXECUTE_SET_MEMBER_SLOT_NULL_BODY();
            }
            DONE(1);
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_GET_BY_INDEX: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_GET_BY_INDEX_BODY();
            }
            DONE_FAST(1);
#endif
...
#endif
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_ARRAY_ADD_INT: {
                FAST_PREPARE_DESTINATION();
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_ARRAY_ADD_INT4: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_ARRAY_ADD_INT4_CONST: {
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_ITER_INIT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_ITER_INIT_BODY("ITER_INIT: receiver does not satisfy iterable contract");
            }
            DONE_FAST(1);
#endif
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_ITER_MOVE_NEXT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_ITER_MOVE_NEXT_BODY("ITER_MOVE_NEXT: receiver does not satisfy iterator contract", DONE_FAST);
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(ITER_MOVE_NEXT) {
                EXECUTE_ITER_MOVE_NEXT_BODY("ITER_MOVE_NEXT: receiver does not satisfy iterator contract", DONE);
            }
            DONE(1);

#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_DYN_ITER_INIT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_ITER_INIT_BODY("DYN_ITER_INIT: receiver does not satisfy dynamic iterable contract");
            }
            DONE_FAST(1);
#endif
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_DYN_ITER_MOVE_NEXT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_ITER_MOVE_NEXT_BODY(
                        "DYN_ITER_MOVE_NEXT: receiver does not satisfy dynamic iterator contract", DONE_FAST);
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(DYN_ITER_MOVE_NEXT) {
                EXECUTE_ITER_MOVE_NEXT_BODY(
                        "DYN_ITER_MOVE_NEXT: receiver does not satisfy dynamic iterator contract", DONE);
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_SUPER_ITER_MOVE_NEXT_JUMP_IF_FALSE: {
...
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_ITER_CURRENT: {
                FAST_PREPARE_DESTINATION();
                EXECUTE_ITER_CURRENT_BODY(DONE_FAST);
            }
            DONE_FAST(1);
#endif
            ZR_INSTRUCTION_LABEL(ITER_CURRENT) {
                EXECUTE_ITER_CURRENT_BODY(DONE);
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
...
LZrFastInstruction_JUMP_IF: {
                SZrTypeValue *condValue = FRAME_VALUE_SLOT(E(instruction));
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_JUMP_IF_BOOL_FALSE: {
                EXECUTE_JUMP_IF_BOOL_FALSE_BODY(JUMP_FAST, callInfo);
            }
            DONE_AFTER_TRAP_FAST_ONE();
#endif
            ZR_INSTRUCTION_LABEL(JUMP_IF_BOOL_FALSE) {
                EXECUTE_JUMP_IF_BOOL_FALSE_BODY(JUMP, callInfo);
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_JUMP_IF_GREATER_SIGNED: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_JUMP_IF_LESS_EQUAL_SIGNED: {
                EXECUTE_JUMP_IF_LESS_EQUAL_SIGNED_BODY(JUMP1_16_FAST, callInfo);
            }
            DONE_AFTER_TRAP_FAST_ONE();
#endif
            ZR_INSTRUCTION_LABEL(JUMP_IF_LESS_EQUAL_SIGNED) {
                EXECUTE_JUMP_IF_LESS_EQUAL_SIGNED_BODY(JUMP1_16, callInfo);
            }
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
...
LZrFastInstruction_JUMP_IF_NOT_EQUAL_SIGNED_CONST: {
            ...
            DONE(1);
#if defined(ZR_INSTRUCTION_USE_DISPATCH_TABLE) && ZR_INSTRUCTION_DISPATCH_TABLE_SUPPORTED
LZrFastInstruction_JUMP_IF_NULL: {
                EXECUTE_JUMP_IF_NULL_BODY(JUMP1_16_FAST, callInfo);
            }
            DONE_AFTER_TRAP_FAST_ONE();
#endif
            ZR_INSTRUCTION_LABEL(JUMP_IF_NULL) {
...
LZrFastInstruction_BIND_INLINE_ARRAY_ELEMENT_PLACE:
            ...
            DONE(1);
            ZR_INSTRUCTION_LABEL(OWN_VIEW_SHARED)
            ...
            DONE(1);
            ZR_INSTRUCTION_LABEL(OWN_VIEW_MUT)
            ...
            DONE(1);
            ZR_INSTRUCTION_LABEL(OWN_INTO_GC_BOX) {
                TZrStackValuePointer sourceBase = BASE(A1(instruction));
                TZrStackValuePointer destinationBase =
                        E(instruction) == ZR_INSTRUCTION_USE_RET_FLAG ? ZR_NULL : BASE(E(instruction));
                destination = destinationBase == ZR_NULL ? &ret : FRAME_VALUE_SLOT(E(instruction));
            ...
            DONE(1);
            ZR_INSTRUCTION_LABEL(OWN_RETURN_TO_GC) {
                TZrStackValuePointer sourceBase = BASE(A1(instruction));
                TZrStackValuePointer destinationBase =
                        E(instruction) == ZR_INSTRUCTION_USE_RET_FLAG ? ZR_NULL : BASE(E(instruction));
                destination = destinationBase == ZR_NULL ? &ret : FRAME_VALUE_SLOT(E(instruction));
            ...
            ZR_INSTRUCTION_LABEL(MARK_TO_BE_CLOSED) {
                ...
                SZrTypeValue *closeValue = FRAME_VALUE_SLOT(closeSlot);
                if (closeValue != ZR_NULL && closeValue != &closePointer->value) {
                    execution_copy_stack_value_allowing_overlap_no_profile(state, &closePointer->value, closeValue);
                }
...
#undef EXECUTE_RESET_STACK_NULL2_BODY
#undef EXECUTE_GET_MEMBER_SLOT_BODY
#undef EXECUTE_GET_MEMBER_SLOT_BODY_FAST
#undef EXECUTE_SET_MEMBER_SLOT_BODY
#undef EXECUTE_SET_MEMBER_SLOT_BODY_FAST
#undef EXECUTE_SET_MEMBER_SLOT_NULL_BODY
#undef EXECUTE_SET_MEMBER_SLOT_NULL_BODY_FAST
#undef EXECUTE_GET_BY_INDEX_BODY
#undef EXECUTE_SET_BY_INDEX_BODY
#undef EXECUTE_ITER_INIT_BODY
#undef EXECUTE_ITER_MOVE_NEXT_BODY
#undef EXECUTE_ITER_CURRENT_BODY
#undef EXECUTE_KNOWN_VM_MEMBER_CALL_BODY
#undef EXECUTE_KNOWN_VM_MEMBER_CALL_LOAD1_U8_BODY
#undef EXECUTE_KNOWN_VM_CALL_BODY
#undef EXECUTE_KNOWN_NATIVE_CALL_BODY
#undef EXECUTE_KNOWN_NATIVE_MEMBER_CALL_BODY
#undef EXECUTE_KNOWN_NATIVE_MEMBER_CALL_RECV_U8_BODY
#undef EXECUTE_SUPER_FUNCTION_CALL_NO_ARGS_BODY
...
#undef EXECUTE_SUPER_META_CALL_NO_ARGS_BODY
#undef EXECUTE_SUPER_META_CALL_CACHED_BODY
#undef EXECUTE_META_CALL_BODY
#undef EXECUTE_FUNCTION_TAIL_CALL_BODY
#undef EXECUTE_KNOWN_VM_TAIL_CALL_BODY
#undef EXECUTE_KNOWN_NATIVE_TAIL_CALL_BODY
#undef EXECUTE_SUPER_FUNCTION_TAIL_CALL_NO_ARGS_BODY
#undef EXECUTE_SUPER_KNOWN_VM_TAIL_CALL_NO_ARGS_BODY
#undef EXECUTE_SUPER_KNOWN_NATIVE_TAIL_CALL_NO_ARGS_BODY
#undef EXECUTE_SUPER_DYN_TAIL_CALL_NO_ARGS_BODY
#undef EXECUTE_SUPER_DYN_TAIL_CALL_CACHED_BODY
#undef EXECUTE_DYN_TAIL_CALL_BODY
#undef EXECUTE_SUPER_META_TAIL_CALL_NO_ARGS_BODY
#undef EXECUTE_SUPER_META_TAIL_CALL_CACHED_BODY
#undef EXECUTE_META_TAIL_CALL_BODY
#undef EXECUTE_FUNCTION_RETURN_BODY
...
#undef EXECUTE_TYPED_NEG_SIGNED_BODY
#undef EXECUTE_TYPED_NEG_FLOAT_BODY
#undef EXECUTE_TYPED_TO_FLOAT_SIGNED_BODY
#undef EXECUTE_TYPED_TO_FLOAT_UNSIGNED_BODY
#undef EXECUTE_TYPED_TO_INT_FLOAT_BODY
#undef EXECUTE_TYPED_TO_INT_UNSIGNED_BODY
#undef EXECUTE_TYPED_TO_UINT_FLOAT_BODY
#undef EXECUTE_TYPED_TO_UINT_SIGNED_BODY
...
#undef EXECUTE_ADD_SIGNED_MOD_CONST_BODY
...
#undef EXECUTE_TYPED_LOGICAL_NOT_BOOL_BODY
...
#undef EXECUTE_JUMP_IF_LESS_EQUAL_SIGNED_BODY
#undef EXECUTE_JUMP_IF_NOT_EQUAL_SIGNED_BODY
#undef EXECUTE_JUMP_IF_NOT_EQUAL_SIGNED_CONST_BODY
#undef EXECUTE_JUMP_IF_NULL_BODY
#undef EXECUTE_JUMP_IF_BOOL_FALSE_BODY
