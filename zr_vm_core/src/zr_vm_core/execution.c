...
#define ALGORITHM_2(REGION, OP, TYPE) ZR_VALUE_FAST_SET(&ret, REGION, (opA->value.nativeObject.REGION) OP (opB->value.nativeObject.REGION), TYPE);
#define ALGORITHM_CVT_2(CVT, REGION, OP, TYPE) ZR_VALUE_FAST_SET(&ret, CVT, (opA->value.nativeObject.REGION) OP (opB->value.nativeObject.REGION), TYPE);
#define ALGORITHM_CONST_2(REGION, OP, TYPE, RIGHT) ZR_VALUE_FAST_SET(&ret, REGION, (opA->value.nativeObject.REGION) OP (RIGHT), TYPE);
#define ALGORITHM_FUNC_2(REGION, OP_FUNC, TYPE) ZR_VALUE_FAST_SET(&ret, REGION, OP_FUNC(opA->value.nativeObject.REGION, opB->value.nativeObject.REGION), TYPE);
LZrStart:
    ...
    for (;;) {
        ...
        ZR_INSTRUCTION_DISPATCH(instruction) {
            ZR_INSTRUCTION_LABEL(GET_STACK) {
                ret = BASE(B0(instruction))->value;
            }
                DONE(1);
            ZR_INSTRUCTION_LABEL(SET_STACK) {
                BASE(B0(instruction))->value = ret;
            }
                DONE(1);
            ...
            ZR_INSTRUCTION_LABEL(LOGICAL_EQUAL) {
                opA = &BASE(A0(instruction))->value;
                opB = &BASE(B0(instruction))->value;
                TBool result = ZrValueEqual(opA, opB);
