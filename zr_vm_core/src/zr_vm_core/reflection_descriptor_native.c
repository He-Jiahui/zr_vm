...
static TZrBool descriptor_native_call_frame(
    ...
    *outArgumentCount = 0u;
    ...
    descriptorValue = ZrCore_Stack_GetValue(functionBase + 1);
    ...
    *outArgumentCount = (TZrSize)(state->stackTop.valuePointer - functionBase - 2);
    return ZR_TRUE;
}
...
static TZrInt64 descriptor_native_query(
    ...
    if (singular) {
        ...
        nameValue = ZrCore_Stack_GetValue(functionBase + 2);
        ...
        if (kind == ZR_REFLECTION_MEMBER_KIND_METHOD) {
            ...
            queryValue = argumentCount >= 3u
                                 ? ZrCore_Stack_GetValue(functionBase + 4)
                                 : ZR_NULL;
        } else {
...
static TZrBool descriptor_native_attach_method(
    ...
    TZrBool closurePinned = ZR_FALSE;
    TZrBool success;

    if (existing != ZR_NULL) {
        return existing->type == ZR_VALUE_TYPE_CLOSURE &&
                       existing->value.object != ZR_NULL &&
                       existing->value.object->type == ZR_RAW_OBJECT_TYPE_CLOSURE &&
                       existing->value.object->isNative &&
                       ZR_CAST_NATIVE_CLOSURE(
                               state, existing->value.object)->nativeFunction == entry
               ? ZR_TRUE
               : ZR_FALSE;
    }
    ...
    closure->nativeBindingUsesReceiver = ZR_TRUE;
    ...
    ZrCore_Value_InitAsRawObject(
            state, &closureValue, ZR_CAST_RAW_OBJECT_AS_SUPER(closure));
    closureValue.type = ZR_VALUE_TYPE_CLOSURE;
    success = ZrCore_Reflection_ObjectSetFieldValue(
            state, descriptor, name, &closureValue);
    if (closurePinned) {
        ZrCore_GarbageCollector_UnignoreObject(
                state->global, ZR_CAST_RAW_OBJECT_AS_SUPER(closure));
    }
    return success;
}
