...
static TZrBool convert_io_constant_to_runtime_value(SZrState *state,
...
static TZrBool populate_runtime_function(SZrState *state, const SZrIoFunction *source, SZrFunction *function) {
    ...
    if (source->localVariablesLength > 0) {
        ...
        for (TZrSize i = 0; i < source->localVariablesLength; i++) {
            ...
            function->localVariableList[i].offsetActivate =
                    (TZrMemoryOffset)source->localVariables[i].instructionStartIndex;
            function->localVariableList[i].offsetDead =
                    (TZrMemoryOffset)source->localVariables[i].instructionEndIndex;
        }
    ...
    if (source->constantVariablesLength > 0) {
        ...
        for (TZrSize i = 0; i < source->constantVariablesLength; i++) {
            if (!convert_io_constant_to_runtime_value(
                        state, &source->constantVariables[i], &function->constantValueList[i])) {
                ZrCore_Function_Free(state, function);
                return ZR_FALSE;
            }
        }
