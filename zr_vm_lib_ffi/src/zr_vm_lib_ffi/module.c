...
static const ZrLibFunctionDescriptor g_ffi_functions[] = {
        {"loadLibrary", 1, 1, ZrFfi_LoadLibrary, "LibraryHandle",
         "Load a dynamic library by absolute or relative path."},
        {"callback", 2, 2, ZrFfi_CreateCallback, "CallbackHandle",
         "Create a C-callable callback trampoline from a zr closure."},
        {"sizeof", 1, 1, ZrFfi_SizeOf, "int", "Return the byte size of a runtime FFI type descriptor."},
        {"alignof", 1, 1, ZrFfi_AlignOf, "int", "Return the alignment of a runtime FFI type descriptor."},
        {"nullPointer", 1, 1, ZrFfi_NullPointer, "PointerHandle", "Create a typed null pointer handle."},
};

static const ZrLibMethodDescriptor g_library_methods[] = {
        {"close", 0, 0, ZrFfi_Library_Close, "null", "Close the library handle. Idempotent.", ZR_FALSE},
...
static const ZrLibMethodDescriptor g_symbol_methods[] = {
        {"call", 1, 1, ZrFfi_Symbol_Call, "value", "Call the compiled symbol with an argument array.", ZR_FALSE},
};

static const ZrLibMetaMethodDescriptor g_symbol_meta_methods[] = {
        {ZR_META_CALL, 0, (TZrUInt16)0xFFFFu, ZrFfi_Symbol_MetaCall, "value",
         "Call the compiled symbol directly with positional arguments."},
};

static const ZrLibMethodDescriptor g_callback_methods[] = {
        {"close", 0, 0, ZrFfi_Callback_Close, "null", "Close the callback handle. Idempotent.", ZR_FALSE},
};
...
static const ZrLibMethodDescriptor g_pointer_methods[] = {
        ...
        {"close", 0, 0, ZrFfi_Pointer_Close, "null", "Release any ownership held by the pointer handle.", ZR_FALSE},
};

static const ZrLibMethodDescriptor g_buffer_methods[] = {
        {"allocate", 1, 1, ZrFfi_Buffer_Allocate, "BufferHandle", "Allocate a managed native buffer.", ZR_TRUE},
        {"close", 0, 0, ZrFfi_Buffer_Close, "null", "Close the buffer handle. Idempotent.", ZR_FALSE},
        {"pin", 0, 0, ZrFfi_Buffer_Pin, "PointerHandle", "Pin the buffer and return a pointer view.", ZR_FALSE},
        {"read", 2, 2, ZrFfi_Buffer_Read, "array", "Read a byte range from the buffer.", ZR_FALSE},
        {"write", 2, 2, ZrFfi_Buffer_Write, "int", "Write bytes into the buffer from an array.", ZR_FALSE},
        {"slice", 2, 2, ZrFfi_Buffer_Slice, "BufferHandle", "Create a copied slice of the buffer.", ZR_FALSE},
};
...
static const ZrLibTypeDescriptor g_ffi_types[] = {
        {"LibraryHandle", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_library_methods,
         ...
         0, ZR_NULL, ZR_FALSE, ZR_TRUE, "LibraryHandle(path: string)"},
        {"SymbolHandle", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_symbol_methods, ZR_ARRAY_COUNT(g_symbol_methods),
         ...
         ZR_NULL, 0, ZR_NULL, ZR_FALSE, ZR_TRUE, "SymbolHandle()"},
        {"CallbackHandle", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_callback_methods,
         ...
         ZR_NULL, 0, ZR_NULL, 0, ZR_NULL, ZR_FALSE, ZR_TRUE, "CallbackHandle(signature: object, fn: function)"},
        {"PointerHandle", ZR_OBJECT_PROTOTYPE_TYPE_CLASS, ZR_NULL, 0, g_pointer_methods,
         ...
         0, ZR_NULL, 0, ZR_NULL, ZR_FALSE, ZR_TRUE, "PointerHandle(type: object)"},
